...
export function applyDynamicContent(lang) {
  ...
  const depGrid = document.getElementById('dep-grid')
  if (depGrid) {
    const depsRaw = getContent('testimonials', lang) || getContent('testimonials', 'pt')
    if (depsRaw) {
      try {
        const deps = JSON.parse(depsRaw)
        if (Array.isArray(deps) && deps.length > 0) {
          const avatarColors = ['#0d2144','#1a3a5c','#0a1628','#164a3c','#2d1b3d','#3d1a1a','#1a2f4a']
          function esc(s) { return String(s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;') }
          function aColor(name) {
            let h = 0; for (const c of (name||'?')) h = (h*31+c.charCodeAt(0)) & 0xffffffff
            return avatarColors[Math.abs(h) % avatarColors.length]
          }
          depGrid.innerHTML = deps.map(d => `
            <div class="dep-card-v2">
              <div class="dep-stars-v2">${'★'.repeat(d.stars||5)}</div>
              <p class="dep-text-v2">"${esc(d.text)}"</p>
              <div class="dep-author-v2">
                <div class="dep-avatar-v2" style="background:${aColor(d.name)}">${(d.name||'?')[0].toUpperCase()}</div>
                <div>
                  <div class="dep-name-v2">${esc(d.name)}</div>
                  <div class="dep-role-v2">${esc(d.role)}</div>
                </div>
              </div>
            </div>`).join('')
        }
