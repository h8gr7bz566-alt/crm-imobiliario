...
// UUID reservado para configurações globais (super_admin sem tenant)
...
export function setSettingsTenant(tenantId) {
  ...
  _s = {}; _c = {}  // limpa cache ao trocar tenant
}
...
export async function loadAllSettings() {
  ...
  // Fallback: se não há conteúdo para o tenant do domínio, tenta GLOBAL_TENANT_ID
  // Isso garante que Super Admin (sem tenant específico) consiga publicar para o site
  const hasContent = cr?.data?.length > 0
  if (!hasContent && _tenantId !== GLOBAL_TENANT_ID) {
    const [sgr, cgr] = await Promise.all([
    ...
    if (sgr.data) sgr.data.forEach(r => { if (_s[r.key] === undefined) _s[r.key] = r.value })
    if (cgr.data) cgr.data.forEach(r => { if (!_c[r.key]) _c[r.key] = r })
    console.info('[ImobiCRM] settings: fallback para GLOBAL_TENANT_ID')
  }
  console.info('[ImobiCRM] tenant:', _tenantId, '| settings:', Object.keys(_s).length, '| content:', Object.keys(_c).length)
}
...
export async function saveContent(key, { pt, en, es }) {
  const row = { key, value_pt: pt, value_en: en, value_es: es, tenant_id: _tenantId, updated_at: new Date().toISOString() }
  const { error } = await supabase
    .from('site_content')
...
export function applyDynamicContent(lang = 'pt') {
  ...
  // Hero — subtítulo (usa data-i18n="hero.subtitle" no elemento)
  ...
  // Stats institucionais — números e legendas
