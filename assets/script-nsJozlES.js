...
${l}`),c=e.area?`<span class="icard-spec"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18M9 3v18"/></svg>${e.area}m²</span>`:"",r=e.bedrooms?`<span class="icard-spec"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M2 20v-6a2 2 0 012-2h16a2 2 0 012 2v6"/><path d="M2 14V8a2 2 0 012-2h4l2 3h8a2 2 0 012 2v3"/></svg>${e.bedrooms} quarto${e.bedrooms!=1?"s":""}</span>`:"",m=e.bathrooms?`<span class="icard-spec"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 6 6.5 3.5a1.5 1.5 0 000-2.12L6 1.5a1.5 1.5 0 00-2.12 0L2 3.38a1.5 1.5 0 000 2.12L5.5 9"/><path d="M2 20h20M20 12H4a2 2 0 00-2 2v4a2 2 0 002 2h16a2 2 0 002-2v-4a2 2 0 00-2-2z"/></svg>${e.bathrooms} banheiro${e.bathrooms!=1?"s":""}</span>`:"",p=e.parking?`<span class="icard-spec"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="1" y="3" width="15" height="13" rx="2"/><path d="M16 8h4l3 5v3h-7V8z"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg>${e.parking} vaga${e.parking!=1?"s":""}</span>`:"",b=Math.max(1,Math.min(a,6)),E=`<div class="icard-dots">${Array.from({length:b},(y,g)=>`<span class="icard-dot${g===0?" active":""}"></span>`).join("")}</div>`;return`
      ...
      <div class="modal-footer">
        ...
        <div class="kanban-card" draggable="true" data-id="${d.id}" data-stage="${f(i.name)}" style="cursor:pointer;">
          ...
          <div class="kanban-card-tags" style="display:flex;flex-wrap:wrap;gap:4px;margin-top:6px;">
            ...
            ${Array.isArray(d.tags)?d.tags.map(m=>{const p=me[m],b=(p==null?void 0:p.color)||"#0369a1";return`<span class="kanban-card-tag" style="background:${b}18;color:${b};border:1px solid ${b}44;">${f(m)}</span>`}).join(""):""}
          </div>
  ...
  `,document.body.appendChild(r),requestAnimationFrame(()=>{r.style.transform="translateX(0)"}),gn(r,a||[],me);const m=r.querySelector("#ldp-pipe"),p=r.querySelector("#ldp-stage");m&&p&&m.addEventListener("change",()=>{const g=m.value,x=Je.filter(I=>I.pipeline_id===g);p.innerHTML=x.map(I=>`<option value="${I.name}">${I.name}</option>`).join("")||'<option value="">— sem etapas —</option>'});const b=()=>{r.style.transform="translateX(100%)",setTimeout(()=>r.remove(),250)};document.getElementById("ldp-close").addEventListener("click",b),document.getElementById("ldp-save").addEventListener("click",async()=>{var T,_;const g=document.getElementById("ldp-save"),x=document.getElementById("ldp-msg"),I=document.getElementById("ldp-name").value.trim();if(!I){x.style.color="#ef4444",x.textContent="Nome é obrigatório.";return}g.disabled=!0,g.textContent="Salvando…";const S=[...r.querySelectorAll("#ldp-tag-badge-area .ldp-tag-badge[data-tag]")].map(q=>q.dataset.tag),L=document.getElementById("ldp-pipe"),k=L?L.value:i||Q,w=t?typeof $t=="function"?$t():{}:{},$={name:I,phone:document.getElementById("ldp-phone").value.trim()||null,email:document.getElementById("ldp-email").value.trim()||null,source:document.getElementById("ldp-source").value.trim()||null,pipeline_id:k||null,stage:((T=document.getElementById("ldp-stage"))==null?void 0:T.value)||null,status:((_=document.getElementById("ldp-status"))==null?void 0:_.value)||null,notes:document.getElementById("ldp-notes").value.trim()||null,tags:S,tenant_id:z(),...t?{utm_source:w.utm_source||null,utm_medium:w.utm_medium||null,utm_campaign:w.utm_campaign||null,utm_content:w.utm_content||null,utm_term:w.utm_term||null,fbclid:w.fbclid||null,gclid:w.gclid||null,fbp:w.fbp||null,fbc:w.fbc||null,landing_url:w.landing_url||null,user_agent:navigator.userAgent||null}:{}};let B;if(t?{error:B}=await h.from("leads").insert($):{error:B}=await h.from("leads").update($).eq("id",e.id),g.disabled=!1,g.textContent="💾 Salvar",B){x.style.color="#ef4444",x.textContent="Erro: "+B.message;return}x.style.color="#22c55e",x.textContent="✅ Salvo!",t&&typeof sendLeadToCAPI=="function"&&sendLeadToCAPI({name:$.name,email:$.email,phone:$.phone,tracking:w}).then(q=>{q!=null&&q.ok&&console.log("[CAPI] Lead enviado ao Meta:",q.event_id)}).catch(()=>{}),setTimeout(()=>{b(),Ye()},700)}),(y=document.getElementById("ldp-delete"))==null||y.addEventListener("click",async()=>{confirm(`Excluir o lead "${e==null?void 0:e.name}"?`)&&(await h.from("leads").delete().eq("id",e.id),b(),Ye())})}let D=[],Ct=!1,he="pending";async function Da(){var e;Ct||(Ct=!0,await ja(),(e=document.getElementById("btn-nova-tarefa"))==null||e.addEventListener("click",()=>Wt()),document.querySelectorAll(".tarefa-filter-btn").forEach(t=>{t.addEventListener("click",()=>{document.querySelectorAll(".tarefa-filter-btn").forEach(n=>n.classList.remove("active")),t.classList.add("active"),he=t.dataset.filter,$e()})}))}async function ja(){const e=document.getElementById("tarefas-list");e&&(e.innerHTML='<div class="empty-row" style="padding:40px;text-align:center;color:#94a3b8;">Carregando…</div>');let t=h.from("tasks").select("*").order("due_date",{ascending:!0,nullsFirst:!1});(v==null?void 0:v.role)==="corretor"?t=t.eq("assigned_to",v.id):v!=null&&v.tenant_id&&(t=t.eq("tenant_id",v.tenant_id));const{data:n,error:a}=await t;if(a){const o=document.getElementById("tarefas-list");o&&(o.innerHTML=`
...
João Silva,joao@email.com,(47)99999-0000,Imobiliária ABC,Diretor`,p=new Blob([m],{type:"text/csv"}),b=document.createElement("a");b.href=URL.createObjectURL(p),b.download="modelo_contatos.csv",b.click()}),(d=document.getElementById("import-csv-file"))==null||d.addEventListener("change",Va),(c=document.getElementById("import-modal-confirm"))==null||c.addEventListener("click",Ga)))}async function Yt(){const e=document.getElementById("contatos-tbody");e&&(e.innerHTML='<tr><td colspan="8" class="empty-row">Carregando…</td></tr>');let t=h.from("leads").select("*").order("created_at",{ascending:!1});(v==null?void 0:v.role)==="corretor"?t=t.eq("assigned_to",v.id):v!=null&&v.tenant_id&&(t=t.eq("tenant_id",v.tenant_id));const{data:a}=await t;Y=a||[],pe()}function pe(){var l,d,c;const e=(((l=document.getElementById("contato-search"))==null?void 0:l.value)||"").toLowerCase(),t=e?Y.filter(r=>(r.name||"").toLowerCase().includes(e)||(r.email||"").toLowerCase().includes(e)||(r.phone||"").toLowerCase().includes(e)):Y,n=t.length,a=Math.max(1,Math.ceil(n/_e));R>a&&(R=a);const o=t.slice((R-1)*_e,R*_e),i=document.getElementById("contatos-tbody");if(!i)return;o.length?i.innerHTML=o.map(r=>`
    ...
    `).join(""):i.innerHTML='<tr><td colspan="8" class="empty-row">Nenhum contato encontrado.</td></tr>';const s=document.getElementById("contatos-pagination");if(s){const r=n===0?0:(R-1)*_e+1,m=Math.min(R*_e,n);s.innerHTML=`
      <div style="display:flex;align-items:center;justify-content:space-between;padding:12px 16px;font-size:13px;color:#64748b;">
        ...
        <div style="display:flex;gap:8px;">
          <button class="btn-cancel" id="pag-prev" ${R<=1?"disabled":""} style="padding:4px 12px;font-size:13px;">Anterior</button>
          <span style="padding:4px 8px;">${R} / ${a}</span>
          <button class="btn-cancel" id="pag-next" ${R>=a?"disabled":""} style="padding:4px 12px;font-size:13px;">Próxima</button>
        </div>
      </div>
    `,(d=s.querySelector("#pag-prev"))==null||d.addEventListener("click",()=>{R--,pe()}),(c=s.querySelector("#pag-next"))==null||c.addEventListener("click",()=>{R++,pe()})}document.querySelectorAll(".contato-edit-btn, .contato-name-link").forEach(r=>{r.addEventListener("click",m=>{m.preventDefault();const p=r.dataset.id,b=Y.find(E=>String(E.id)===String(p));b&&Kt(b)})})}async function Kt(e=null){var g,x,I,S,L,k;const t=document.getElementById("contato-modal-root");t&&t.remove();const n=!!e,a=z(),[{data:o},{data:i},{data:s}]=await Promise.all([h.from("crm_pipelines").select("*").eq("tenant_id",a).order("sort_order"),h.from("crm_tags").select("*").eq("tenant_id",a).order("name"),h.from("crm_lead_statuses").select("*").eq("tenant_id",a).order("sort_order")]),l=o||[],d=i||[],c=s||[],r=l.map(w=>w.id),{data:m}=r.length?await h.from("crm_stages").select("*").in("pipeline_id",r).order("sort_order"):{data:[]},p=m||[],b=(e==null?void 0:e.pipeline_id)||((g=l[0])==null?void 0:g.id)||"";function E(w){const $=p.filter(B=>B.pipeline_id===w);return $.length?'<option value="">— Selecionar etapa —</option>'+$.map(B=>`<option value="${f(B.name)}" ${(e==null?void 0:e.stage)===B.name?"selected":""}>${f(B.name)}</option>`).join(""):'<option value="">— Nenhuma etapa —</option>'}const u=document.createElement("div");u.id="contato-modal-root",u.className="modal-backdrop",u.innerHTML=`
...
`+k);window.open("mailto:?subject="+$+"&body="+B,"_blank")}),(I=document.getElementById("share-copy"))==null||I.addEventListener("click",()=>{var w;const k=document.getElementById("share-link-input");k&&((w=navigator.clipboard)==null||w.writeText(k.value).then(()=>{const $=document.getElementById("share-copy"),B=$.textContent;$.textContent="✅ Copiado!",setTimeout(()=>{$.textContent=B},2e3)}))}),(S=document.getElementById("view-modal-edit"))==null||S.addEventListener("click",()=>{var q;if((v==null?void 0:v.role)!=="admin"&&(v==null?void 0:v.role)!=="super_admin")return;const k=Number(document.getElementById("view-modal-edit").dataset.pid),w=C.find(A=>A.id===k);if(!w)return;De(),P=w.id;const $=document.getElementById("property-form"),B=document.getElementById("form-submit-btn");B.textContent="Salvar Alterações",$.querySelector('[name="title"]').value=w.title||"",$.querySelector('[name="rua"]').value=w.rua||"",$.querySelector('[name="numero"]').value=w.numero||"",$.querySelector('[name="city"]').value=w.city||"",$.querySelector('[name="price"]').value=w.price||"",$.querySelector('[name="bedrooms"]').value=w.bedrooms||"",$.querySelector('[name="suites"]').value=w.suites||"",$.querySelector('[name="parking"]').value=w.parking||"",$.querySelector('[name="description"]').value=w.description||"",$.querySelector('[name="construction_status"]').value=w.construction_status||"",$.querySelector('[name="owner_name"]').value=w.owner_name||"",$.querySelector('[name="owner_phone"]').value=w.owner_phone||"",$.querySelector('[name="owner_email"]').value=w.owner_email||"",$.querySelector('[name="owner_notes"]').value=w.owner_notes||"",$.querySelector('[name="condominium"]').value=w.condominium||"";const T=document.getElementById("adminPublished");T&&(T.value=w.published===!0?"true":"false");const _=document.getElementById("adminCitySelect");_&&(_.value=w.city||"",_.dispatchEvent(new Event("change")),setTimeout(()=>{const A=document.getElementById("adminNeighborhood");A&&(A.value=w.neighborhood||"")},50)),ee=w.cover_image||((q=w.images)==null?void 0:q[0])||"",ze(w.images||[]),nt("Editar Imóvel")}),document.querySelectorAll(".tab-btn[data-tab]").forEach(k=>{k.addEventListener("click",()=>{var w;document.querySelectorAll(".tab-btn").forEach($=>$.classList.remove("active")),k.classList.add("active"),document.querySelectorAll(".tab-panel").forEach($=>$.classList.add("hidden")),(w=document.getElementById(`tab-${k.dataset.tab}`))==null||w.classList.remove("hidden")})}),(L=document.getElementById("admin-properties"))==null||L.addEventListener("click",k=>{if(k.target.closest(".action-btns"))return;const w=k.target.closest("tr");if(!w)return;const $=Number(w.dataset.id);if(!$)return;const B=C.find(T=>T.id===$);B&&Aa(B)})}async function Zt(){const e=document.getElementById("section-depoimentos");if(!e||e.dataset.loaded)return;e.dataset.loaded="1";const{data:t}=await h.from("site_content").select("value_pt").eq("key","testimonials").eq("tenant_id",z()).maybeSingle();let n=[];try{n=JSON.parse((t==null?void 0:t.value_pt)||"[]")}catch{n=[]}function a(s){const l=["#0d2144","#1a3a5c","#0a1628","#164a3c","#2d1b3d","#3d1a1a","#1a2f4a"];let d=0;for(const c of s||"?")d=d*31+c.charCodeAt(0)&4294967295;return l[Math.abs(d)%l.length]}function o(){e.querySelector("#dep-save-msg"),e.innerHTML=`
  ...
  `,await U())}async function U(){var b,E;const e=document.getElementById("crm-body");if(!e)return;const t=z(),[{data:n},{data:a},{data:o},{data:i}]=await Promise.all([h.from("crm_pipelines").select("*").eq("tenant_id",t).order("sort_order"),h.from("crm_stages").select("*").eq("tenant_id",t).order("sort_order"),h.from("crm_tags").select("*").eq("tenant_id",t).order("name"),h.from("crm_lead_statuses").select("*").eq("tenant_id",t).order("sort_order")]),s=n||[],l=s.find(u=>u.is_default)||s[0],d=s.map(u=>`<option value="${u.id}"${u.id===(l==null?void 0:l.id)?" selected":""}>${f(u.name)}</option>`).join(""),r=(a||[]).filter(u=>u.pipeline_id===(l==null?void 0:l.id)).map(u=>`
    ...
    <div style="background:#fff;border-radius:16px;width:100%;max-width:680px;max-height:90vh;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 24px 60px rgba(0,0,0,.25);">
      ...
      <div style="background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 1px 6px rgba(0,0,0,.07);">
        ...
        <table style="width:100%;border-collapse:collapse;min-width:560px;">
          ...
          <tbody>${r.map(p=>{const[b,E]=m(p.stage||p.status||"").split(","),u=(p.phone||"").replace(/\D/g,"");return`<tr data-lid="${p.id}" style="border-top:1px solid #f1f5f9;">
              ...
              <td style="padding:12px 16px;"><span style="background:${b};color:${E};font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px;">${f(p.stage||p.status||"Novo")}</span></td>
      ...
      <div style="display:grid;gap:20px;max-width:800px;">
        ...
        <div style="background:#fff;border-radius:12px;padding:24px;box-shadow:0 1px 6px rgba(0,0,0,.07);">
          ...
          <a href="${f(p)}" target="_blank" style="display:inline-block;background:#c9a84c;color:#fff;padding:10px 20px;border-radius:8px;text-decoration:none;font-size:13px;font-weight:700;">${u}</a>
      ...
      </div>`,(c=document.getElementById("tp-open-edit"))==null||c.addEventListener("click",()=>sa(e))}}function sa(e){var c,r,m,p,b,E,u,y;const t=document.getElementById("sa-edit-tenant-modal");t&&t.remove();const n=document.createElement("div");n.id="sa-edit-tenant-modal",n.className="sa-modal-backdrop";const a="https://onknpbzdcrhbfozzvxtz.supabase.co/functions/v1/public-api";n.innerHTML=`
...
await fetch(\`\${API}/leads?key=\${KEY}\`, {
  ...
  `,document.body.appendChild(n),h.from("plans").select("id, name").then(({data:g})=>{const x=document.getElementById("et-plan");x&&g&&(x.innerHTML='<option value="">Sem plano</option>'+g.map(I=>`<option value="${I.id}"${String(I.id)===String(e.plan_id)?" selected":""}>${f(I.name)}</option>`).join(""))}),(c=document.getElementById("et-logo-input"))==null||c.addEventListener("change",g=>{const x=g.target.files[0];if(!x)return;const I=URL.createObjectURL(x),S=document.getElementById("et-logo-preview");S&&(S.innerHTML=`<img src="${I}" style="width:100%;height:100%;object-fit:cover;">`)}),(r=document.getElementById("et-logo-preview"))==null||r.addEventListener("click",()=>{var g;(g=document.getElementById("et-logo-input"))==null||g.click()}),(m=document.getElementById("et-pwd-toggle"))==null||m.addEventListener("click",()=>{const g=document.getElementById("et-admin-password");g.type=g.type==="password"?"text":"password"}),(p=document.getElementById("et-copy-key"))==null||p.addEventListener("click",()=>{var S,L;const g=(S=document.getElementById("et-api-key"))==null?void 0:S.value;if(!g)return;(L=navigator.clipboard)==null||L.writeText(g);const x=document.getElementById("et-copy-key"),I=x.textContent;x.textContent="✅ Copiada!",setTimeout(()=>{x.textContent=I},2e3)});const o=["dados","config","api"];function i(g){o.forEach(x=>{document.getElementById(`et-pane-${x}`).style.display=x===g?"":"none";const I=document.getElementById(`et-tab-${x}`);I.style.borderBottomColor=x===g?"#2563eb":"transparent",I.style.color=x===g?"#2563eb":"#64748b",I.style.fontWeight=x===g?"600":"500"}),g==="config"&&l()}o.forEach(g=>{var x;return(x=document.getElementById(`et-tab-${g}`))==null?void 0:x.addEventListener("click",()=>i(g))});let s=!1;async function l(){var I;if(s)return;s=!0;const{data:g}=await h.from("settings").select("key,value").eq("tenant_id",e.id),x={};g==null||g.forEach(S=>{x[S.key]=S.value}),document.getElementById("et-pane-config").innerHTML=`
    ...
    `,(I=document.getElementById("et-cfg-save"))==null||I.addEventListener("click",async()=>{const S=document.getElementById("et-cfg-save"),L=document.getElementById("et-cfg-msg");S.disabled=!0,S.textContent="Salvando…",L.textContent="",L.style.color="#64748b";const k=document.getElementById("et-cfg-wa").value.trim().replace(/\D/g,""),w=document.getElementById("et-cfg-phone").value.trim(),$=document.getElementById("et-cfg-email").value.trim(),B=document.getElementById("et-cfg-city").value.trim(),T=document.getElementById("et-cfg-slogan").value.trim(),{error:_}=await h.from("settings").upsert([{key:"company.whatsapp",value:k,tenant_id:e.id},{key:"company.phone",value:w,tenant_id:e.id},{key:"company.email",value:$,tenant_id:e.id},{key:"company.city",value:B,tenant_id:e.id},{key:"company.address",value:B,tenant_id:e.id},{key:"company.slogan",value:T,tenant_id:e.id}],{onConflict:"tenant_id,key"});S.disabled=!1,S.textContent="💾 Salvar configurações",_?(L.textContent="❌ "+_.message,L.style.color="#ef4444"):(L.textContent="✅ Configurações salvas!",L.style.color="#22c55e")})}const d=()=>n.remove();(b=document.getElementById("et-close"))==null||b.addEventListener("click",d),(E=document.getElementById("et-cancel"))==null||E.addEventListener("click",d),n.addEventListener("click",g=>{g.target===n&&d()}),(u=document.getElementById("et-delete"))==null||u.addEventListener("click",async()=>{if(!confirm(`⚠️ Tem certeza que deseja EXCLUIR a imobiliária "${e.name}"?

Essa ação é irreversível e removerá o registro da plataforma.`))return;const x=document.getElementById("et-delete");x.disabled=!0,x.textContent="Excluindo…";const{error:I}=await h.from("tenants").delete().eq("id",e.id);if(I){alert("Erro ao excluir: "+I.message),x.disabled=!1,x.textContent="🗑️ Excluir";return}d(),de()}),(y=document.getElementById("et-save"))==null||y.addEventListener("click",async()=>{var q,A,V,j,O,ne,re,ke,Se,H,ve,ht;const g=(A=(q=document.getElementById("et-name"))==null?void 0:q.value)==null?void 0:A.trim(),x=(j=(V=document.getElementById("et-slug"))==null?void 0:V.value)==null?void 0:j.trim(),I=(ne=(O=document.getElementById("et-domain"))==null?void 0:O.value)==null?void 0:ne.trim(),S=(re=document.getElementById("et-plan"))==null?void 0:re.value,L=(Se=(ke=document.getElementById("et-admin-email"))==null?void 0:ke.value)==null?void 0:Se.trim(),k=(ve=(H=document.getElementById("et-admin-password"))==null?void 0:H.value)==null?void 0:ve.trim(),w=(ht=document.getElementById("et-logo-input"))==null?void 0:ht.files[0],$=document.getElementById("et-msg"),B=document.getElementById("et-save");if(!g){$.textContent="❌ Nome é obrigatório.",$.style.color="#ef4444";return}B.disabled=!0,B.textContent="Salvando…",$.textContent="⏳ Salvando…",$.style.color="#64748b";let T=e.logo_url;if(w)try{const M=await Re(w,256,.85),xt=`tenant-logos/${e.id}-${Date.now()}.jpg`,{error:ma}=await h.storage.from("imoveis").upload(xt,M,{contentType:"image/jpeg",upsert:!0});if(!ma){const{data:{publicUrl:pa}}=h.storage.from("imoveis").getPublicUrl(xt);T=pa}}catch(M){console.error("Logo upload:",M)}const{error:_}=await h.from("tenants").update({name:g,slug:x||e.slug,domain:I||null,plan_id:S||null,logo_url:T}).eq("id",e.id);if(_){B.disabled=!1,B.textContent="Salvar",$.textContent="❌ "+_.message,$.style.color="#ef4444";return}if(L&&k&&k.length>=6){$.textContent="⏳ Criando usuário admin…";const M=await ue({email:L,password:k,role:"admin",tenant_id:e.id});M!=null&&M.success?(M!=null&&M.user_id&&!(M!=null&&M.linked)&&await h.from("profiles").update({tenant_id:e.id}).eq("id",M.user_id),$.textContent="✅ Salvo e admin criado!",$.style.color="#22c55e"):($.textContent="⚠️ Salvo, mas erro ao criar admin: "+((M==null?void 0:M.error)||"Tente novamente"),$.style.color="#f59e0b")}else $.textContent="✅ Imobiliária atualizada!",$.style.color="#22c55e";B.disabled=!1,B.textContent="Salvar",setTimeout(()=>{d(),de()},1200)})}const Rt=[{key:"name",label:"Nome",required:!0},{key:"phone",label:"Telefone",required:!1},{key:"email",label:"E-mail",required:!1},{key:"notes",label:"Notas",required:!1}];let Qe=[],fe=[],Pe={};function dn(){var e;(e=document.getElementById("btn-import-leads"))==null||e.addEventListener("click",rn)}function rn(){Qe=[],fe=[],Pe={};const e=document.createElement("div");e.id="import-leads-overlay",e.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;",e.innerHTML=`
...
`).map(l=>l.trimEnd()).filter(l=>l.length);if(a.length<2)return{error:"Arquivo vazio ou sem dados."};function o(l){const d=[];let c="",r=!1;for(let m=0;m<l.length;m++){const p=l[m];p==='"'?r&&l[m+1]==='"'?(c+='"',m++):r=!r:p===n&&!r?(d.push(c.trim()),c=""):c+=p}return d.push(c.trim()),d}const i=o(a[0]).map(l=>l.replace(/^["']+|["']+$/g,"")),s=a.slice(1).map(o);return{headers:i,rows:s}}async function cn(e){const t=document.getElementById("import-upload-error");try{window.XLSX||await new Promise((i,s)=>{const l=document.createElement("script");l.src="https://cdnjs.cloudflare.com/ajax/libs/xlsx/0.18.5/xlsx.full.min.js",l.onload=i,l.onerror=s,document.head.appendChild(l)});const n=window.XLSX.read(e,{type:"array"}),a=n.Sheets[n.SheetNames[0]],o=window.XLSX.utils.sheet_to_json(a,{header:1,defval:""});if(!o||o.length<2){t.textContent="Planilha vazia.",t.style.display="";return}fe=o[0].map(String),Qe=o.slice(1),ra()}catch(n){t.textContent="Erro ao ler o arquivo Excel: "+n.message,t.style.display=""}}function mn(e,t){for(let n=0;n<e.length;n++){const a=e[n].toLowerCase();if(t.some(o=>a.includes(o)))return n}return""}function ra(){document.getElementById("import-step-upload").style.display="none",document.getElementById("import-step-map").style.display="";const e=document.getElementById("import-field-rows"),t={name:["nome","name","contact","cliente","contato"],phone:["tel","fone","celular","whatsapp","phone","mobile"],email:["email","e-mail","mail"],notes:["obs","nota","note","comment","coment","descri"]},n='<option value="">— ignorar —</option>'+fe.map((a,o)=>`<option value="${o}">${f(a)}</option>`).join("");e.innerHTML=Rt.map(a=>{const o=mn(fe,t[a.key]||[]);return Pe[a.key]=o!==""?parseInt(o):"",`
      ...
      </span>`}).join("")}function u(I=""){const S=b(),L=I.toLowerCase().trim(),k=t.filter(w=>!L||w.name.toLowerCase().includes(L));if(!k.length){l.innerHTML='<div class="ldp-tag-opt-empty">Nenhuma tag encontrada</div>';return}l.innerHTML=k.map(w=>{const $=S.includes(w.name);return`<div class="ldp-tag-opt${$?" active":""}" data-tag="${f(w.name)}" style="--tc:${w.color}">
...
</div>`;const n=new Date,a=n.getHours(),o=a<12?"Bom dia":a<18?"Boa tarde":"Boa noite",i=["Domingo","Segunda-feira","Terça-feira","Quarta-feira","Quinta-feira","Sexta-feira","Sábado"],s=["janeiro","fevereiro","março","abril","maio","junho","julho","agosto","setembro","outubro","novembro","dezembro"],l=`${i[n.getDay()]}, ${n.getDate()} de ${s[n.getMonth()]} de ${n.getFullYear()}`,d=((y=v==null?void 0:v.name)==null?void 0:y.split(" ")[0])||"Corretor",c=e.querySelector(".db-greeting");c&&(c.innerHTML=`${o}, <span class="db-greeting-name">${N(d)}</span> 👋`);const r=document.getElementById("db-subline");r&&(r.textContent=`Aqui está o resumo do seu negócio — ${l}`);let m=[],p=[];try{const[g,x]=await Promise.all([Pt(),vn()]);m=g||[],p=x||[]}catch(g){console.warn("[Dashboard] Erro ao carregar dados:",g)}yn(m,p,n);const b=m.filter(g=>g.published).length,E=p.filter(g=>mt(g.created_at)===mt(n.toISOString())).length,u=document.getElementById("db-header-chips");u&&(u.innerHTML=`
