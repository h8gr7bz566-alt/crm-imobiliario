...
Essa ação é irreversível e removerá o registro da plataforma.`))return;const E=document.getElementById("et-delete");E.disabled=!0,E.textContent="Excluindo…";const{error:B}=await y.from("tenants").delete().eq("id",e.id);if(B){alert("Erro ao excluir: "+B.message),E.disabled=!1,E.textContent="🗑️ Excluir";return}l(),Q()}),(x=document.getElementById("et-save"))==null||x.addEventListener("click",async()=>{var N,A,G,V,W,ve,fe,ye,be,R,se,nt;const v=(A=(N=document.getElementById("et-name"))==null?void 0:N.value)==null?void 0:A.trim(),E=(V=(G=document.getElementById("et-slug"))==null?void 0:G.value)==null?void 0:V.trim(),B=(ve=(W=document.getElementById("et-domain"))==null?void 0:W.value)==null?void 0:ve.trim(),k=(fe=document.getElementById("et-plan"))==null?void 0:fe.value,S=(be=(ye=document.getElementById("et-admin-email"))==null?void 0:ye.value)==null?void 0:be.trim(),$=(se=(R=document.getElementById("et-admin-password"))==null?void 0:R.value)==null?void 0:se.trim(),w=(nt=document.getElementById("et-logo-input"))==null?void 0:nt.files[0],I=document.getElementById("et-msg"),L=document.getElementById("et-save");if(!v){I.textContent="❌ Nome é obrigatório.",I.style.color="#ef4444";return}L.disabled=!0,L.textContent="Salvando…",I.textContent="⏳ Salvando…",I.style.color="#64748b";let C=e.logo_url;if(w)try{const q=await Se(w,256,.85),ot=`tenant-logos/${e.id}-${Date.now()}.jpg`,{error:Lt}=await y.storage.from("imoveis").upload(ot,q,{contentType:"image/jpeg",upsert:!0});if(!Lt){const{data:{publicUrl:St}}=y.storage.from("imoveis").getPublicUrl(ot);C=St}}catch(q){console.error("Logo upload:",q)}const{error:T}=await y.from("tenants").update({name:v,slug:E||e.slug,domain:B||null,plan_id:k||null,logo_url:C}).eq("id",e.id);if(T){L.disabled=!1,L.textContent="Salvar",I.textContent="❌ "+T.message,I.style.color="#ef4444";return}if(S&&$&&$.length>=6){I.textContent="⏳ Criando usuário admin…";const q=await ge({email:S,password:$,role:"admin",tenant_id:e.id});q!=null&&q.success?(q!=null&&q.user_id&&!(q!=null&&q.linked)&&await y.from("profiles").update({tenant_id:e.id}).eq("id",q.user_id),I.textContent="✅ Salvo e admin criado!",I.style.color="#22c55e"):(I.textContent="⚠️ Salvo, mas erro ao criar admin: "+((q==null?void 0:q.error)||"Tente novamente"),I.style.color="#f59e0b")}else I.textContent="✅ Imobiliária atualizada!",I.style.color="#22c55e";L.disabled=!1,L.textContent="Salvar",setTimeout(()=>{l(),Q()},1200)})}
