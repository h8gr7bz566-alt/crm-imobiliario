...
CREATE POLICY "tasks_access" ON public.tasks
  ...
  WITH CHECK (auth.uid() IS NOT NULL);</code>
      </div>`);return}D=n||[],$e()}function Ft(e){if(!e)return null;const t=e.includes("T")?new Date(e):new Date(e+"T00:00:00");return isNaN(t.getTime())?null:t}function $e(){const e=document.getElementById("tarefas-list");if(!e)return;let t=D;if(he==="pending"&&(t=D.filter(a=>a.status!=="done")),he==="done"&&(t=D.filter(a=>a.status==="done")),!t.length){e.innerHTML=`<div style="text-align:center;padding:40px;color:#94a3b8;">
      ...
      </div>`}).join(""),e.querySelectorAll(".tarefa-check").forEach(a=>{a.addEventListener("change",async o=>{o.stopPropagation();const i=a.dataset.id,s=a.checked?"done":"pending";await h.from("tasks").update({status:s}).eq("id",i);const l=D.find(d=>String(d.id)===i);l&&(l.status=s),$e()})}),e.querySelectorAll(".tarefa-del-btn").forEach(a=>{a.addEventListener("click",async o=>{o.stopPropagation(),confirm("Excluir esta tarefa?")&&(await h.from("tasks").delete().eq("id",a.dataset.id),D=D.filter(i=>String(i.id)!==String(a.dataset.id)),$e())})}),e.querySelectorAll(".tarefa-item").forEach(a=>{a.addEventListener("click",o=>{if(o.target.closest(".tarefa-check")||o.target.closest(".tarefa-del-btn"))return;const i=a.dataset.id,s=D.find(l=>String(l.id)===i);s&&Xt(s)})})}function Xt(e=null){var d,c,r,m;const t=document.getElementById("tarefa-modal-root");t&&t.remove();const n=!!e,a=(e==null?void 0:e.status)==="done",o=Ft(e==null?void 0:e.due_date);o&&o.toLocaleDateString("pt-BR");const i=e!=null&&e.due_date?e.due_date.includes("T")?e.due_date.split("T")[0]:e.due_date:"",s=document.createElement("div");s.id="tarefa-modal-root",s.className="modal-backdrop",s.innerHTML=`
...
Essa ação é irreversível e removerá o registro da plataforma.`))return;const x=document.getElementById("et-delete");x.disabled=!0,x.textContent="Excluindo…";const{error:I}=await h.from("tenants").delete().eq("id",e.id);if(I){alert("Erro ao excluir: "+I.message),x.disabled=!1,x.textContent="🗑️ Excluir";return}d(),de()}),(y=document.getElementById("et-save"))==null||y.addEventListener("click",async()=>{var N,T,V,R,O,ne,re,ke,Se,H,fe,yt;const g=(T=(N=document.getElementById("et-name"))==null?void 0:N.value)==null?void 0:T.trim(),x=(R=(V=document.getElementById("et-slug"))==null?void 0:V.value)==null?void 0:R.trim(),I=(ne=(O=document.getElementById("et-domain"))==null?void 0:O.value)==null?void 0:ne.trim(),S=(re=document.getElementById("et-plan"))==null?void 0:re.value,L=(Se=(ke=document.getElementById("et-admin-email"))==null?void 0:ke.value)==null?void 0:Se.trim(),k=(fe=(H=document.getElementById("et-admin-password"))==null?void 0:H.value)==null?void 0:fe.trim(),w=(yt=document.getElementById("et-logo-input"))==null?void 0:yt.files[0],$=document.getElementById("et-msg"),B=document.getElementById("et-save");if(!g){$.textContent="❌ Nome é obrigatório.",$.style.color="#ef4444";return}B.disabled=!0,B.textContent="Salvando…",$.textContent="⏳ Salvando…",$.style.color="#64748b";let q=e.logo_url;if(w)try{const A=await je(w,256,.85),bt=`tenant-logos/${e.id}-${Date.now()}.jpg`,{error:da}=await h.storage.from("imoveis").upload(bt,A,{contentType:"image/jpeg",upsert:!0});if(!da){const{data:{publicUrl:ra}}=h.storage.from("imoveis").getPublicUrl(bt);q=ra}}catch(A){console.error("Logo upload:",A)}const{error:C}=await h.from("tenants").update({name:g,slug:x||e.slug,domain:I||null,plan_id:S||null,logo_url:q}).eq("id",e.id);if(C){B.disabled=!1,B.textContent="Salvar",$.textContent="❌ "+C.message,$.style.color="#ef4444";return}if(L&&k&&k.length>=6){$.textContent="⏳ Criando usuário admin…";const A=await ue({email:L,password:k,role:"admin",tenant_id:e.id});A!=null&&A.success?(A!=null&&A.user_id&&!(A!=null&&A.linked)&&await h.from("profiles").update({tenant_id:e.id}).eq("id",A.user_id),$.textContent="✅ Salvo e admin criado!",$.style.color="#22c55e"):($.textContent="⚠️ Salvo, mas erro ao criar admin: "+((A==null?void 0:A.error)||"Tente novamente"),$.style.color="#f59e0b")}else $.textContent="✅ Imobiliária atualizada!",$.style.color="#22c55e";B.disabled=!1,B.textContent="Salvar",setTimeout(()=>{d(),de()},1200)})}const Mt=[{key:"name",label:"Nome",required:!0},{key:"phone",label:"Telefone",required:!1},{key:"email",label:"E-mail",required:!1},{key:"notes",label:"Notas",required:!1}];let Ke=[],ve=[],Pe={};function tn(){var e;(e=document.getElementById("btn-import-leads"))==null||e.addEventListener("click",an)}function an(){Ke=[],ve=[],Pe={};const e=document.createElement("div");e.id="import-leads-overlay",e.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;",e.innerHTML=`
...
</div>`;const n=new Date,a=n.getHours(),o=a<12?"Bom dia":a<18?"Boa tarde":"Boa noite",i=["Domingo","Segunda-feira","Terça-feira","Quarta-feira","Quinta-feira","Sexta-feira","Sábado"],s=["janeiro","fevereiro","março","abril","maio","junho","julho","agosto","setembro","outubro","novembro","dezembro"],l=`${i[n.getDay()]}, ${n.getDate()} de ${s[n.getMonth()]} de ${n.getFullYear()}`,d=((y=f==null?void 0:f.name)==null?void 0:y.split(" ")[0])||"Corretor",c=e.querySelector(".db-greeting");c&&(c.innerHTML=`${o}, <span class="db-greeting-name">${z(d)}</span> 👋`);const r=document.getElementById("db-subline");r&&(r.textContent=`Aqui está o resumo do seu negócio — ${l}`);let m=[],p=[];try{const[g,x]=await Promise.all([Rt(),mn()]);m=g||[],p=x||[]}catch(g){console.warn("[Dashboard] Erro ao carregar dados:",g)}pn(m,p,n);const b=m.filter(g=>g.published).length,E=p.filter(g=>lt(g.created_at)===lt(n.toISOString())).length,u=document.getElementById("db-header-chips");u&&(u.innerHTML=`
