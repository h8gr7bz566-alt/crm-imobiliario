...
</div>`;const n=new Date,a=n.getHours(),o=a<12?"Bom dia":a<18?"Boa tarde":"Boa noite",i=["Domingo","Segunda-feira","Terça-feira","Quarta-feira","Quinta-feira","Sexta-feira","Sábado"],s=["janeiro","fevereiro","março","abril","maio","junho","julho","agosto","setembro","outubro","novembro","dezembro"],l=`${i[n.getDay()]}, ${n.getDate()} de ${s[n.getMonth()]} de ${n.getFullYear()}`,d=((y=v==null?void 0:v.name)==null?void 0:y.split(" ")[0])||"Corretor",c=e.querySelector(".db-greeting");c&&(c.innerHTML=`${o}, <span class="db-greeting-name">${N(d)}</span> 👋`);const r=document.getElementById("db-subline");r&&(r.textContent=`Aqui está o resumo do seu negócio — ${l}`);let m=[],p=[];try{const[g,x]=await Promise.all([Ut(),vn()]);m=g||[],p=x||[]}catch(g){console.warn("[Dashboard] Erro ao carregar dados:",g)}yn(m,p,n);const b=m.filter(g=>g.published).length,E=p.filter(g=>mt(g.created_at)===mt(n.toISOString())).length,u=document.getElementById("db-header-chips");u&&(u.innerHTML=`
