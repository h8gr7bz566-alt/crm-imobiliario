...
</div>`;const n=new Date,a=n.getHours(),o=a<12?"Bom dia":a<18?"Boa tarde":"Boa noite",i=["Domingo","Segunda-feira","Terça-feira","Quarta-feira","Quinta-feira","Sexta-feira","Sábado"],d=["janeiro","fevereiro","março","abril","maio","junho","julho","agosto","setembro","outubro","novembro","dezembro"],r=`${i[n.getDay()]}, ${n.getDate()} de ${d[n.getMonth()]} de ${n.getFullYear()}`,s=((y=f==null?void 0:f.name)==null?void 0:y.split(" ")[0])||"Corretor",c=e.querySelector(".db-greeting");c&&(c.innerHTML=`${o}, <span class="db-greeting-name">${z(s)}</span> 👋`);const l=document.getElementById("db-subline");l&&(l.textContent=`Aqui está o resumo do seu negócio — ${r}`);let m=[],p=[];try{const g=f==null?void 0:f.tenant_id,[E,w]=await Promise.all([Nt(),nn(g)]);m=E||[],p=w||[]}catch(g){console.warn("[Dashboard] Erro ao carregar dados:",g)}on(m,p,n);const b=m.filter(g=>g.published).length,x=p.filter(g=>dt(g.created_at)===dt(n.toISOString())).length,u=document.getElementById("db-header-chips");u&&(u.innerHTML=`
