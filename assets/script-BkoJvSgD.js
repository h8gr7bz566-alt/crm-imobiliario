...
CREATE POLICY "tasks_access" ON public.tasks
  ...
  `,document.body.appendChild(d);const l=()=>d.remove();(s=document.getElementById("tm-close"))==null||s.addEventListener("click",l),(r=document.getElementById("tm-cancel"))==null||r.addEventListener("click",l),d.addEventListener("click",m=>{m.target===d&&l()}),(c=document.getElementById("tm-toggle-done"))==null||c.addEventListener("click",async()=>{const m=a?"pending":"done";await h.from("tasks").update({status:m}).eq("id",e.id);const E=M.find(f=>String(f.id)===String(e.id));E&&(E.status=m),l(),m==="done"&&(Q="done",document.querySelectorAll(".tarefa-filter-btn").forEach(f=>{f.classList.toggle("active",f.dataset.filter==="done")})),ae()}),(u=document.getElementById("tm-save"))==null||u.addEventListener("click",async()=>{var v,x;const m=document.getElementById("tarefa-form");if(!m.checkValidity()){m.reportValidity();return}const E=new FormData(m),f=document.getElementById("tm-save");f.disabled=!0,f.textContent="Salvando…";const y={title:(v=E.get("title"))==null?void 0:v.trim(),description:((x=E.get("description"))==null?void 0:x.trim())||null,due_date:E.get("due_date")||null,priority:E.get("priority")||"medium",status:(e==null?void 0:e.status)||"pending",assigned_to:(p==null?void 0:p.id)||null,tenant_id:(p==null?void 0:p.tenant_id)||null};let b;if(n){if({error:b}=await h.from("tasks").update(y).eq("id",e.id),!b){const L=M.findIndex(I=>String(I.id)===String(e.id));L>=0&&(M[L]={...M[L],...y})}}else{const{data:L,error:I}=await h.from("tasks").insert(y).select();b=I,!b&&(L!=null&&L[0])&&M.unshift(L[0])}if(f.disabled=!1,f.textContent=n?"Salvar":"Criar Tarefa",b){alert("Erro: "+b.message);return}l(),ae()})}async function Gt(e){const t=document.getElementById("search-results");if(!t)return;if(!e||e.length<2){t.innerHTML='<div class="search-hint">Digite para pesquisar…</div>';return}t.innerHTML='<div class="search-hint">Buscando…</div>';const n=`%${e}%`;p==null||p.role,p==null||p.tenant_id;const[{data:a},{data:o}]=await Promise.all([h.from("properties").select("id,title,reference,type,city").ilike("title",n).limit(5),h.from("leads").select("id,name,phone,email").or(`name.ilike.${n},phone.ilike.${n},email.ilike.${n}`).limit(5)]),i=[];a!=null&&a.length&&(i.push('<div class="search-group-label">Imóveis</div>'),i.push(...a.map(d=>`
...
`).filter(l=>l.trim()).slice(1).map(l=>{const[s,r,c,u,m]=l.split(",").map(E=>E.trim().replace(/^"|"$/g,""));return{name:s,email:r,phone:c,company:u,job_title:m}}).filter(l=>l.name);const i=document.getElementById("import-preview");i&&(i.textContent=`${ee.length} contato(s) encontrados para importar.`);const d=document.getElementById("import-modal-confirm");d&&(d.disabled=ee.length===0)},n.readAsText(t)}async function Zt(){if(!ee.length)return;const e=document.getElementById("import-modal-confirm");e&&(e.disabled=!0,e.textContent="Importando…");const t=ee.map(a=>({...a,stage:"novo",source:"importado",assigned_to:(p==null?void 0:p.id)||null,tenant_id:(p==null?void 0:p.tenant_id)||null})),{error:n}=await h.from("leads").insert(t);if(e&&(e.disabled=!1,e.textContent="Importar"),n){alert("Erro na importação: "+n.message);return}Me(),await vt(),alert(`${t.length} contato(s) importados com sucesso!`)}function ea(){const e=document.getElementById("import-modal");e&&e.classList.remove("hidden"),ee=[];const t=document.getElementById("import-preview");t&&(t.textContent="");const n=document.getElementById("import-modal-confirm");n&&(n.disabled=!0);const a=document.getElementById("import-csv-file");a&&(a.value="")}function Me(){const e=document.getElementById("import-modal");e&&e.classList.add("hidden")}const ta="https://onknpbzdcrhbfozzvxtz.supabase.co/functions/v1/invite-user";async function oe(e){return(await fetch(ta,{method:"POST",headers:{Authorization:`Bearer ${Bt}`,"Content-Type":"application/json"},body:JSON.stringify(e)})).json()}async function nt(e){var s,r,c,u;const t=document.getElementById("settings-name"),n=document.getElementById("settings-email"),a=document.getElementById("settings-avatar-preview"),o=document.getElementById("settings-avatar-initial"),i=document.getElementById("settings-avatar-input"),d=document.getElementById("settings-save-profile");if(!t)return;if(t.value=(e==null?void 0:e.name)||"",n){const{data:{user:m}}=await h.auth.getUser();n.value=(m==null?void 0:m.email)||""}const l=((e==null?void 0:e.name)||"?")[0].toUpperCase();if(o&&(o.textContent=l),e!=null&&e.avatar_url&&a&&(a.src=e.avatar_url,a.style.display="",o&&(o.style.display="none")),i==null||i.addEventListener("change",m=>{const E=m.target.files[0];if(!E)return;const f=URL.createObjectURL(E);a&&(a.src=f,a.style.display=""),o&&(o.style.display="none")}),(s=document.getElementById("btn-change-password"))==null||s.addEventListener("click",async()=>{var v,x;const m=((v=document.getElementById("change-password-new"))==null?void 0:v.value)||"",E=((x=document.getElementById("change-password-confirm"))==null?void 0:x.value)||"",f=document.getElementById("change-password-msg"),y=document.getElementById("btn-change-password");if(f&&(f.style.display="none"),m.length<6){f&&(f.textContent="Mínimo 6 caracteres.",f.style.display="");return}if(m!==E){f&&(f.textContent="As senhas não coincidem.",f.style.display="");return}y&&(y.disabled=!0,y.textContent="Salvando…");const{error:b}=await h.auth.updateUser({password:m});y&&(y.disabled=!1,y.textContent="Salvar Nova Senha"),b?f&&(f.textContent="Erro: "+b.message,f.style.display=""):(f&&(f.style.color="#16a34a",f.textContent="Senha alterada com sucesso!",f.style.display=""),document.getElementById("change-password-new").value="",document.getElementById("change-password-confirm").value="")}),d==null||d.addEventListener("click",async()=>{var x;const m=t.value.trim();let E=(p==null?void 0:p.avatar_url)||"";const f=i==null?void 0:i.files[0],y=d.textContent;if(d.disabled=!0,d.textContent="Salvando…",f)try{const L=await he(f,400,.85),I=`avatars/${p.id}-${Date.now()}.jpg`,{error:C}=await h.storage.from("imoveis").upload(I,L,{contentType:"image/jpeg",upsert:!0});if(!C){const{data:{publicUrl:w}}=h.storage.from("imoveis").getPublicUrl(I);E=w}}catch(L){console.error("Avatar upload:",L)}const{error:b}=await h.from("profiles").update({name:m,avatar_url:E}).eq("id",p.id);if(d.disabled=!1,d.textContent=y,b){alert("Erro ao salvar perfil.");return}p={...p,name:m,avatar_url:E},Le(p);const v=document.getElementById("settings-avatar-initial");v&&(v.textContent=((x=m[0])==null?void 0:x.toUpperCase())||"?")}),(e==null?void 0:e.role)==="admin"||(e==null?void 0:e.role)==="super_admin"){const m=document.getElementById("settings-corretores-section");m&&(m.style.display=""),await $e(),(r=document.getElementById("btn-invite-corretor"))==null||r.addEventListener("click",async()=>{var x,L;const f=(x=document.getElementById("invite-email"))==null?void 0:x.value.trim(),y=(L=document.getElementById("invite-password"))==null?void 0:L.value.trim(),b=document.getElementById("btn-invite-corretor"),v=document.getElementById("invite-note");if(!f){alert("Informe o e-mail do corretor.");return}if(!y||y.length<6){alert("A senha precisa ter no mínimo 6 caracteres.");return}b&&(b.disabled=!0,b.textContent="Criando…"),v&&(v.style.display="none");try{const I=await oe({email:f,password:y,tenant_id:(p==null?void 0:p.tenant_id)||null});if(I.success){const C=document.getElementById("invite-email"),w=document.getElementById("invite-password");C&&(C.value=""),w&&(w.value=""),await $e(),v&&(I.email_sent===!1?(v.innerHTML=`
                ...
                <strong>Senha:</strong> ${g(y)}`,v.style.color="#0f172a"):(v.textContent="✅ Acesso criado! O corretor receberá um e-mail com o login e a senha.",v.style.color="#16a34a"),v.style.display="")}else alert("Erro: "+(I.error||"Falha desconhecida"))}catch(I){alert("Erro ao criar acesso: "+I.message)}finally{b&&(b.disabled=!1,b.textContent="+ Criar Acesso")}});const E=document.getElementById("settings-locations-section");E&&(E.style.display=""),await me(),(c=document.getElementById("loc-add-city-btn"))==null||c.addEventListener("click",async()=>{const f=document.getElementById("loc-new-city"),y=f==null?void 0:f.value.trim();if(!y)return;const{error:b}=await h.from("locations").insert({type:"cidade",name:y});if(b){alert("Erro ao adicionar cidade.");return}f&&(f.value=""),await me(),Ue()}),(u=document.getElementById("loc-add-neighborhood-btn"))==null||u.addEventListener("click",async()=>{var x;const f=parseInt((x=document.getElementById("loc-new-neighborhood-city"))==null?void 0:x.value,10),y=document.getElementById("loc-new-neighborhood"),b=y==null?void 0:y.value.trim();if(!f||!b){alert("Selecione a cidade e informe o nome do bairro.");return}const{error:v}=await h.from("locations").insert({type:"bairro",name:b,parent_id:f});if(v){alert("Erro ao adicionar bairro.");return}y&&(y.value=""),await me()})}}async function $e(){const e=document.getElementById("corretores-list");if(!e)return;let t=h.from("profiles").select("*").order("created_at");p!=null&&p.tenant_id&&(t=t.eq("tenant_id",p.tenant_id));const{data:n,error:a}=await t;if(a||!n){e.innerHTML='<p style="color:#6b7280;font-size:14px">Erro ao carregar.</p>';return}e.innerHTML=n.map(o=>{const i=(o.name||"?")[0].toUpperCase(),d=o.avatar_url?`<img src="${o.avatar_url}" class="corretor-avatar" alt="">`:`<div class="corretor-avatar-initial">${g(i)}</div>`,l=o.id===(p==null?void 0:p.id),s=o.active!==!1,r=s?'<span class="badge badge-green">Ativo</span>':'<span class="badge badge-gray">Pausado</span>',c=l?'<span class="corretor-you">Você</span>':`<select class="form-control corretor-role-sel" data-uid="${o.id}" style="width:110px;padding:6px 8px;font-size:13px">
...
`+w);window.open("mailto:?subject="+k+"&body="+$,"_blank")}),(L=document.getElementById("share-copy"))==null||L.addEventListener("click",()=>{var B;const w=document.getElementById("share-link-input");w&&((B=navigator.clipboard)==null||B.writeText(w.value).then(()=>{const k=document.getElementById("share-copy"),$=k.textContent;k.textContent="✅ Copiado!",setTimeout(()=>{k.textContent=$},2e3)}))}),(I=document.getElementById("view-modal-edit"))==null||I.addEventListener("click",()=>{var P;if((p==null?void 0:p.role)!=="admin")return;const w=document.getElementById("view-modal-title").textContent,B=S.find(z=>z.title===w);if(!B)return;Ee(),N=B.id;const k=document.getElementById("property-form"),$=document.getElementById("form-submit-btn");$.textContent="Salvar Alterações",k.querySelector('[name="title"]').value=B.title||"",k.querySelector('[name="rua"]').value=B.rua||"",k.querySelector('[name="numero"]').value=B.numero||"",k.querySelector('[name="city"]').value=B.city||"",k.querySelector('[name="price"]').value=B.price||"",k.querySelector('[name="bedrooms"]').value=B.bedrooms||"",k.querySelector('[name="suites"]').value=B.suites||"",k.querySelector('[name="parking"]').value=B.parking||"",k.querySelector('[name="description"]').value=B.description||"",k.querySelector('[name="construction_status"]').value=B.construction_status||"",k.querySelector('[name="owner_name"]').value=B.owner_name||"",k.querySelector('[name="owner_phone"]').value=B.owner_phone||"",k.querySelector('[name="owner_email"]').value=B.owner_email||"",k.querySelector('[name="owner_notes"]').value=B.owner_notes||"",k.querySelector('[name="condominium"]').value=B.condominium||"";const _=document.getElementById("adminPublished");_&&(_.value=B.published===!0?"true":"false");const T=document.getElementById("adminCitySelect");T&&(T.value=B.city||"",T.dispatchEvent(new Event("change")),setTimeout(()=>{const z=document.getElementById("adminNeighborhood");z&&(z.value=B.neighborhood||"")},50)),V=B.cover_image||((P=B.images)==null?void 0:P[0])||"",Ie(B.images||[]),qe("Editar Imóvel")}),document.querySelectorAll(".tab-btn[data-tab]").forEach(w=>{w.addEventListener("click",()=>{var B;document.querySelectorAll(".tab-btn").forEach(k=>k.classList.remove("active")),w.classList.add("active"),document.querySelectorAll(".tab-panel").forEach(k=>k.classList.add("hidden")),(B=document.getElementById(`tab-${w.dataset.tab}`))==null||B.classList.remove("hidden")})}),(C=document.getElementById("admin-properties"))==null||C.addEventListener("click",w=>{if(w.target.closest(".action-btns"))return;const B=w.target.closest("tr");if(!B)return;const k=Number(B.dataset.id);if(!k)return;const $=S.find(_=>_.id===k);$&&Ht($)})}document.addEventListener("DOMContentLoaded",async()=>{var i,d,l;await Promise.all([xt(),yt()]),K=G("company.whatsapp",K),pe=`https://wa.me/${K}`,je(),Mt(),zt();const e=document.getElementById("adminCitySelect"),t=document.getElementById("adminNeighborhood");e&&t&&(Ue(),e.addEventListener("change",()=>{const s=Y().find(c=>c.name===e.value),r=s?Re(s.id):[];t.innerHTML='<option value="">Selecione a cidade primeiro</option>'+r.map(c=>`<option value="${c.name}">${g(c.name)}</option>`).join("")}));const n=document.getElementById("admin-login"),a=document.getElementById("admin-root");if(n){const s=new URLSearchParams(window.location.hash.replace("#","")),r=new URLSearchParams(window.location.search),c=s.get("type")||r.get("type")||"",u=ct||c==="recovery"||c==="invite"||window.location.hash.includes("access_token")||r.has("code"),m=document.getElementById("password-reset-overlay");if(u){n.style.display="none",a&&a.classList.add("hidden"),m&&(m.style.display="flex"),(i=document.getElementById("password-reset-form"))==null||i.addEventListener("submit",async f=>{var I,C;f.preventDefault();const y=((I=document.getElementById("new-password"))==null?void 0:I.value)||"",b=((C=document.getElementById("confirm-password"))==null?void 0:C.value)||"",v=document.getElementById("password-reset-msg"),x=f.target.querySelector('button[type="submit"]');if(v&&(v.style.display="none"),y!==b){v&&(v.textContent="As senhas não coincidem.",v.style.display="");return}x&&(x.disabled=!0,x.textContent="Salvando…");const{error:L}=await h.auth.updateUser({password:y});if(L){v&&(v.textContent="Erro: "+L.message,v.style.display=""),x&&(x.disabled=!1,x.textContent="Definir Senha");return}window.location.href=window.location.pathname}),r.has("code")&&await h.auth.exchangeCodeForSession(r.get("code")??"");return}const{data:{session:E}}=await h.auth.getSession();if(E){if(n.classList.add("hidden"),a&&a.classList.remove("hidden"),Ae(),ot(),Qe(),window.lucide&&lucide.createIcons(),p=await Ye(E.user.id),!p){await h.auth.signOut(),n.classList.remove("hidden"),a&&a.classList.add("hidden");return}if(p.active===!1){await h.auth.signOut(),n.classList.remove("hidden"),a&&a.classList.add("hidden"),alert("Seu acesso está pausado. Entre em contato com o administrador.");return}if(p.needs_password_reset){n.style.display="none",a&&a.classList.add("hidden");const f=document.getElementById("password-reset-overlay");f&&(f.style.display="flex"),(d=document.getElementById("password-reset-form"))==null||d.addEventListener("submit",async y=>{var C,w;y.preventDefault();const b=((C=document.getElementById("new-password"))==null?void 0:C.value)||"",v=((w=document.getElementById("confirm-password"))==null?void 0:w.value)||"",x=document.getElementById("password-reset-msg"),L=y.target.querySelector('button[type="submit"]');if(x&&(x.style.display="none"),b!==v){x&&(x.textContent="As senhas não coincidem.",x.style.display="");return}if(b.length<6){x&&(x.textContent="Mínimo 6 caracteres.",x.style.display="");return}L&&(L.disabled=!0,L.textContent="Salvando…");const{error:I}=await h.auth.updateUser({password:b});if(I){x&&(x.textContent="Erro: "+I.message,x.style.display=""),L&&(L.disabled=!1,L.textContent="Definir Senha");return}await h.from("profiles").update({needs_password_reset:!1}).eq("id",p.id),window.location.href=window.location.pathname});return}We((p==null?void 0:p.tenant_id)||null),Le(p),Ke(p.role),await we(),await nt(p),window.lucide&&lucide.createIcons(),Yt()}else{a&&a.classList.add("hidden"),n.classList.remove("hidden");const f=document.getElementById("login-form");f&&((l=document.getElementById("forgot-password-btn"))==null||l.addEventListener("click",async()=>{var v,x;const y=(x=(v=f.querySelector('input[name="email"]'))==null?void 0:v.value)==null?void 0:x.trim();if(!y){alert("Digite seu e-mail no campo acima primeiro.");return}const{error:b}=await h.auth.resetPasswordForEmail(y,{redirectTo:"https://omarcorretor.com.br/admin.html"});alert(b?"Erro: "+b.message:"E-mail de redefinição enviado! Verifique sua caixa de entrada.")}),f.addEventListener("submit",async y=>{y.preventDefault();const b=new FormData(f),v=b.get("email"),x=b.get("password");if(await Ct(v,x)){n.classList.add("hidden"),a&&a.classList.remove("hidden"),Ae(),ot(),window.lucide&&lucide.createIcons();const{data:{session:I}}=await h.auth.getSession();if(p=I?await Ye(I.user.id):null,!p){await h.auth.signOut();return}if(p.active===!1){await h.auth.signOut(),n.classList.remove("hidden"),a&&a.classList.add("hidden"),alert("Seu acesso está pausado. Entre em contato com o administrador.");return}Qe(),We((p==null?void 0:p.tenant_id)||null),Le(p),Ke(p.role),await we(),await nt(p),window.lucide&&lucide.createIcons()}else alert("E-mail ou senha incorretos")}))}}else Ae();await fe();const o=(()=>{try{return localStorage.getItem("lang")||"pt"}catch{return"pt"}})();wt(o),It(K)});async function aa(){const e=S.filter(o=>!o.reference);if(!e.length)return;const t=S.map(o=>o.reference||"").filter(o=>/^IO-\d+$/.test(o)).map(o=>parseInt(o.replace("IO-",""),10));let n=t.length?Math.max(...t)+1:1;const a=[...e].sort((o,i)=>o.id-i.id);for(const o of a){const i="IO-"+String(n).padStart(4,"0"),{error:d}=await h.from("properties").update({reference:i}).eq("id",o.id);if(!d){const l=S.findIndex(s=>s.id===o.id);l>=0&&(S[l].reference=i),n++}}ye(He(S))}async function na(){const e=S.filter(t=>{var n;return(n=t.images)==null?void 0:n.some(a=>!a.includes("/wm-"))});if(e.length){for(const t of e){if(!t.images.some(i=>!i.includes("/wm-")))continue;const a=[];let o=!1;for(const i of t.images)if(i.includes("/wm-"))a.push(i);else try{const d=await oa(i);a.push(d),o=!0}catch{a.push(i)}if(o){await h.from("properties").update({images:a}).eq("id",t.id);const i=S.findIndex(d=>d.id===t.id);i>=0&&(S[i].images=a)}}ye(He(S))}}async function oa(e){try{const t=await fetch(e);if(!t.ok)return e;const n=await t.blob(),a=URL.createObjectURL(n),o=await fetch("/logo.png"),i=o.ok?await o.blob():null,d=i?URL.createObjectURL(i):null;return new Promise(l=>{const s=new Image;s.onload=()=>{URL.revokeObjectURL(a);const r=document.createElement("canvas"),c=1200;let u=s.width,m=s.height;u>c&&(m=Math.round(m*c/u),u=c),r.width=u,r.height=m;const E=r.getContext("2d");E.drawImage(s,0,0,u,m);const f=y=>{if(y){const b=Math.round(u*.18),v=Math.round(y.naturalHeight*b/y.naturalWidth),x=Math.round(u*.02);E.globalAlpha=.45,E.drawImage(y,u-b-x,m-v-x,b,v),E.globalAlpha=1}r.toBlob(async b=>{try{const v=`wm-${Date.now()}-${Math.random().toString(36).slice(2)}.jpg`,{error:x}=await h.storage.from("imoveis").upload(v,b,{contentType:"image/jpeg",cacheControl:"31536000",upsert:!1});if(x){console.error("Upload watermark error:",x),l(e);return}const{data:{publicUrl:L}}=h.storage.from("imoveis").getPublicUrl(v);l(L)}catch(v){console.error("Watermark upload exception:",v),l(e)}},"image/jpeg",.82)};if(d){const y=new Image;y.onload=()=>{URL.revokeObjectURL(d),f(y)},y.onerror=()=>{URL.revokeObjectURL(d),f(null)},y.src=d}else f(null)},s.onerror=()=>{URL.revokeObjectURL(a),l(e)},s.src=a})}catch(t){return console.error("applyWatermarkToUrl error:",t),e}}function H(e,t){e&&(e.textContent=t?"✓ Salvo com sucesso!":"✗ Erro ao salvar.",e.className="cfg-save-msg "+(t?"ok":"err"),e.style.display="",setTimeout(()=>{e.style.display="none"},3e3))}async function Pe(e,t="assets"){const n=await he(e,1200,.85),a=`${t}/${Date.now()}-${Math.random().toString(36).slice(2)}.jpg`,{error:o}=await h.storage.from("imoveis").upload(a,n,{contentType:"image/jpeg",cacheControl:"31536000",upsert:!1});if(o)throw o;const{data:{publicUrl:i}}=h.storage.from("imoveis").getPublicUrl(a);return i}async function ia(){const e=document.getElementById("section-empresa");if(!e||e.dataset.loaded)return;e.dataset.loaded="1";const{data:t}=await h.from("settings").select("key,value").eq("tenant_id",Ne()),n={};t==null||t.forEach(o=>{n[o.key]=o.value||""});const a=o=>g(String(n[o]||""));e.innerHTML=`
  ...
  `}).join(""),e.querySelectorAll('[data-action="toggle-tenant"]').forEach(r=>{r.addEventListener("click",async c=>{c.stopPropagation();const u=r.dataset.active==="true";await h.from("tenants").update({active:!u}).eq("id",r.dataset.id),X()})}),e.querySelectorAll('[data-action="open-panel"]').forEach(r=>{r.addEventListener("click",()=>{const c=(i||[]).find(u=>String(u.id)===String(r.dataset.id));c&&fa(c)})})}async function ua(){const e=document.getElementById("sa-plans-list");if(!e)return;e.dataset.loaded="1";const{data:t,error:n}=await h.from("plans").select("*").order("price_brl");if(n){e.innerHTML=`<div class="sa-error">Erro: ${n.message}</div>`;return}e.innerHTML=(t||[]).map(a=>`
  ...
  `,document.body.appendChild(t),document.getElementById("tp-back").addEventListener("click",()=>t.remove()),document.getElementById("tp-edit-btn").addEventListener("click",()=>bt(e)),t.querySelectorAll(".tp-tab").forEach(o=>{o.addEventListener("click",()=>{t.querySelectorAll(".tp-tab").forEach(i=>{i.style.fontWeight="500",i.style.color="#64748b",i.style.borderBottomColor="transparent"}),o.style.fontWeight="700",o.style.color="#2563eb",o.style.borderBottomColor="#2563eb",lt(e,o.dataset.tab)})}),lt(e,"properties")}async function lt(e,t){var a,o,i;const n=document.getElementById("tp-content");if(n){if(n.innerHTML='<div style="text-align:center;padding:64px;color:#94a3b8;font-size:14px;">Carregando…</div>',t==="properties"){const{data:d}=await h.from("properties").select("*").eq("tenant_id",e.id).order("created_at",{ascending:!1});if(!(d!=null&&d.length)){n.innerHTML='<div style="text-align:center;padding:64px;color:#94a3b8;"><div style="font-size:48px;margin-bottom:12px;">🏠</div><p style="font-size:14px;">Nenhum imóvel cadastrado ainda.</p></div>';return}n.innerHTML=`
      ...
      </div>`}if(t==="leads"){const{data:d}=await h.from("leads").select("*").eq("tenant_id",e.id).order("created_at",{ascending:!1}).limit(100);if(!(d!=null&&d.length)){n.innerHTML='<div style="text-align:center;padding:64px;color:#94a3b8;"><div style="font-size:48px;margin-bottom:12px;">📋</div><p style="font-size:14px;">Nenhum lead ainda.</p></div>';return}const l=r=>({novo:"Novo",contato:"Contato",proposta:"Proposta",fechado:"Fechado"})[r]||r||"Novo",s=r=>({novo:"#dbeafe,#1d4ed8",contato:"#fef3c7,#92400e",proposta:"#ede9fe,#6d28d9",fechado:"#dcfce7,#15803d"})[r]||"#f1f5f9,#64748b";n.innerHTML=`
      <div style="background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 1px 6px rgba(0,0,0,.07);">
    ...
    <div class="sa-modal" style="max-width:560px;">
      ...
      <div id="et-pane-api" class="sa-modal-body" style="display:none;">
        ...
        <div style="display:flex;flex-direction:column;gap:8px;">
          ${[["GET","properties","Lista imóveis publicados"],["GET","properties/ID","Detalhe de um imóvel"],["POST","leads","Registra lead / formulário de contato"],["GET","settings","Dados da empresa (nome, WhatsApp, logo…)"]].map(([y,b,v])=>`
...
await fetch(\`\${API}/leads?key=\${KEY}\`, {
  ...
  `,document.body.appendChild(n),h.from("plans").select("id, name").then(({data:y})=>{const b=document.getElementById("et-plan");b&&y&&(b.innerHTML='<option value="">Sem plano</option>'+y.map(v=>`<option value="${v.id}"${String(v.id)===String(e.plan_id)?" selected":""}>${g(v.name)}</option>`).join(""))}),(i=document.getElementById("et-logo-input"))==null||i.addEventListener("change",y=>{const b=y.target.files[0];if(!b)return;const v=URL.createObjectURL(b),x=document.getElementById("et-logo-preview");x&&(x.innerHTML=`<img src="${v}" style="width:100%;height:100%;object-fit:cover;">`)}),(d=document.getElementById("et-logo-preview"))==null||d.addEventListener("click",()=>{var y;(y=document.getElementById("et-logo-input"))==null||y.click()}),(l=document.getElementById("et-pwd-toggle"))==null||l.addEventListener("click",()=>{const y=document.getElementById("et-admin-password");y.type=y.type==="password"?"text":"password"}),(s=document.getElementById("et-copy-key"))==null||s.addEventListener("click",()=>{var x,L;const y=(x=document.getElementById("et-api-key"))==null?void 0:x.value;if(!y)return;(L=navigator.clipboard)==null||L.writeText(y);const b=document.getElementById("et-copy-key"),v=b.textContent;b.textContent="✅ Copiada!",setTimeout(()=>{b.textContent=v},2e3)}),(r=document.getElementById("et-tab-api"))==null||r.addEventListener("click",()=>{document.getElementById("et-pane-dados").style.display="none",document.getElementById("et-pane-api").style.display="",document.getElementById("et-tab-dados").style.borderBottomColor="transparent",document.getElementById("et-tab-dados").style.color="#64748b",document.getElementById("et-tab-api").style.borderBottomColor="#2563eb",document.getElementById("et-tab-api").style.color="#2563eb"}),(c=document.getElementById("et-tab-dados"))==null||c.addEventListener("click",()=>{document.getElementById("et-pane-api").style.display="none",document.getElementById("et-pane-dados").style.display="",document.getElementById("et-tab-api").style.borderBottomColor="transparent",document.getElementById("et-tab-api").style.color="#64748b",document.getElementById("et-tab-dados").style.borderBottomColor="#2563eb",document.getElementById("et-tab-dados").style.color="#2563eb"});const o=()=>n.remove();(u=document.getElementById("et-close"))==null||u.addEventListener("click",o),(m=document.getElementById("et-cancel"))==null||m.addEventListener("click",o),n.addEventListener("click",y=>{y.target===n&&o()}),(E=document.getElementById("et-delete"))==null||E.addEventListener("click",async()=>{if(!confirm(`⚠️ Tem certeza que deseja EXCLUIR a imobiliária "${e.name}"?

Essa ação é irreversível e removerá o registro da plataforma.`))return;const b=document.getElementById("et-delete");b.disabled=!0,b.textContent="Excluindo…";const{error:v}=await h.from("tenants").delete().eq("id",e.id);if(v){alert("Erro ao excluir: "+v.message),b.disabled=!1,b.textContent="🗑️ Excluir";return}o(),X()}),(f=document.getElementById("et-save"))==null||f.addEventListener("click",async()=>{var _,T,P,z,ie,se,de,De,Oe,Fe,Xe,Ge;const y=(T=(_=document.getElementById("et-name"))==null?void 0:_.value)==null?void 0:T.trim(),b=(z=(P=document.getElementById("et-slug"))==null?void 0:P.value)==null?void 0:z.trim(),v=(se=(ie=document.getElementById("et-domain"))==null?void 0:ie.value)==null?void 0:se.trim(),x=(de=document.getElementById("et-plan"))==null?void 0:de.value,L=(Oe=(De=document.getElementById("et-admin-email"))==null?void 0:De.value)==null?void 0:Oe.trim(),I=(Xe=(Fe=document.getElementById("et-admin-password"))==null?void 0:Fe.value)==null?void 0:Xe.trim(),C=(Ge=document.getElementById("et-logo-input"))==null?void 0:Ge.files[0],w=document.getElementById("et-msg"),B=document.getElementById("et-save");if(!y){w.textContent="❌ Nome é obrigatório.",w.style.color="#ef4444";return}B.disabled=!0,B.textContent="Salvando…",w.textContent="⏳ Salvando…",w.style.color="#64748b";let k=e.logo_url;if(C)try{const A=await he(C,256,.85),Ve=`tenant-logos/${e.id}-${Date.now()}.jpg`,{error:ht}=await h.storage.from("imoveis").upload(Ve,A,{contentType:"image/jpeg",upsert:!0});if(!ht){const{data:{publicUrl:Et}}=h.storage.from("imoveis").getPublicUrl(Ve);k=Et}}catch(A){console.error("Logo upload:",A)}const{error:$}=await h.from("tenants").update({name:y,slug:b||e.slug,domain:v||null,plan_id:x||null,logo_url:k}).eq("id",e.id);if($){B.disabled=!1,B.textContent="Salvar",w.textContent="❌ "+$.message,w.style.color="#ef4444";return}if(L&&I&&I.length>=6){w.textContent="⏳ Criando usuário admin…";const A=await oe({email:L,password:I,role:"admin",tenant_id:e.id});A!=null&&A.success?(A!=null&&A.user_id&&!(A!=null&&A.linked)&&await h.from("profiles").update({tenant_id:e.id}).eq("id",A.user_id),w.textContent="✅ Salvo e admin criado!",w.style.color="#22c55e"):(w.textContent="⚠️ Salvo, mas erro ao criar admin: "+((A==null?void 0:A.error)||"Tente novamente"),w.style.color="#f59e0b")}else w.textContent="✅ Imobiliária atualizada!",w.style.color="#22c55e";B.disabled=!1,B.textContent="Salvar",setTimeout(()=>{o(),X()},1200)})}
