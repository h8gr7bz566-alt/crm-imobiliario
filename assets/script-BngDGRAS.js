...
${d}`),s=e.area?`<span class="icard-spec"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18M9 3v18"/></svg>${e.area}m²</span>`:"",c=e.bedrooms?`<span class="icard-spec"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M2 20v-6a2 2 0 012-2h16a2 2 0 012 2v6"/><path d="M2 14V8a2 2 0 012-2h4l2 3h8a2 2 0 012 2v3"/></svg>${e.bedrooms} quarto${e.bedrooms!=1?"s":""}</span>`:"",l=e.bathrooms?`<span class="icard-spec"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 6 6.5 3.5a1.5 1.5 0 000-2.12L6 1.5a1.5 1.5 0 00-2.12 0L2 3.38a1.5 1.5 0 000 2.12L5.5 9"/><path d="M2 20h20M20 12H4a2 2 0 00-2 2v4a2 2 0 002 2h16a2 2 0 002-2v-4a2 2 0 00-2-2z"/></svg>${e.bathrooms} banheiro${e.bathrooms!=1?"s":""}</span>`:"",m=e.parking?`<span class="icard-spec"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="1" y="3" width="15" height="13" rx="2"/><path d="M16 8h4l3 5v3h-7V8z"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg>${e.parking} vaga${e.parking!=1?"s":""}</span>`:"",p=Math.max(1,Math.min(n,6)),b=`<div class="icard-dots">${Array.from({length:p},(u,f)=>`<span class="icard-dot${f===0?" active":""}"></span>`).join("")}</div>`;return`
    ...
    <div style="flex:1;overflow-y:auto;padding:20px 24px;display:flex;flex-direction:column;gap:14px;">
      ...
      <div id="ldp-tags-wrap" class="ldp-tags-section">
        ...
        <div class="ldp-tag-badge-area" id="ldp-tag-badge-area">
          ${((e==null?void 0:e.tags)||[]).map(g=>{const w=(ce[g]||{}).color||"#6366F1";return`<span class="ldp-tag-badge" data-tag="${v(g)}" style="background:${w}18;color:${w};border-color:${w}55;">${v(g)}<span class="ldp-tag-rm" data-tag="${v(g)}">×</span></span>`}).join("")||'<span class="ldp-tag-empty">Nenhuma tag — clique em + para adicionar</span>'}
        </div>
  ...
  `,document.body.appendChild(l),requestAnimationFrame(()=>{l.style.transform="translateX(0)"}),tn(l,a||[],ce);const m=l.querySelector("#ldp-pipe"),p=l.querySelector("#ldp-stage");m&&p&&m.addEventListener("change",()=>{const g=m.value,E=Xe.filter(w=>w.pipeline_id===g);p.innerHTML=E.map(w=>`<option value="${w.name}">${w.name}</option>`).join("")||'<option value="">— sem etapas —</option>'});const b=()=>{l.style.transform="translateX(100%)",setTimeout(()=>l.remove(),250)};document.getElementById("ldp-close").addEventListener("click",b),document.getElementById("ldp-save").addEventListener("click",async()=>{var B,q;const g=document.getElementById("ldp-save"),E=document.getElementById("ldp-msg"),w=document.getElementById("ldp-name").value.trim();if(!w){E.style.color="#ef4444",E.textContent="Nome é obrigatório.";return}g.disabled=!0,g.textContent="Salvando…";const S=[...l.querySelectorAll("#ldp-tag-badge-area .ldp-tag-badge[data-tag]")].map(C=>C.dataset.tag),L=document.getElementById("ldp-pipe"),k=L?L.value:i||Q,I={name:w,phone:document.getElementById("ldp-phone").value.trim()||null,email:document.getElementById("ldp-email").value.trim()||null,source:document.getElementById("ldp-source").value.trim()||null,pipeline_id:k||null,stage:((B=document.getElementById("ldp-stage"))==null?void 0:B.value)||null,status:((q=document.getElementById("ldp-status"))==null?void 0:q.value)||null,notes:document.getElementById("ldp-notes").value.trim()||null,tags:S,tenant_id:M()};let $;if(t?{error:$}=await h.from("leads").insert(I):{error:$}=await h.from("leads").update(I).eq("id",e.id),g.disabled=!1,g.textContent="💾 Salvar",$){E.style.color="#ef4444",E.textContent="Erro: "+$.message;return}E.style.color="#22c55e",E.textContent="✅ Salvo!",setTimeout(()=>{b(),Ge()},700)}),(f=document.getElementById("ldp-delete"))==null||f.addEventListener("click",async()=>{confirm(`Excluir o lead "${e==null?void 0:e.name}"?`)&&(await h.from("leads").delete().eq("id",e.id),b(),Ge())})}let D=[],$t=!1,be="pending";async function Sa(){var e;$t||($t=!0,await La(),(e=document.getElementById("btn-nova-tarefa"))==null||e.addEventListener("click",()=>Ht()),document.querySelectorAll(".tarefa-filter-btn").forEach(t=>{t.addEventListener("click",()=>{document.querySelectorAll(".tarefa-filter-btn").forEach(n=>n.classList.remove("active")),t.classList.add("active"),be=t.dataset.filter,Ie()})}))}async function La(){const e=document.getElementById("tarefas-list");e&&(e.innerHTML='<div class="empty-row" style="padding:40px;text-align:center;color:#94a3b8;">Carregando…</div>');let t=h.from("tasks").select("*").order("due_date",{ascending:!0,nullsFirst:!1});(y==null?void 0:y.role)==="corretor"?t=t.eq("assigned_to",y.id):y!=null&&y.tenant_id&&(t=t.eq("tenant_id",y.tenant_id));const{data:n,error:a}=await t;if(a){const o=document.getElementById("tarefas-list");o&&(o.innerHTML=`
            ...
            <div class="form-row">
              <div class="form-group">
                ...
                <select id="cm-pipe" name="pipeline_id" class="form-control">
                  ...
                  ${r.map(I=>`<option value="${I.id}" ${String(e==null?void 0:e.pipeline_id)===String(I.id)?"selected":""}>${v(I.name)}</option>`).join("")}
                </select>
              </div>
              <div class="form-group">
          ...
          <div class="form-row single">
            <div class="form-group">
              ...
              <select name="status" class="form-control">
                <option value="">— Sem status —</option>
                ${c.map(I=>`<option value="${v(I.name)}" ${(e==null?void 0:e.status)===I.name?"selected":""}>${v(I.name)}</option>`).join("")}
              </select>
            </div>
          </div>`:""}
...
`+k);window.open("mailto:?subject="+$+"&body="+B,"_blank")}),(w=document.getElementById("share-copy"))==null||w.addEventListener("click",()=>{var I;const k=document.getElementById("share-link-input");k&&((I=navigator.clipboard)==null||I.writeText(k.value).then(()=>{const $=document.getElementById("share-copy"),B=$.textContent;$.textContent="✅ Copiado!",setTimeout(()=>{$.textContent=B},2e3)}))}),(S=document.getElementById("view-modal-edit"))==null||S.addEventListener("click",()=>{var N;if((y==null?void 0:y.role)!=="admin"&&(y==null?void 0:y.role)!=="super_admin")return;const k=Number(document.getElementById("view-modal-edit").dataset.pid),I=_.find(T=>T.id===k);if(!I)return;ze(),R=I.id;const $=document.getElementById("property-form"),B=document.getElementById("form-submit-btn");B.textContent="Salvar Alterações",$.querySelector('[name="title"]').value=I.title||"",$.querySelector('[name="rua"]').value=I.rua||"",$.querySelector('[name="numero"]').value=I.numero||"",$.querySelector('[name="city"]').value=I.city||"",$.querySelector('[name="price"]').value=I.price||"",$.querySelector('[name="bedrooms"]').value=I.bedrooms||"",$.querySelector('[name="suites"]').value=I.suites||"",$.querySelector('[name="parking"]').value=I.parking||"",$.querySelector('[name="description"]').value=I.description||"",$.querySelector('[name="construction_status"]').value=I.construction_status||"",$.querySelector('[name="owner_name"]').value=I.owner_name||"",$.querySelector('[name="owner_phone"]').value=I.owner_phone||"",$.querySelector('[name="owner_email"]').value=I.owner_email||"",$.querySelector('[name="owner_notes"]').value=I.owner_notes||"",$.querySelector('[name="condominium"]').value=I.condominium||"";const q=document.getElementById("adminPublished");q&&(q.value=I.published===!0?"true":"false");const C=document.getElementById("adminCitySelect");C&&(C.value=I.city||"",C.dispatchEvent(new Event("change")),setTimeout(()=>{const T=document.getElementById("adminNeighborhood");T&&(T.value=I.neighborhood||"")},50)),le=I.cover_image||((N=I.images)==null?void 0:N[0])||"",Ue(I.images||[]),Ze("Editar Imóvel")}),document.querySelectorAll(".tab-btn[data-tab]").forEach(k=>{k.addEventListener("click",()=>{var I;document.querySelectorAll(".tab-btn").forEach($=>$.classList.remove("active")),k.classList.add("active"),document.querySelectorAll(".tab-panel").forEach($=>$.classList.add("hidden")),(I=document.getElementById(`tab-${k.dataset.tab}`))==null||I.classList.remove("hidden")})}),(L=document.getElementById("admin-properties"))==null||L.addEventListener("click",k=>{if(k.target.closest(".action-btns"))return;const I=k.target.closest("tr");if(!I)return;const $=Number(I.dataset.id);if(!$)return;const B=_.find(q=>q.id===$);B&&Ea(B)})}async function Xt(){const e=document.getElementById("section-depoimentos");if(!e||e.dataset.loaded)return;e.dataset.loaded="1";const{data:t}=await h.from("site_content").select("value_pt").eq("key","testimonials").eq("tenant_id",M()).maybeSingle();let n=[];try{n=JSON.parse((t==null?void 0:t.value_pt)||"[]")}catch{n=[]}function a(d){const r=["#0d2144","#1a3a5c","#0a1628","#164a3c","#2d1b3d","#3d1a1a","#1a2f4a"];let s=0;for(const c of d||"?")s=s*31+c.charCodeAt(0)&4294967295;return r[Math.abs(s)%r.length]}function o(){e.querySelector("#dep-save-msg"),e.innerHTML=`
  ...
  `,document.getElementById("crm-add-stage").addEventListener("click",async()=>{const u=document.getElementById("crm-new-stage").value.trim(),f=document.getElementById("crm-new-stage-color").value,g=parseInt(document.getElementById("crm-pipe-sel").value,10);u&&(await h.from("crm_stages").insert({pipeline_id:g,name:u,color:f,sort_order:99,tenant_id:M()}),document.getElementById("crm-new-stage").value="",await F())}),e.querySelectorAll(".stage-del").forEach(u=>{u.addEventListener("click",async()=>{confirm("Remover esta etapa?")&&(await h.from("crm_stages").delete().eq("id",u.dataset.id),await F())})}),e.querySelectorAll(".stage-color-pick").forEach(u=>{u.addEventListener("change",async f=>{await h.from("crm_stages").update({color:f.target.value}).eq("id",u.dataset.sid);const g=u.closest(".stage-item").querySelector(".stage-color-dot");g&&(g.style.background=f.target.value)})});const p=async()=>{const u=document.getElementById("crm-new-tag"),f=document.getElementById("crm-new-tag-color"),g=u==null?void 0:u.value.trim(),E=(f==null?void 0:f.value)||"#6366F1";if(!g){u==null||u.focus();return}await h.from("crm_tags").insert({name:g,color:E,tenant_id:M()}),u&&(u.value=""),await F()};(b=document.getElementById("crm-add-tag"))==null||b.addEventListener("click",p),(x=document.getElementById("crm-new-tag"))==null||x.addEventListener("keydown",u=>{u.key==="Enter"&&p()}),e.querySelectorAll(".tm-del-btn").forEach(u=>{u.addEventListener("click",async()=>{confirm("Excluir esta tag? Os leads que a possuem não serão afetados.")&&(await h.from("crm_tags").delete().eq("id",u.dataset.id),await F())})}),e.querySelectorAll(".tm-name-input").forEach(u=>{const f=u.closest(".tm-row"),g=f==null?void 0:f.querySelector(".tm-save-btn");g&&(g.style.display="none"),u.addEventListener("input",()=>{const E=u.value.trim()!==u.dataset.orig;g&&(g.style.display=E?"":"none")})}),e.querySelectorAll(".tm-color-input").forEach(u=>{const f=u.closest(".tm-row"),g=f==null?void 0:f.querySelector(".tm-color-swatch"),E=f==null?void 0:f.querySelector(".tm-save-btn");u.addEventListener("input",w=>{g&&(g.style.background=w.target.value),E&&(E.style.display="")})}),e.querySelectorAll(".tm-save-btn").forEach(u=>{u.style.display="none",u.addEventListener("click",async()=>{var w,S;const f=u.closest(".tm-row"),g=(w=f.querySelector(".tm-name-input"))==null?void 0:w.value.trim(),E=(S=f.querySelector(".tm-color-input"))==null?void 0:S.value;g&&(u.disabled=!0,u.textContent="✓ Salvando…",await h.from("crm_tags").update({name:g,color:E}).eq("id",u.dataset.id),await F())})}),e.querySelectorAll(".tm-tpl-btn").forEach(u=>{u.addEventListener("click",async()=>{const f=u.dataset.name,g=u.dataset.color;u.disabled=!0,u.innerHTML="✓",await h.from("crm_tags").insert({name:f,color:g,tenant_id:M()}),await F()})}),e.querySelectorAll(".tag-chip-del").forEach(u=>{u.addEventListener("click",async()=>{await h.from("crm_tags").delete().eq("id",u.dataset.id),await F()})}),document.getElementById("crm-add-status").addEventListener("click",async()=>{const u=document.getElementById("crm-new-status").value.trim(),f=document.getElementById("crm-new-status-color").value,g=document.getElementById("crm-new-status-final").checked;u&&(await h.from("crm_lead_statuses").insert({name:u,color:f,is_final:g,sort_order:99,tenant_id:M()}),document.getElementById("crm-new-status").value="",await F())}),e.querySelectorAll(".status-del").forEach(u=>{u.addEventListener("click",async()=>{confirm("Remover este status?")&&(await h.from("crm_lead_statuses").delete().eq("id",u.dataset.id),await F())})}),document.getElementById("crm-add-pipeline").addEventListener("click",async()=>{var g;const u=(g=prompt("Nome do novo funil:"))==null?void 0:g.trim();if(!u)return;const{error:f}=await h.from("crm_pipelines").insert({name:u,sort_order:99,tenant_id:M()});if(f){alert("Erro ao criar funil: "+f.message);return}tt=!1,await F()})}async function Yt(){const e=document.getElementById("section-integracoes");if(!e||e.dataset.loaded)return;e.dataset.loaded="1";const{data:t}=await h.from("integrations").select("*"),n={};t==null||t.forEach(r=>{n[r.key]=r});const a=r=>{var s;return v(((s=n[r])==null?void 0:s.value)||"")},o=r=>{var s;return(s=n[r])!=null&&s.enabled?"checked":""},i=[{key:"meta_pixel_id",icon:"📘",label:"Meta Pixel",desc:"ID do Pixel do Facebook/Instagram para rastreamento de conversões",placeholder:"123456789012345"},{key:"ga_measurement_id",icon:"📊",label:"Google Analytics 4",desc:"Measurement ID do GA4 (ex: G-XXXXXXXXXX)",placeholder:"G-XXXXXXXXXX"},{key:"gtm_container_id",icon:"🏷️",label:"Google Tag Manager",desc:"ID do container do GTM (ex: GTM-XXXXXXX)",placeholder:"GTM-XXXXXXX"},{key:"webhook_new_lead",icon:"🔔",label:"Webhook — Novo Lead",desc:"URL chamada quando um novo lead chega (POST com JSON)",placeholder:"https://..."},{key:"webhook_new_property",icon:"🏠",label:"Webhook — Novo Imóvel",desc:"URL chamada quando um imóvel é publicado",placeholder:"https://..."}],d=[{key:"smtp_host",label:"Host SMTP",placeholder:"smtp.gmail.com"},{key:"smtp_port",label:"Porta",placeholder:"587"},{key:"smtp_user",label:"Usuário",placeholder:"email@dominio.com"},{key:"smtp_from_name",label:"Nome do remetente",placeholder:"Omar Corretor"}];e.innerHTML=`
    <div class="section-topbar">
  ...
  `,document.body.appendChild(t),h.from("plans").select("id, name").then(({data:c})=>{const l=document.getElementById("nt-plan");l&&c&&(l.innerHTML='<option value="">Sem plano</option>'+c.map(m=>`<option value="${m.id}">${v(m.name)}</option>`).join(""))}),(a=document.getElementById("nt-name"))==null||a.addEventListener("input",c=>{const l=document.getElementById("nt-slug");l&&!l.dataset.manual&&(l.value=Fa(c.target.value))}),(o=document.getElementById("nt-slug"))==null||o.addEventListener("input",c=>{c.target.dataset.manual="1"}),(i=document.getElementById("nt-pwd-toggle"))==null||i.addEventListener("click",()=>{const c=document.getElementById("nt-admin-password");c.type=c.type==="password"?"text":"password"});const n=()=>t.remove();(d=document.getElementById("sa-modal-close-btn"))==null||d.addEventListener("click",n),(r=document.getElementById("nt-cancel"))==null||r.addEventListener("click",n),t.addEventListener("click",c=>{c.target===t&&n()}),(s=document.getElementById("nt-save"))==null||s.addEventListener("click",async()=>{var L,k,I,$,B,q,C,N,T,V,H,U;const c=(k=(L=document.getElementById("nt-name"))==null?void 0:L.value)==null?void 0:k.trim(),l=($=(I=document.getElementById("nt-slug"))==null?void 0:I.value)==null?void 0:$.trim(),m=(q=(B=document.getElementById("nt-domain"))==null?void 0:B.value)==null?void 0:q.trim(),p=(C=document.getElementById("nt-plan"))==null?void 0:C.value,b=(T=(N=document.getElementById("nt-admin-email"))==null?void 0:N.value)==null?void 0:T.trim(),x=(H=(V=document.getElementById("nt-admin-password"))==null?void 0:V.value)==null?void 0:H.trim(),u=document.getElementById("nt-msg"),f=document.getElementById("nt-save");if(!c||!l){u.textContent="❌ Nome e slug são obrigatórios.",u.style.color="#ef4444";return}if(!b){u.textContent="❌ Informe o e-mail do admin.",u.style.color="#ef4444";return}if(!x||x.length<6){u.textContent="❌ A senha precisa ter mínimo 6 caracteres.",u.style.color="#ef4444";return}f.disabled=!0,f.textContent="Criando…",u.textContent="⏳ Criando imobiliária…",u.style.color="#64748b";const{data:g,error:E}=await h.from("tenants").insert({name:c,slug:l,domain:m||null,plan_id:p||null,active:!0}).select();if(E){f.disabled=!1,f.textContent="Criar Imobiliária",u.textContent="❌ "+E.message,u.style.color="#ef4444";return}const w=(U=g==null?void 0:g[0])==null?void 0:U.id;u.textContent="⏳ Criando usuário admin…";const S=await pe({email:b,password:x,role:"admin",tenant_id:w});if(!(S!=null&&S.success)){f.disabled=!1,f.textContent="Criar Imobiliária",u.innerHTML="⚠️ Imobiliária criada, mas erro ao criar usuário: "+v((S==null?void 0:S.error)||"Desconhecido"),u.style.color="#f59e0b",setTimeout(()=>{n(),ie()},3e3);return}w&&(S!=null&&S.user_id)&&!(S!=null&&S.linked)&&await h.from("profiles").update({tenant_id:w}).eq("id",S.user_id),f.disabled=!1,f.textContent="Criar Imobiliária",S.email_sent===!1?(u.innerHTML=`
  ...
  `,document.body.appendChild(t),document.getElementById("tp-back").addEventListener("click",()=>t.remove()),document.getElementById("tp-edit-btn").addEventListener("click",()=>Qt(e)),t.querySelectorAll(".tp-tab").forEach(o=>{o.addEventListener("click",()=>{t.querySelectorAll(".tp-tab").forEach(i=>{i.style.fontWeight="500",i.style.color="#64748b",i.style.borderBottomColor="transparent"}),o.style.fontWeight="700",o.style.color="#2563eb",o.style.borderBottomColor="#2563eb",st(e,o.dataset.tab)})}),st(e,"properties")}function Ga(e,t){const n=document.getElementById("tp-prop-edit-modal");n&&n.remove();const a=document.createElement("div");a.id="tp-prop-edit-modal",a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,.55);z-index:10000;display:flex;align-items:center;justify-content:center;padding:16px;";const o=(r,s,c,l="text",m="")=>`<div style="display:flex;flex-direction:column;gap:4px;">
    ...
    <div style="background:#fff;border-radius:16px;width:100%;max-width:680px;max-height:90vh;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 24px 60px rgba(0,0,0,.25);">
      ...
      </div>`,(r=document.getElementById("tp-copy-key"))==null||r.addEventListener("click",()=>{var E;(E=navigator.clipboard)==null||E.writeText(e.id);const f=document.getElementById("tp-copy-key"),g=f.textContent;f.textContent="✅ Copiada!",setTimeout(()=>{f.textContent=g},2e3)})}if(t==="config"){const{data:l}=await h.from("settings").select("key,value").eq("tenant_id",e.id),m={};l==null||l.forEach(b=>{m[b.key]=b.value});const p=(b,x)=>`
      ...
      </div>`,(c=document.getElementById("tp-open-edit"))==null||c.addEventListener("click",()=>Qt(e))}}function Qt(e){var c,l,m,p,b,x,u,f;const t=document.getElementById("sa-edit-tenant-modal");t&&t.remove();const n=document.createElement("div");n.id="sa-edit-tenant-modal",n.className="sa-modal-backdrop";const a="https://onknpbzdcrhbfozzvxtz.supabase.co/functions/v1/public-api";n.innerHTML=`
    <div class="sa-modal" style="max-width:560px;">
      ...
      <div id="et-pane-api" class="sa-modal-body" style="display:none;">
        ...
        <div style="display:flex;flex-direction:column;gap:8px;">
          ${[["GET","properties","Lista imóveis publicados"],["GET","properties/ID","Detalhe de um imóvel"],["POST","leads","Registra lead / formulário de contato"],["GET","settings","Dados da empresa (nome, WhatsApp, logo…)"]].map(([g,E,w])=>`
...
await fetch(\`\${API}/leads?key=\${KEY}\`, {
  ...
  `,document.body.appendChild(n),h.from("plans").select("id, name").then(({data:g})=>{const E=document.getElementById("et-plan");E&&g&&(E.innerHTML='<option value="">Sem plano</option>'+g.map(w=>`<option value="${w.id}"${String(w.id)===String(e.plan_id)?" selected":""}>${v(w.name)}</option>`).join(""))}),(c=document.getElementById("et-logo-input"))==null||c.addEventListener("change",g=>{const E=g.target.files[0];if(!E)return;const w=URL.createObjectURL(E),S=document.getElementById("et-logo-preview");S&&(S.innerHTML=`<img src="${w}" style="width:100%;height:100%;object-fit:cover;">`)}),(l=document.getElementById("et-logo-preview"))==null||l.addEventListener("click",()=>{var g;(g=document.getElementById("et-logo-input"))==null||g.click()}),(m=document.getElementById("et-pwd-toggle"))==null||m.addEventListener("click",()=>{const g=document.getElementById("et-admin-password");g.type=g.type==="password"?"text":"password"}),(p=document.getElementById("et-copy-key"))==null||p.addEventListener("click",()=>{var S,L;const g=(S=document.getElementById("et-api-key"))==null?void 0:S.value;if(!g)return;(L=navigator.clipboard)==null||L.writeText(g);const E=document.getElementById("et-copy-key"),w=E.textContent;E.textContent="✅ Copiada!",setTimeout(()=>{E.textContent=w},2e3)});const o=["dados","config","api"];function i(g){o.forEach(E=>{document.getElementById(`et-pane-${E}`).style.display=E===g?"":"none";const w=document.getElementById(`et-tab-${E}`);w.style.borderBottomColor=E===g?"#2563eb":"transparent",w.style.color=E===g?"#2563eb":"#64748b",w.style.fontWeight=E===g?"600":"500"}),g==="config"&&r()}o.forEach(g=>{var E;return(E=document.getElementById(`et-tab-${g}`))==null?void 0:E.addEventListener("click",()=>i(g))});let d=!1;async function r(){var w;if(d)return;d=!0;const{data:g}=await h.from("settings").select("key,value").eq("tenant_id",e.id),E={};g==null||g.forEach(S=>{E[S.key]=S.value}),document.getElementById("et-pane-config").innerHTML=`
      <div class="form-group">
    ...
    `,(w=document.getElementById("et-cfg-save"))==null||w.addEventListener("click",async()=>{const S=document.getElementById("et-cfg-save"),L=document.getElementById("et-cfg-msg");S.disabled=!0,S.textContent="Salvando…",L.textContent="",L.style.color="#64748b";const k=document.getElementById("et-cfg-wa").value.trim().replace(/\D/g,""),I=document.getElementById("et-cfg-phone").value.trim(),$=document.getElementById("et-cfg-email").value.trim(),B=document.getElementById("et-cfg-city").value.trim(),q=document.getElementById("et-cfg-slogan").value.trim(),{error:C}=await h.from("settings").upsert([{key:"company.whatsapp",value:k,tenant_id:e.id},{key:"company.phone",value:I,tenant_id:e.id},{key:"company.email",value:$,tenant_id:e.id},{key:"company.city",value:B,tenant_id:e.id},{key:"company.address",value:B,tenant_id:e.id},{key:"company.slogan",value:q,tenant_id:e.id}],{onConflict:"tenant_id,key"});S.disabled=!1,S.textContent="💾 Salvar configurações",C?(L.textContent="❌ "+C.message,L.style.color="#ef4444"):(L.textContent="✅ Configurações salvas!",L.style.color="#22c55e")})}const s=()=>n.remove();(b=document.getElementById("et-close"))==null||b.addEventListener("click",s),(x=document.getElementById("et-cancel"))==null||x.addEventListener("click",s),n.addEventListener("click",g=>{g.target===n&&s()}),(u=document.getElementById("et-delete"))==null||u.addEventListener("click",async()=>{if(!confirm(`⚠️ Tem certeza que deseja EXCLUIR a imobiliária "${e.name}"?

Essa ação é irreversível e removerá o registro da plataforma.`))return;const E=document.getElementById("et-delete");E.disabled=!0,E.textContent="Excluindo…";const{error:w}=await h.from("tenants").delete().eq("id",e.id);if(w){alert("Erro ao excluir: "+w.message),E.disabled=!1,E.textContent="🗑️ Excluir";return}s(),ie()}),(f=document.getElementById("et-save"))==null||f.addEventListener("click",async()=>{var N,T,V,H,U,ae,se,$e,ke,P,ve,vt;const g=(T=(N=document.getElementById("et-name"))==null?void 0:N.value)==null?void 0:T.trim(),E=(H=(V=document.getElementById("et-slug"))==null?void 0:V.value)==null?void 0:H.trim(),w=(ae=(U=document.getElementById("et-domain"))==null?void 0:U.value)==null?void 0:ae.trim(),S=(se=document.getElementById("et-plan"))==null?void 0:se.value,L=(ke=($e=document.getElementById("et-admin-email"))==null?void 0:$e.value)==null?void 0:ke.trim(),k=(ve=(P=document.getElementById("et-admin-password"))==null?void 0:P.value)==null?void 0:ve.trim(),I=(vt=document.getElementById("et-logo-input"))==null?void 0:vt.files[0],$=document.getElementById("et-msg"),B=document.getElementById("et-save");if(!g){$.textContent="❌ Nome é obrigatório.",$.style.color="#ef4444";return}B.disabled=!0,B.textContent="Salvando…",$.textContent="⏳ Salvando…",$.style.color="#64748b";let q=e.logo_url;if(I)try{const A=await Me(I,256,.85),ft=`tenant-logos/${e.id}-${Date.now()}.jpg`,{error:na}=await h.storage.from("imoveis").upload(ft,A,{contentType:"image/jpeg",upsert:!0});if(!na){const{data:{publicUrl:oa}}=h.storage.from("imoveis").getPublicUrl(ft);q=oa}}catch(A){console.error("Logo upload:",A)}const{error:C}=await h.from("tenants").update({name:g,slug:E||e.slug,domain:w||null,plan_id:S||null,logo_url:q}).eq("id",e.id);if(C){B.disabled=!1,B.textContent="Salvar",$.textContent="❌ "+C.message,$.style.color="#ef4444";return}if(L&&k&&k.length>=6){$.textContent="⏳ Criando usuário admin…";const A=await pe({email:L,password:k,role:"admin",tenant_id:e.id});A!=null&&A.success?(A!=null&&A.user_id&&!(A!=null&&A.linked)&&await h.from("profiles").update({tenant_id:e.id}).eq("id",A.user_id),$.textContent="✅ Salvo e admin criado!",$.style.color="#22c55e"):($.textContent="⚠️ Salvo, mas erro ao criar admin: "+((A==null?void 0:A.error)||"Tente novamente"),$.style.color="#f59e0b")}else $.textContent="✅ Imobiliária atualizada!",$.style.color="#22c55e";B.disabled=!1,B.textContent="Salvar",setTimeout(()=>{s(),ie()},1200)})}const qt=[{key:"name",label:"Nome",required:!0},{key:"phone",label:"Telefone",required:!1},{key:"email",label:"E-mail",required:!1},{key:"notes",label:"Notas",required:!1}];let Je=[],ge=[],Oe={};function Wa(){var e;(e=document.getElementById("btn-import-leads"))==null||e.addEventListener("click",Ja)}function Ja(){Je=[],ge=[],Oe={};const e=document.createElement("div");e.id="import-leads-overlay",e.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;",e.innerHTML=`
...
`).map(r=>r.trimEnd()).filter(r=>r.length);if(a.length<2)return{error:"Arquivo vazio ou sem dados."};function o(r){const s=[];let c="",l=!1;for(let m=0;m<r.length;m++){const p=r[m];p==='"'?l&&r[m+1]==='"'?(c+='"',m++):l=!l:p===n&&!l?(s.push(c.trim()),c=""):c+=p}return s.push(c.trim()),s}const i=o(a[0]).map(r=>r.replace(/^["']+|["']+$/g,"")),d=a.slice(1).map(o);return{headers:i,rows:d}}async function Ka(e){const t=document.getElementById("import-upload-error");try{window.XLSX||await new Promise((i,d)=>{const r=document.createElement("script");r.src="https://cdnjs.cloudflare.com/ajax/libs/xlsx/0.18.5/xlsx.full.min.js",r.onload=i,r.onerror=d,document.head.appendChild(r)});const n=window.XLSX.read(e,{type:"array"}),a=n.Sheets[n.SheetNames[0]],o=window.XLSX.utils.sheet_to_json(a,{header:1,defval:""});if(!o||o.length<2){t.textContent="Planilha vazia.",t.style.display="";return}ge=o[0].map(String),Je=o.slice(1),ea()}catch(n){t.textContent="Erro ao ler o arquivo Excel: "+n.message,t.style.display=""}}function Qa(e,t){for(let n=0;n<e.length;n++){const a=e[n].toLowerCase();if(t.some(o=>a.includes(o)))return n}return""}function ea(){document.getElementById("import-step-upload").style.display="none",document.getElementById("import-step-map").style.display="";const e=document.getElementById("import-field-rows"),t={name:["nome","name","contact","cliente","contato"],phone:["tel","fone","celular","whatsapp","phone","mobile"],email:["email","e-mail","mail"],notes:["obs","nota","note","comment","coment","descri"]},n='<option value="">— ignorar —</option>'+ge.map((a,o)=>`<option value="${o}">${v(a)}</option>`).join("");e.innerHTML=qt.map(a=>{const o=Qa(ge,t[a.key]||[]);return Oe[a.key]=o!==""?parseInt(o):"",`
      ...
      </div>`}).join(""),qt.forEach(a=>{const o=document.getElementById(`import-map-${a.key}`);o&&Oe[a.key]!==""&&(o.value=Oe[a.key])}),Za(),Zt()}function Za(){const e=document.getElementById("import-preview-wrap");if(!e)return;const t=Je.slice(0,5);if(!t.length){e.innerHTML='<p style="padding:10px;color:#94a3b8;font-size:.8rem;">Sem dados</p>';return}const n=`<tr>${ge.map(o=>`<th style="padding:6px 10px;background:#f1f5f9;font-size:.78rem;white-space:nowrap;border:1px solid #e2e8f0;">${v(o)}</th>`).join("")}</tr>`,a=t.map(o=>`<tr>${ge.map((i,d)=>`<td style="padding:5px 10px;font-size:.78rem;border:1px solid #e2e8f0;white-space:nowrap;max-width:160px;overflow:hidden;text-overflow:ellipsis;">${v(String(o[d]??""))}</td>`).join("")}</tr>`).join("");e.innerHTML=`<table style="border-collapse:collapse;min-width:100%;">${n}${a}</table>`}function en(){return window.Chart?Promise.resolve():new Promise((e,t)=>{const n=document.createElement("script");n.src="https://cdn.jsdelivr.net/npm/chart.js@4/dist/chart.umd.min.js",n.onload=e,n.onerror=t,document.head.appendChild(n)})}function tn(e,t,n){const a=e.querySelector("#ldp-tag-badge-area"),o=e.querySelector("#ldp-tag-add-btn"),i=e.querySelector("#ldp-tag-dropdown"),d=e.querySelector("#ldp-tag-search"),r=e.querySelector("#ldp-tag-opt-list"),s=e.querySelector("#ldp-tag-show-create"),c=e.querySelector("#ldp-tag-create-row"),l=e.querySelector("#ldp-tag-new-name"),m=e.querySelector("#ldp-tag-new-color"),p=e.querySelector("#ldp-tag-create-btn");if(!a||!o||!i)return;function b(){return[...a.querySelectorAll(".ldp-tag-badge[data-tag]")].map(w=>w.dataset.tag)}function x(w){if(!w.length){a.innerHTML='<span class="ldp-tag-empty">Nenhuma tag — clique em + para adicionar</span>';return}a.innerHTML=w.map(S=>{const k=(n[S]||{}).color||"#6366F1";return`<span class="ldp-tag-badge" data-tag="${v(S)}" style="background:${k}18;color:${k};border-color:${k}55;">
        ${v(S)}<span class="ldp-tag-rm" data-tag="${v(S)}">×</span>
      </span>`}).join("")}function u(w=""){const S=b(),L=w.toLowerCase().trim(),k=t.filter(I=>!L||I.name.toLowerCase().includes(L));if(!k.length){r.innerHTML='<div class="ldp-tag-opt-empty">Nenhuma tag encontrada</div>';return}r.innerHTML=k.map(I=>{const $=S.includes(I.name);return`<div class="ldp-tag-opt${$?" active":""}" data-tag="${v(I.name)}" style="--tc:${I.color}">
        <span class="ldp-tag-opt-dot" style="background:${I.color}"></span>
        <span class="ldp-tag-opt-name">${v(I.name)}</span>
        ${$?'<span class="ldp-tag-opt-check">✓</span>':""}
      </div>`}).join("")}function f(){i.classList.remove("hidden"),u(""),d.value="",c.classList.add("hidden"),d.focus()}function g(){i.classList.add("hidden")}o.addEventListener("click",w=>{w.stopPropagation(),i.classList.contains("hidden")?f():g()}),document.addEventListener("mousedown",function w(S){e.contains(S.target)||(g(),document.removeEventListener("mousedown",w))}),i.addEventListener("mousedown",w=>w.stopPropagation()),d.addEventListener("input",()=>u(d.value)),r.addEventListener("click",w=>{const S=w.target.closest(".ldp-tag-opt");if(!S)return;const L=S.dataset.tag,k=b();k.includes(L)?x(k.filter(I=>I!==L)):x([...k,L]),u(d.value)}),a.addEventListener("click",w=>{const S=w.target.closest(".ldp-tag-rm");if(!S)return;const L=S.dataset.tag;x(b().filter(k=>k!==L))}),s.addEventListener("click",()=>{c.classList.toggle("hidden"),c.classList.contains("hidden")||l.focus()});async function E(){const w=l.value.trim();if(!w){l.focus();return}const S=m.value||"#6366F1";if(t.some(L=>L.name.toLowerCase()===w.toLowerCase())){l.style.borderColor="#ef4444",setTimeout(()=>{l.style.borderColor=""},1500);return}p.disabled=!0,p.textContent="Criando…";try{const{data:L,error:k}=await h.from("crm_tags").insert({name:w,color:S,tenant_id:y==null?void 0:y.tenant_id}).select().single();if(k)throw k;const I={id:L.id,name:L.name,color:L.color};t.push(I),n[I.name]=I,typeof ce<"u"&&(ce[I.name]=I),x([...b(),I.name]),u(d.value),l.value="",m.value="#6366F1",c.classList.add("hidden")}catch(L){console.error("Error creating tag:",L),alert("Erro ao criar tag: "+(L.message||L))}finally{p.disabled=!1,p.textContent="Criar e adicionar"}}p.addEventListener("click",E),l.addEventListener("keydown",w=>{w.key==="Enter"&&E()})}async function an(){var f;const e=document.getElementById("section-dashboard");if(!e)return;e.dataset.dbInit==="1"&&(window._dbLeadsChartInstance&&(window.window._dbLeadsChartInstance.destroy(),window._dbLeadsChartInstance=null),window._dbOriginChartInstance&&(window.window._dbOriginChartInstance.destroy(),window._dbOriginChartInstance=null)),e.dataset.dbInit="1",e.innerHTML=`
<div class="db-wrap">
...
</div>`;const n=new Date,a=n.getHours(),o=a<12?"Bom dia":a<18?"Boa tarde":"Boa noite",i=["Domingo","Segunda-feira","Terça-feira","Quarta-feira","Quinta-feira","Sexta-feira","Sábado"],d=["janeiro","fevereiro","março","abril","maio","junho","julho","agosto","setembro","outubro","novembro","dezembro"],r=`${i[n.getDay()]}, ${n.getDate()} de ${d[n.getMonth()]} de ${n.getFullYear()}`,s=((f=y==null?void 0:y.name)==null?void 0:f.split(" ")[0])||"Corretor",c=e.querySelector(".db-greeting");c&&(c.innerHTML=`${o}, <span class="db-greeting-name">${z(s)}</span> 👋`);const l=document.getElementById("db-subline");l&&(l.textContent=`Aqui está o resumo do seu negócio — ${r}`);let m=[],p=[];try{const g=M(),[E,w]=await Promise.all([Nt(),nn(g)]);m=E||[],p=w||[]}catch(g){console.warn("[Dashboard] Erro ao carregar dados:",g)}on(m,p,n);const b=m.filter(g=>g.published).length,x=p.filter(g=>dt(g.created_at)===dt(n.toISOString())).length,u=document.getElementById("db-header-chips");u&&(u.innerHTML=`
