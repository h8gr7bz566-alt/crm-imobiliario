...
${l}`),c=e.area?`<span class="icard-spec"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18M9 3v18"/></svg>${e.area}m²</span>`:"",r=e.bedrooms?`<span class="icard-spec"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M2 20v-6a2 2 0 012-2h16a2 2 0 012 2v6"/><path d="M2 14V8a2 2 0 012-2h4l2 3h8a2 2 0 012 2v3"/></svg>${e.bedrooms} quarto${e.bedrooms!=1?"s":""}</span>`:"",m=e.bathrooms?`<span class="icard-spec"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 6 6.5 3.5a1.5 1.5 0 000-2.12L6 1.5a1.5 1.5 0 00-2.12 0L2 3.38a1.5 1.5 0 000 2.12L5.5 9"/><path d="M2 20h20M20 12H4a2 2 0 00-2 2v4a2 2 0 002 2h16a2 2 0 002-2v-4a2 2 0 00-2-2z"/></svg>${e.bathrooms} banheiro${e.bathrooms!=1?"s":""}</span>`:"",u=e.parking?`<span class="icard-spec"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="1" y="3" width="15" height="13" rx="2"/><path d="M16 8h4l3 5v3h-7V8z"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg>${e.parking} vaga${e.parking!=1?"s":""}</span>`:"",x=Math.max(1,Math.min(a,6)),E=`<div class="icard-dots">${Array.from({length:x},(f,g)=>`<span class="icard-dot${g===0?" active":""}"></span>`).join("")}</div>`;return`
    <div class="imovel-card" data-pid="${e.id}">
      ...
      <div class="icard-body" data-href="property.html?id=${e.id}">
        ${(()=>{const f=e.furnishing_status||(e.furnished===!0?"mobiliado":"");return f==="mobiliado"?'<span class="icard-badge badge-furn-mob">Mobiliado</span>':f==="semimobiliado"?'<span class="icard-badge badge-furn-semi">Semimobiliado</span>':f==="vazio"?'<span class="icard-badge badge-furn-vazio">Vazio</span>':""})()}
  ...
  `}let ie=[],K=0;function Ta(e){var m,u;const t=document.getElementById("view-modal-edit");t&&(t.dataset.pid=e.id),document.getElementById("view-code").textContent=e.id||"",document.getElementById("view-modal-title").textContent=e.title||"Imóvel";const n=document.getElementById("view-status-badge");e.published?(n.textContent="● Publicado",n.className="badge badge-green"):(n.textContent="○ Rascunho",n.className="badge badge-gray");const a=[e.rua,e.numero?`nº ${e.numero}`:"",e.neighborhood,e.city].filter(Boolean);document.getElementById("view-modal-address").textContent=a.length?`📍 ${a.join(", ")}`:"";const o=`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(a.join(" "))}`;document.getElementById("view-map-link").href=o,document.getElementById("view-directions-link").href=o;const i=((m=e.images)==null?void 0:m[0])||Ie[0];document.getElementById("view-thumb-preview").src=i,ie=(u=e.images)!=null&&u.length?e.images:Ie,K=0,Ve(),document.getElementById("view-price").textContent=Ne(e.price,"pt"),document.getElementById("view-bedrooms").textContent=e.bedrooms||"—",document.getElementById("view-suites").textContent=e.suites||"—",document.getElementById("view-parking").textContent=e.parking||"—",document.getElementById("view-area").textContent=e.area?`${e.area} m²`:"—";const s=document.getElementById("view-condominium-item"),l=document.getElementById("view-condominium");l&&(l.textContent=e.condominium||""),s&&(s.style.display=e.condominium?"":"none"),document.getElementById("view-description").textContent=e.description||"Sem descrição.",document.getElementById("conf-name").textContent=e.owner_name||"—",document.getElementById("conf-phone").textContent=e.owner_phone||"—",document.getElementById("conf-email").textContent=e.owner_email||"—",document.getElementById("conf-notes").textContent=e.owner_notes||"—",document.querySelectorAll("#view-modal .tab-btn").forEach(x=>x.classList.remove("active")),document.querySelectorAll("#view-modal .tab-panel").forEach(x=>x.classList.add("hidden")),document.querySelector('#view-modal .tab-btn[data-tab="principal"]').classList.add("active"),document.getElementById("tab-principal").classList.remove("hidden");const d="https://omarcorretor.com.br/property.html?id="+e.id,c=document.getElementById("share-link-input");c&&(c.value=d);const r=document.getElementById("share-panel");r&&(r.style.display="none",r.dataset.pid=e.id),document.getElementById("view-modal").classList.remove("hidden"),document.body.style.overflow="hidden"}function De(){document.getElementById("view-modal").classList.add("hidden"),document.body.style.overflow=""}function Ve(){const e=document.getElementById("view-main-img"),t=document.getElementById("view-counter"),n=document.getElementById("view-prev"),a=document.getElementById("view-next"),o=document.getElementById("view-thumbs");e.src=ie[K],e.alt=`Foto ${K+1}`;const i=ie.length>1;n.style.display=i?"flex":"none",a.style.display=i?"flex":"none",t.textContent=i?`${K+1} / ${ie.length}`:"",o.innerHTML=i?ie.map((s,l)=>`<img src="${s}" class="view-thumb${l===K?" active":""}" data-i="${l}" alt="Foto ${l+1}">`).join(""):"",o.querySelectorAll(".view-thumb").forEach(s=>{s.addEventListener("click",()=>{K=+s.dataset.i,Ve()})})}async function kt(e){const{data:t}=await b.from("profiles").select("*").eq("id",e).maybeSingle();return t}function Ge(e){var m,u;const t=document.getElementById("topnav-avatar-initial"),n=document.getElementById("topnav-avatar-img"),a=document.getElementById("topnav-name"),o=document.getElementById("topnav-role");if(!a)return;const i=(e==null?void 0:e.name)||"Sem nome",s=(e==null?void 0:e.role)==="super_admin"?"Super Admin":(e==null?void 0:e.role)==="admin"?"Administrador":"Corretor";a.textContent=i,o&&(o.textContent=s),e!=null&&e.avatar_url&&n?(n.src=e.avatar_url,n.style.display="",t&&(t.style.display="none")):(t&&(t.textContent=((m=i[0])==null?void 0:m.toUpperCase())||"?",t.style.display=""),n&&(n.style.display="none"));const l=document.getElementById("avatar-dd-name"),d=document.getElementById("avatar-dd-role"),c=document.getElementById("avatar-dd-img"),r=document.getElementById("avatar-dd-initial");l&&(l.textContent=i),d&&(d.textContent=s),e!=null&&e.avatar_url&&c?(c.src=e.avatar_url,c.style.display="",r&&(r.style.display="none")):(r&&(r.textContent=((u=i[0])==null?void 0:u.toUpperCase())||"?",r.style.display=""),c&&(c.style.display="none"))}async function St(e){const t=document.getElementById("avatar-dd-ver-site");if(!t)return;const n=(e==null?void 0:e.tenant_id)||z(),a=n&&n!==F,o=window.location.origin,i=a?`${o}/demo.html?key=${n}`:`${o}/index.html`;if(t.href=i,!!a)try{const{data:s}=await b.from("tenants").select("domain").eq("id",n).maybeSingle(),l=window.location.hostname.replace(/^www\./,""),d=((s==null?void 0:s.domain)||"").replace(/^https?:\/\//,"").replace(/^www\./,"").replace(/\/.*$/,"").trim();d&&d!==l&&(t.href=`https://${d}`)}catch{}}const je={dashboard:()=>fn(),empresa:()=>ea(),visual:()=>ta(),"site-config":()=>aa(),"crm-config":()=>na(),integracoes:()=>oa(),midia:()=>ia(),depoimentos:()=>Zt()};function ae(e){var n,a;document.querySelectorAll(".topnav-link, .topnav-dropdown-item").forEach(o=>o.classList.remove("active"));const t=document.querySelector(`.topnav-link[data-section="${e}"], .topnav-dropdown-item[data-section="${e}"]`);if(t&&t.classList.add("active"),document.querySelectorAll(".admin-section").forEach(o=>o.classList.add("hidden")),(n=document.getElementById(`section-${e}`))==null||n.classList.remove("hidden"),typeof je<"u"&&je[e]){const o=je[e];je[e]=null,setTimeout(o,0)}(a=document.getElementById("topnav-links"))==null||a.classList.remove("open"),Z(),e==="contatos"&&Xa(),e==="funil"&&Na(),e==="tarefas"&&Da()}function Lt(e){const t=document.getElementById("admin-root");if(t&&(t.dataset.role=e||"corretor"),(e==="admin"||e==="super_admin")&&(document.querySelectorAll(".admin-only").forEach(a=>{a.style.display=""}),Object.entries({empresa:ea,visual:ta,"site-config":aa,"crm-config":na,integracoes:oa,midia:ia,depoimentos:Zt}).forEach(([a,o])=>{const i=document.querySelector(`.topnav-dropdown-item[data-section="${a}"]`)||document.querySelector(`.nav-item[data-section="${a}"]`);i&&i.addEventListener("click",()=>o(),{once:!0})}),window.lucide&&lucide.createIcons()),e==="super_admin"){document.querySelectorAll(".super-admin-only").forEach(a=>{a.style.display=""});const n=document.querySelector('.topnav-link[data-section="super-admin"]')||document.querySelector('.nav-item[data-section="super-admin"]');n&&n.addEventListener("click",()=>Za(),{once:!0}),window.lucide&&lucide.createIcons()}}function Z(){var e,t;(e=document.getElementById("avatar-dropdown"))==null||e.classList.add("hidden"),(t=document.getElementById("notif-dropdown"))==null||t.classList.add("hidden")}function Ma(){var a,o,i;const e=document.getElementById("change-pass-modal-root");e&&e.remove();const t=document.createElement("div");t.id="change-pass-modal-root",t.className="modal-backdrop",t.innerHTML=`
    ...
    </div>`,document.body.appendChild(t);const o=()=>t.remove();(s=document.getElementById("cph-close"))==null||s.addEventListener("click",o),(l=document.getElementById("cph-cancel"))==null||l.addEventListener("click",o),t.addEventListener("click",r=>{r.target===t&&o()}),(d=document.getElementById("cph-file"))==null||d.addEventListener("change",r=>{const m=r.target.files[0];if(!m)return;const u=URL.createObjectURL(m),x=document.getElementById("cph-preview"),E=document.getElementById("cph-initial");x&&(x.src=u,x.style.display=""),E&&(E.style.display="none"),document.getElementById("cph-save").disabled=!1}),(c=document.getElementById("cph-save"))==null||c.addEventListener("click",async()=>{var x;const r=(x=document.getElementById("cph-file"))==null?void 0:x.files[0];if(!r)return;const m=document.getElementById("cph-save"),u=document.getElementById("cph-msg");m.disabled=!0,m.textContent="Salvando…";try{const E=await Re(r,400,.85),p=`avatars/${y.id}-${Date.now()}.jpg`,{error:f}=await b.storage.from("imoveis").upload(p,E,{contentType:"image/jpeg",upsert:!0});if(f)throw f;const{data:{publicUrl:g}}=b.storage.from("imoveis").getPublicUrl(p);await b.from("profiles").update({avatar_url:g}).eq("id",y.id),y={...y,avatar_url:g},Ge(y),o()}catch(E){u.style.color="#ef4444",u.textContent="Erro: "+E.message,u.style.display="",m.disabled=!1,m.textContent="Salvar Foto"}})}function ot(e,t){var i,s,l;const n=document.getElementById("add-corretor-modal-root");n&&n.remove();const a=document.createElement("div");a.id="add-corretor-modal-root",a.className="modal-backdrop",a.innerHTML=`
      ...
      <div class="modal-footer">
    ...
    </div>`,document.body.appendChild(a);const o=()=>a.remove();(i=document.getElementById("ac-close"))==null||i.addEventListener("click",o),(s=document.getElementById("ac-cancel"))==null||s.addEventListener("click",o),a.addEventListener("click",d=>{d.target===a&&o()}),(l=document.getElementById("ac-save"))==null||l.addEventListener("click",async()=>{var u,x,E;const d=(u=document.getElementById("ac-email"))==null?void 0:u.value.trim(),c=(x=document.getElementById("ac-password"))==null?void 0:x.value.trim(),r=document.getElementById("ac-save"),m=document.getElementById("ac-note");if(!d){alert("Informe o e-mail do corretor.");return}if(!c||c.length<6){alert("A senha precisa ter no mínimo 6 caracteres.");return}r.disabled=!0,r.textContent="Criando…",m.style.display="none";try{const p=e||(y==null?void 0:y.tenant_id)||null,f=((E=document.getElementById("ac-role"))==null?void 0:E.value)||"corretor",g=await ue({email:d,password:c,role:f,tenant_id:p});r.disabled=!1,r.textContent="+ Criar Acesso",g.success?(document.getElementById("ac-email").value="",document.getElementById("ac-password").value="",g.email_sent===!1?(m.innerHTML=`✅ Acesso criado! <span style="color:#ef4444;">⚠️ E-mail não enviado.</span><br>Compartilhe manualmente:<br><strong>E-mail:</strong> ${v(d)}<br><strong>Senha:</strong> ${v(c)}`,m.style.color="#0f172a"):(m.textContent="✅ Acesso criado! O corretor receberá um e-mail com as credenciais.",m.style.color="#16a34a"),m.style.display="",typeof t=="function"&&setTimeout(t,1500)):alert("Erro: "+(g.error||"Falha desconhecida"))}catch(p){r.disabled=!1,r.textContent="+ Criar Acesso",alert("Erro: "+p.message)}})}function _t(){var i,s,l,d,c,r,m,u,x,E,p;const e=document.getElementById("topnav-avatar-wrap"),t=document.getElementById("avatar-dropdown");e==null||e.addEventListener("click",f=>{var h;f.stopPropagation(),(t==null?void 0:t.classList.toggle("hidden"))||(h=document.getElementById("notif-dropdown"))==null||h.classList.add("hidden")}),(i=document.getElementById("avatar-dd-change-photo"))==null||i.addEventListener("click",f=>{f.stopPropagation(),Z(),za()}),(s=document.getElementById("avatar-dd-change-pass"))==null||s.addEventListener("click",f=>{f.stopPropagation(),Z(),Ma()}),(l=document.getElementById("avatar-dd-add-corretor"))==null||l.addEventListener("click",f=>{f.stopPropagation(),Z(),ot()}),(d=document.getElementById("avatar-dd-settings"))==null||d.addEventListener("click",f=>{f.stopPropagation(),Z(),ae("settings")}),(c=document.getElementById("avatar-dd-logout"))==null||c.addEventListener("click",async f=>{f.stopPropagation(),await b.auth.signOut(),location.reload()});const n=document.getElementById("topnav-notif-wrap"),a=document.getElementById("notif-dropdown");n==null||n.addEventListener("click",f=>{var h;f.stopPropagation(),(a==null?void 0:a.classList.toggle("hidden"))||((h=document.getElementById("avatar-dropdown"))==null||h.classList.add("hidden"),Ha())}),(r=document.getElementById("notif-mark-all"))==null||r.addEventListener("click",()=>{Pa(),Z()}),(m=document.getElementById("btn-search-open"))==null||m.addEventListener("click",()=>{var f,g;(f=document.getElementById("search-overlay"))==null||f.classList.remove("hidden"),(g=document.getElementById("search-input"))==null||g.focus()}),(u=document.getElementById("search-overlay-close"))==null||u.addEventListener("click",()=>{var f;(f=document.getElementById("search-overlay"))==null||f.classList.add("hidden")}),(x=document.getElementById("search-overlay"))==null||x.addEventListener("click",f=>{f.target.id==="search-overlay"&&document.getElementById("search-overlay").classList.add("hidden")});let o;(E=document.getElementById("search-input"))==null||E.addEventListener("input",f=>{clearTimeout(o),o=setTimeout(()=>Oa(f.target.value.trim()),280)}),(p=document.getElementById("search-input"))==null||p.addEventListener("keydown",f=>{var g;f.key==="Escape"&&((g=document.getElementById("search-overlay"))==null||g.classList.add("hidden"))}),document.addEventListener("click",Z)}let it=!1,J=[],Je=[],We=[],me={},Gt=[],Q=null,_e=null,G={search:"",tags:new Set,status:""};async function Na(){var t;if(it){await Bt();return}it=!0,await Bt(),(t=document.getElementById("btn-funil-add-lead"))==null||t.addEventListener("click",()=>dt()),dn();const e=document.getElementById("funil-pipe-sel");e==null||e.addEventListener("change",async()=>{Q=parseInt(e.value,10),await Ye()})}function st(e){var i;const t=document.getElementById("kanban-filters");if(!t)return;t.style.display="block";const n=document.getElementById("kf-status");n&&(n.innerHTML='<option value="">Todos os status</option>'+Gt.map(s=>`<option value="${v(s.name)}">${v(s.name)}</option>`).join(""),n.value=G.status,n.onchange=()=>{G.status=n.value,be()});const a=document.getElementById("kf-tags");if(a){if(!e.length){a.style.display="none";return}a.style.display="flex",a.innerHTML='<span class="kf-tags-label">Tags:</span>'+e.map(s=>`<button class="kf-tag-btn${G.tags.has(s.name)?" active":""}" data-tag="${v(s.name)}"
          style="--kf-tc:${s.color}">
          ${v(s.name)}
        </button>`).join(""),a.querySelectorAll(".kf-tag-btn").forEach(s=>{s.addEventListener("click",()=>{const l=s.dataset.tag;G.tags.has(l)?G.tags.delete(l):G.tags.add(l),st(e),be()})})}const o=document.getElementById("kf-search");o&&(o.value=G.search,o.oninput=()=>{G.search=o.value.toLowerCase(),be()}),(i=document.getElementById("kf-clear"))==null||i.addEventListener("click",()=>{G={search:"",tags:new Set,status:""},st(e),be()})}async function Bt(){const e=z(),[{data:t},{data:n},{data:a}]=await Promise.all([b.from("crm_pipelines").select("*").eq("tenant_id",e).order("sort_order"),b.from("crm_tags").select("*").eq("tenant_id",e).order("name"),b.from("crm_lead_statuses").select("*").eq("tenant_id",e).order("sort_order")]);J=t||[],Gt=a||[],me={},(n||[]).forEach(l=>{me[l.name]=l});const o=J.map(l=>l.id),{data:i}=o.length?await b.from("crm_stages").select("*").in("pipeline_id",o).order("sort_order"):{data:[]};Je=i||[],st(n||[]);const s=document.getElementById("funil-pipe-sel");if(s){const l=Q;s.innerHTML=J.length?J.map(c=>`<option value="${c.id}">${v(c.name)}</option>`).join(""):'<option value="">Sem funis cadastrados</option>';const d=J.find(c=>c.id===l)||J.find(c=>c.is_default)||J[0];d?(s.value=d.id,Q=d.id):Q=null}await Ye()}async function Ye(){const e=document.getElementById("kanban-board");if(!e)return;e.innerHTML='<div class="kanban-loading">Carregando…</div>';let t=b.from("leads").select("*").order("created_at",{ascending:!1});(y==null?void 0:y.role)==="corretor"?t=t.eq("assigned_to",y.id):y!=null&&y.tenant_id&&(t=t.eq("tenant_id",y.tenant_id)),Q&&(t=t.eq("pipeline_id",Q));const{data:n}=await t;We=n||[],be()}function be(){const e=document.getElementById("kanban-board");if(!e)return;const t=Je.filter(i=>i.pipeline_id===Q);if(!t.length){e.innerHTML='<div class="kanban-loading">Nenhuma etapa configurada para este funil. Crie etapas em Configurações → CRM Config.</div>';return}const n=G,a=We.filter(i=>{if(n.search&&!`${i.name||""} ${i.phone||""} ${i.email||""}`.toLowerCase().includes(n.search)||n.status&&i.status!==n.status)return!1;if(n.tags.size>0){const s=Array.isArray(i.tags)?i.tags:[];if(![...n.tags].every(l=>s.includes(l)))return!1}return!0}),o={};t.forEach(i=>{o[i.name]=[]}),a.forEach(i=>{var l,d,c,r;const s=i.stage||((l=t[0])==null?void 0:l.name);o[s]||(o[((d=t[0])==null?void 0:d.name)||""]=[]),(r=o[s]||o[(c=t[0])==null?void 0:c.name])==null||r.push(i)}),e.innerHTML=t.map(i=>{const s=o[i.name]||[],l=s.length?s.map(d=>{const c=(d.phone||"").replace(/\D/g,""),r=encodeURIComponent(`Olá ${d.name}! Aqui é da ${W("company.name","nossa imobiliária")}. Vi seu interesse e gostaria de ajudar. Posso falar agora?`);return`
        <div class="kanban-card" draggable="true" data-id="${d.id}" data-stage="${v(i.name)}" style="cursor:pointer;">
          ...
          <div class="kanban-card-tags" style="display:flex;flex-wrap:wrap;gap:4px;margin-top:6px;">
            ...
            ${Array.isArray(d.tags)?d.tags.map(m=>{const u=me[m],x=(u==null?void 0:u.color)||"#0369a1";return`<span class="kanban-card-tag" style="background:${x}18;color:${x};border:1px solid ${x}44;">${v(m)}</span>`}).join(""):""}
          </div>
  ...
  `,document.body.appendChild(r),requestAnimationFrame(()=>{r.style.transform="translateX(0)"}),gn(r,a||[],me);const m=r.querySelector("#ldp-pipe"),u=r.querySelector("#ldp-stage");m&&u&&m.addEventListener("change",()=>{const g=m.value,h=Je.filter(I=>I.pipeline_id===g);u.innerHTML=h.map(I=>`<option value="${I.name}">${I.name}</option>`).join("")||'<option value="">— sem etapas —</option>'});const x=()=>{r.style.transform="translateX(100%)",setTimeout(()=>r.remove(),250)};document.getElementById("ldp-close").addEventListener("click",x),document.getElementById("ldp-save").addEventListener("click",async()=>{var A,B;const g=document.getElementById("ldp-save"),h=document.getElementById("ldp-msg"),I=document.getElementById("ldp-name").value.trim();if(!I){h.style.color="#ef4444",h.textContent="Nome é obrigatório.";return}g.disabled=!0,g.textContent="Salvando…";const S=[...r.querySelectorAll("#ldp-tag-badge-area .ldp-tag-badge[data-tag]")].map(q=>q.dataset.tag),L=document.getElementById("ldp-pipe"),k=L?L.value:i||Q,w=t?typeof $t=="function"?$t():{}:{},$={name:I,phone:document.getElementById("ldp-phone").value.trim()||null,email:document.getElementById("ldp-email").value.trim()||null,source:document.getElementById("ldp-source").value.trim()||null,pipeline_id:k||null,stage:((A=document.getElementById("ldp-stage"))==null?void 0:A.value)||null,status:((B=document.getElementById("ldp-status"))==null?void 0:B.value)||null,notes:document.getElementById("ldp-notes").value.trim()||null,tags:S,tenant_id:z(),...t?{utm_source:w.utm_source||null,utm_medium:w.utm_medium||null,utm_campaign:w.utm_campaign||null,utm_content:w.utm_content||null,utm_term:w.utm_term||null,fbclid:w.fbclid||null,gclid:w.gclid||null,fbp:w.fbp||null,fbc:w.fbc||null,landing_url:w.landing_url||null,user_agent:navigator.userAgent||null}:{}};let _;if(t?{error:_}=await b.from("leads").insert($):{error:_}=await b.from("leads").update($).eq("id",e.id),g.disabled=!1,g.textContent="💾 Salvar",_){h.style.color="#ef4444",h.textContent="Erro: "+_.message;return}h.style.color="#22c55e",h.textContent="✅ Salvo!",t&&typeof sendLeadToCAPI=="function"&&sendLeadToCAPI({name:$.name,email:$.email,phone:$.phone,tracking:w}).then(async q=>{if(q!=null&&q.ok){console.log("[CAPI] Lead enviado ao Meta:",q.event_id);try{await b.from("leads").update({capi_event_id:q.event_id,capi_sent_at:new Date().toISOString()}).eq("name",$.name).eq("phone",$.phone||"").order("created_at",{ascending:!1}).limit(1)}catch(T){console.warn("[CAPI] falha ao salvar event_id:",T)}}else console.warn("[CAPI] não confirmado pelo Meta:",q)}).catch(q=>console.warn("[CAPI] erro:",q)),setTimeout(()=>{x(),Ye()},700)}),(f=document.getElementById("ldp-delete"))==null||f.addEventListener("click",async()=>{confirm(`Excluir o lead "${e==null?void 0:e.name}"?`)&&(await b.from("leads").delete().eq("id",e.id),x(),Ye())})}let j=[],Ct=!1,he="pending";async function Da(){var e;Ct||(Ct=!0,await ja(),(e=document.getElementById("btn-nova-tarefa"))==null||e.addEventListener("click",()=>Wt()),document.querySelectorAll(".tarefa-filter-btn").forEach(t=>{t.addEventListener("click",()=>{document.querySelectorAll(".tarefa-filter-btn").forEach(n=>n.classList.remove("active")),t.classList.add("active"),he=t.dataset.filter,$e()})}))}async function ja(){const e=document.getElementById("tarefas-list");e&&(e.innerHTML='<div class="empty-row" style="padding:40px;text-align:center;color:#94a3b8;">Carregando…</div>');let t=b.from("tasks").select("*").order("due_date",{ascending:!0,nullsFirst:!1});(y==null?void 0:y.role)==="corretor"?t=t.eq("assigned_to",y.id):y!=null&&y.tenant_id&&(t=t.eq("tenant_id",y.tenant_id));const{data:n,error:a}=await t;if(a){const o=document.getElementById("tarefas-list");o&&(o.innerHTML=`
...
CREATE POLICY "tasks_access" ON public.tasks
  ...
  `,document.body.appendChild(s);const l=()=>s.remove();(d=document.getElementById("tm-close"))==null||d.addEventListener("click",l),(c=document.getElementById("tm-cancel"))==null||c.addEventListener("click",l),s.addEventListener("click",u=>{u.target===s&&l()}),(r=document.getElementById("tm-toggle-done"))==null||r.addEventListener("click",async()=>{const u=a?"pending":"done";await b.from("tasks").update({status:u}).eq("id",e.id);const x=j.find(E=>String(E.id)===String(e.id));x&&(x.status=u),l(),u==="done"&&(he="done",document.querySelectorAll(".tarefa-filter-btn").forEach(E=>{E.classList.toggle("active",E.dataset.filter==="done")})),$e()}),(m=document.getElementById("tm-save"))==null||m.addEventListener("click",async()=>{var g,h;const u=document.getElementById("tarefa-form");if(!u.checkValidity()){u.reportValidity();return}const x=new FormData(u),E=document.getElementById("tm-save");E.disabled=!0,E.textContent="Salvando…";const p={title:(g=x.get("title"))==null?void 0:g.trim(),description:((h=x.get("description"))==null?void 0:h.trim())||null,due_date:x.get("due_date")||null,priority:x.get("priority")||"medium",status:(e==null?void 0:e.status)||"pending",assigned_to:(y==null?void 0:y.id)||null,tenant_id:(y==null?void 0:y.tenant_id)||null};let f;if(n){if({error:f}=await b.from("tasks").update(p).eq("id",e.id),!f){const I=j.findIndex(S=>String(S.id)===String(e.id));I>=0&&(j[I]={...j[I],...p})}}else{const{data:I,error:S}=await b.from("tasks").insert(p).select();f=S,!f&&(I!=null&&I[0])&&j.unshift(I[0])}if(E.disabled=!1,E.textContent=n?"Salvar":"Criar Tarefa",f){alert("Erro: "+f.message);return}l(),$e()})}async function Oa(e){const t=document.getElementById("search-results");if(!t)return;if(!e||e.length<2){t.innerHTML='<div class="search-hint">Digite para pesquisar…</div>';return}t.innerHTML='<div class="search-hint">Buscando…</div>';const n=`%${e}%`;y==null||y.role,y==null||y.tenant_id;const[{data:a},{data:o}]=await Promise.all([b.from("properties").select("id,title,reference,type,city").ilike("title",n).limit(5),b.from("leads").select("id,name,phone,email").or(`name.ilike.${n},phone.ilike.${n},email.ilike.${n}`).limit(5)]),i=[];a!=null&&a.length&&(i.push('<div class="search-group-label">Imóveis</div>'),i.push(...a.map(s=>`
...
João Silva,joao@email.com,(47)99999-0000,Imobiliária ABC,Diretor`,u=new Blob([m],{type:"text/csv"}),x=document.createElement("a");x.href=URL.createObjectURL(u),x.download="modelo_contatos.csv",x.click()}),(d=document.getElementById("import-csv-file"))==null||d.addEventListener("change",Va),(c=document.getElementById("import-modal-confirm"))==null||c.addEventListener("click",Ga)))}async function Yt(){const e=document.getElementById("contatos-tbody");e&&(e.innerHTML='<tr><td colspan="8" class="empty-row">Carregando…</td></tr>');let t=b.from("leads").select("*").order("created_at",{ascending:!1});(y==null?void 0:y.role)==="corretor"?t=t.eq("assigned_to",y.id):y!=null&&y.tenant_id&&(t=t.eq("tenant_id",y.tenant_id));const{data:a}=await t;Y=a||[],pe()}function pe(){var l,d,c;const e=(((l=document.getElementById("contato-search"))==null?void 0:l.value)||"").toLowerCase(),t=e?Y.filter(r=>(r.name||"").toLowerCase().includes(e)||(r.email||"").toLowerCase().includes(e)||(r.phone||"").toLowerCase().includes(e)):Y,n=t.length,a=Math.max(1,Math.ceil(n/Be));D>a&&(D=a);const o=t.slice((D-1)*Be,D*Be),i=document.getElementById("contatos-tbody");if(!i)return;o.length?i.innerHTML=o.map(r=>`
      <tr>
        ...
        <td style="display:flex;gap:6px;align-items:center;">
          ${(()=>{const m=(r.phone||"").replace(/\D/g,"");if(!m)return"";const u=encodeURIComponent(`Olá ${(r.name||"").split(" ")[0]}! Aqui é da ${W("company.name","nossa imobiliária")}. Podemos conversar sobre seu interesse em imóveis?`);return`<a href="https://wa.me/${m}?text=${u}" target="_blank" rel="noopener" title="WhatsApp"
    ...
    `).join(""):i.innerHTML='<tr><td colspan="8" class="empty-row">Nenhum contato encontrado.</td></tr>';const s=document.getElementById("contatos-pagination");if(s){const r=n===0?0:(D-1)*Be+1,m=Math.min(D*Be,n);s.innerHTML=`
    ...
    `,(d=s.querySelector("#pag-prev"))==null||d.addEventListener("click",()=>{D--,pe()}),(c=s.querySelector("#pag-next"))==null||c.addEventListener("click",()=>{D++,pe()})}document.querySelectorAll(".contato-edit-btn, .contato-name-link").forEach(r=>{r.addEventListener("click",m=>{m.preventDefault();const u=r.dataset.id,x=Y.find(E=>String(E.id)===String(u));x&&Kt(x)})})}async function Kt(e=null){var g,h,I,S,L,k;const t=document.getElementById("contato-modal-root");t&&t.remove();const n=!!e,a=z(),[{data:o},{data:i},{data:s}]=await Promise.all([b.from("crm_pipelines").select("*").eq("tenant_id",a).order("sort_order"),b.from("crm_tags").select("*").eq("tenant_id",a).order("name"),b.from("crm_lead_statuses").select("*").eq("tenant_id",a).order("sort_order")]),l=o||[],d=i||[],c=s||[],r=l.map(w=>w.id),{data:m}=r.length?await b.from("crm_stages").select("*").in("pipeline_id",r).order("sort_order"):{data:[]},u=m||[],x=(e==null?void 0:e.pipeline_id)||((g=l[0])==null?void 0:g.id)||"";function E(w){const $=u.filter(_=>_.pipeline_id===w);return $.length?'<option value="">— Selecionar etapa —</option>'+$.map(_=>`<option value="${v(_.name)}" ${(e==null?void 0:e.stage)===_.name?"selected":""}>${v(_.name)}</option>`).join(""):'<option value="">— Nenhuma etapa —</option>'}const p=document.createElement("div");p.id="contato-modal-root",p.className="modal-backdrop",p.innerHTML=`
  ...
  `,document.body.appendChild(p);const f=()=>p.remove();(h=document.getElementById("cm-close"))==null||h.addEventListener("click",f),(I=document.getElementById("cm-cancel"))==null||I.addEventListener("click",f),p.addEventListener("click",w=>{w.target===p&&f()}),(S=document.getElementById("cm-pipe"))==null||S.addEventListener("change",w=>{const $=document.getElementById("cm-stage");$&&($.innerHTML=E(w.target.value))}),(L=document.getElementById("cm-delete"))==null||L.addEventListener("click",async()=>{if(!confirm(`Excluir o contato "${e==null?void 0:e.name}"?`))return;await b.from("leads").delete().eq("id",e.id);const w=Y.findIndex($=>String($.id)===String(e.id));w>=0&&Y.splice(w,1),f(),pe()}),(k=document.getElementById("cm-save"))==null||k.addEventListener("click",async()=>{var V,O,H,ne,re,ke,Se;const w=document.getElementById("contato-form");if(!w.checkValidity()){w.reportValidity();return}const $=new FormData(w),_=document.getElementById("cm-save");_.disabled=!0,_.textContent="Salvando…";const A=$.getAll("tag"),B=$.get("pipeline_id")||null,q={name:(V=$.get("name"))==null?void 0:V.trim(),company:((O=$.get("company"))==null?void 0:O.trim())||null,email:((H=$.get("email"))==null?void 0:H.trim())||null,phone:((ne=$.get("phone"))==null?void 0:ne.trim())||null,job_title:((re=$.get("job_title"))==null?void 0:re.trim())||null,city_interest:((ke=$.get("city_interest"))==null?void 0:ke.trim())||null,notes:((Se=$.get("notes"))==null?void 0:Se.trim())||null,pipeline_id:B,stage:$.get("stage")||null,status:$.get("status")||null,tags:A,assigned_to:(y==null?void 0:y.id)||null,tenant_id:(y==null?void 0:y.tenant_id)||null,source:(e==null?void 0:e.source)||"manual"};let T;if(n){if({error:T}=await b.from("leads").update(q).eq("id",e.id),!T){const P=Y.findIndex(ve=>String(ve.id)===String(e.id));P>=0&&(Y[P]={...Y[P],...q})}}else{const{data:P,error:ve}=await b.from("leads").insert(q).select();T=ve,!T&&(P!=null&&P[0])&&Y.unshift(P[0])}if(_.disabled=!1,_.textContent=n?"Salvar":"Criar Contato",T){alert("Erro: "+T.message);return}f(),pe()})}let we=[];function Va(e){const t=e.target.files[0];if(!t)return;const n=new FileReader;n.onload=a=>{we=a.target.result.split(`
`).filter(l=>l.trim()).slice(1).map(l=>{const[d,c,r,m,u]=l.split(",").map(x=>x.trim().replace(/^"|"$/g,""));return{name:d,email:c,phone:r,company:m,job_title:u}}).filter(l=>l.name);const i=document.getElementById("import-preview");i&&(i.textContent=`${we.length} contato(s) encontrados para importar.`);const s=document.getElementById("import-modal-confirm");s&&(s.disabled=we.length===0)},n.readAsText(t)}async function Ga(){if(!we.length)return;const e=document.getElementById("import-modal-confirm");e&&(e.disabled=!0,e.textContent="Importando…");const t=we.map(a=>({...a,stage:"novo",source:"importado",assigned_to:(y==null?void 0:y.id)||null,tenant_id:(y==null?void 0:y.tenant_id)||null})),{error:n}=await b.from("leads").insert(t);if(e&&(e.disabled=!1,e.textContent="Importar"),n){alert("Erro na importação: "+n.message);return}rt(),await Yt(),alert(`${t.length} contato(s) importados com sucesso!`)}function Ja(){const e=document.getElementById("import-modal");e&&e.classList.remove("hidden"),we=[];const t=document.getElementById("import-preview");t&&(t.textContent="");const n=document.getElementById("import-modal-confirm");n&&(n.disabled=!0);const a=document.getElementById("import-csv-file");a&&(a.value="")}function rt(){const e=document.getElementById("import-modal");e&&e.classList.add("hidden")}const Wa="https://onknpbzdcrhbfozzvxtz.supabase.co/functions/v1/invite-user";async function ue(e){return(await fetch(Wa,{method:"POST",headers:{Authorization:`Bearer ${ga}`,"Content-Type":"application/json"},body:JSON.stringify(e)})).json()}async function At(e){var d,c,r,m;const t=document.getElementById("settings-name"),n=document.getElementById("settings-email"),a=document.getElementById("settings-avatar-preview"),o=document.getElementById("settings-avatar-initial"),i=document.getElementById("settings-avatar-input"),s=document.getElementById("settings-save-profile");if(!t)return;if(t.value=(e==null?void 0:e.name)||"",n){const{data:{user:u}}=await b.auth.getUser();n.value=(u==null?void 0:u.email)||""}const l=((e==null?void 0:e.name)||"?")[0].toUpperCase();if(o&&(o.textContent=l),e!=null&&e.avatar_url&&a&&(a.src=e.avatar_url,a.style.display="",o&&(o.style.display="none")),i==null||i.addEventListener("change",u=>{const x=u.target.files[0];if(!x)return;const E=URL.createObjectURL(x);a&&(a.src=E,a.style.display=""),o&&(o.style.display="none")}),(d=document.getElementById("btn-change-password"))==null||d.addEventListener("click",async()=>{var g,h;const u=((g=document.getElementById("change-password-new"))==null?void 0:g.value)||"",x=((h=document.getElementById("change-password-confirm"))==null?void 0:h.value)||"",E=document.getElementById("change-password-msg"),p=document.getElementById("btn-change-password");if(E&&(E.style.display="none"),u.length<6){E&&(E.textContent="Mínimo 6 caracteres.",E.style.display="");return}if(u!==x){E&&(E.textContent="As senhas não coincidem.",E.style.display="");return}p&&(p.disabled=!0,p.textContent="Salvando…");const{error:f}=await b.auth.updateUser({password:u});p&&(p.disabled=!1,p.textContent="Salvar Nova Senha"),f?E&&(E.textContent="Erro: "+f.message,E.style.display=""):(E&&(E.style.color="#16a34a",E.textContent="Senha alterada com sucesso!",E.style.display=""),document.getElementById("change-password-new").value="",document.getElementById("change-password-confirm").value="")}),s==null||s.addEventListener("click",async()=>{var h;const u=t.value.trim();let x=(y==null?void 0:y.avatar_url)||"";const E=i==null?void 0:i.files[0],p=s.textContent;if(s.disabled=!0,s.textContent="Salvando…",E)try{const I=await Re(E,400,.85),S=`avatars/${y.id}-${Date.now()}.jpg`,{error:L}=await b.storage.from("imoveis").upload(S,I,{contentType:"image/jpeg",upsert:!0});if(!L){const{data:{publicUrl:k}}=b.storage.from("imoveis").getPublicUrl(S);x=k}}catch(I){console.error("Avatar upload:",I)}const{error:f}=await b.from("profiles").update({name:u,avatar_url:x}).eq("id",y.id);if(s.disabled=!1,s.textContent=p,f){alert("Erro ao salvar perfil.");return}y={...y,name:u,avatar_url:x},Ge(y);const g=document.getElementById("settings-avatar-initial");g&&(g.textContent=((h=u[0])==null?void 0:h.toUpperCase())||"?")}),(e==null?void 0:e.role)==="admin"||(e==null?void 0:e.role)==="super_admin"){const u=document.getElementById("settings-corretores-section");u&&(u.style.display=""),await Ke(),(c=document.getElementById("btn-invite-corretor"))==null||c.addEventListener("click",async()=>{var h,I;const E=(h=document.getElementById("invite-email"))==null?void 0:h.value.trim(),p=(I=document.getElementById("invite-password"))==null?void 0:I.value.trim(),f=document.getElementById("btn-invite-corretor"),g=document.getElementById("invite-note");if(!E){alert("Informe o e-mail do corretor.");return}if(!p||p.length<6){alert("A senha precisa ter no mínimo 6 caracteres.");return}f&&(f.disabled=!0,f.textContent="Criando…"),g&&(g.style.display="none");try{const S=await ue({email:E,password:p,tenant_id:(y==null?void 0:y.tenant_id)||null});if(S.success){const L=document.getElementById("invite-email"),k=document.getElementById("invite-password");L&&(L.value=""),k&&(k.value=""),await Ke(),g&&(S.email_sent===!1?(g.innerHTML=`
                ✅ Acesso criado!
                ...
                <strong>Senha:</strong> ${v(p)}`,g.style.color="#0f172a"):(g.textContent="✅ Acesso criado! O corretor receberá um e-mail com o login e a senha.",g.style.color="#16a34a"),g.style.display="")}else alert("Erro: "+(S.error||"Falha desconhecida"))}catch(S){alert("Erro ao criar acesso: "+S.message)}finally{f&&(f.disabled=!1,f.textContent="+ Criar Acesso")}});const x=document.getElementById("settings-locations-section");x&&(x.style.display=""),await qe(),(r=document.getElementById("loc-add-city-btn"))==null||r.addEventListener("click",async()=>{const E=document.getElementById("loc-new-city"),p=E==null?void 0:E.value.trim();if(!p)return;const{error:f}=await b.from("locations").insert({type:"cidade",name:p});if(f){alert("Erro ao adicionar cidade.");return}E&&(E.value=""),await qe(),yt()}),(m=document.getElementById("loc-add-neighborhood-btn"))==null||m.addEventListener("click",async()=>{var h;const E=parseInt((h=document.getElementById("loc-new-neighborhood-city"))==null?void 0:h.value,10),p=document.getElementById("loc-new-neighborhood"),f=p==null?void 0:p.value.trim();if(!E||!f){alert("Selecione a cidade e informe o nome do bairro.");return}const{error:g}=await b.from("locations").insert({type:"bairro",name:f,parent_id:E});if(g){alert("Erro ao adicionar bairro.");return}p&&(p.value=""),await qe()})}}async function Ke(){const e=document.getElementById("corretores-list");if(!e)return;let t=b.from("profiles").select("*").order("created_at");y!=null&&y.tenant_id&&(t=t.eq("tenant_id",y.tenant_id));const{data:n,error:a}=await t;if(a||!n){e.innerHTML='<p style="color:#6b7280;font-size:14px">Erro ao carregar.</p>';return}e.innerHTML=n.map(o=>{const i=(o.name||"?")[0].toUpperCase(),s=o.avatar_url?`<img src="${o.avatar_url}" class="corretor-avatar" alt="">`:`<div class="corretor-avatar-initial">${v(i)}</div>`,l=o.id===(y==null?void 0:y.id),d=o.active!==!1,c=d?'<span class="badge badge-green">Ativo</span>':'<span class="badge badge-gray">Pausado</span>',r=l?'<span class="corretor-you">Você</span>':`<select class="form-control corretor-role-sel" data-uid="${o.id}" style="width:110px;padding:6px 8px;font-size:13px">
...
`+k);window.open("mailto:?subject="+$+"&body="+_,"_blank")}),(I=document.getElementById("share-copy"))==null||I.addEventListener("click",()=>{var w;const k=document.getElementById("share-link-input");k&&((w=navigator.clipboard)==null||w.writeText(k.value).then(()=>{const $=document.getElementById("share-copy"),_=$.textContent;$.textContent="✅ Copiado!",setTimeout(()=>{$.textContent=_},2e3)}))}),(S=document.getElementById("view-modal-edit"))==null||S.addEventListener("click",()=>{var q;if((y==null?void 0:y.role)!=="admin"&&(y==null?void 0:y.role)!=="super_admin")return;const k=Number(document.getElementById("view-modal-edit").dataset.pid),w=C.find(T=>T.id===k);if(!w)return;De(),U=w.id;const $=document.getElementById("property-form"),_=document.getElementById("form-submit-btn");_.textContent="Salvar Alterações",$.querySelector('[name="title"]').value=w.title||"",$.querySelector('[name="rua"]').value=w.rua||"",$.querySelector('[name="numero"]').value=w.numero||"",$.querySelector('[name="city"]').value=w.city||"",$.querySelector('[name="price"]').value=w.price||"",$.querySelector('[name="bedrooms"]').value=w.bedrooms||"",$.querySelector('[name="suites"]').value=w.suites||"",$.querySelector('[name="parking"]').value=w.parking||"",$.querySelector('[name="description"]').value=w.description||"",$.querySelector('[name="construction_status"]').value=w.construction_status||"",$.querySelector('[name="owner_name"]').value=w.owner_name||"",$.querySelector('[name="owner_phone"]').value=w.owner_phone||"",$.querySelector('[name="owner_email"]').value=w.owner_email||"",$.querySelector('[name="owner_notes"]').value=w.owner_notes||"",$.querySelector('[name="condominium"]').value=w.condominium||"";const A=document.getElementById("adminPublished");A&&(A.value=w.published===!0?"true":"false");const B=document.getElementById("adminCitySelect");B&&(B.value=w.city||"",B.dispatchEvent(new Event("change")),setTimeout(()=>{const T=document.getElementById("adminNeighborhood");T&&(T.value=w.neighborhood||"")},50)),ee=w.cover_image||((q=w.images)==null?void 0:q[0])||"",ze(w.images||[]),nt("Editar Imóvel")}),document.querySelectorAll(".tab-btn[data-tab]").forEach(k=>{k.addEventListener("click",()=>{var w;document.querySelectorAll(".tab-btn").forEach($=>$.classList.remove("active")),k.classList.add("active"),document.querySelectorAll(".tab-panel").forEach($=>$.classList.add("hidden")),(w=document.getElementById(`tab-${k.dataset.tab}`))==null||w.classList.remove("hidden")})}),(L=document.getElementById("admin-properties"))==null||L.addEventListener("click",k=>{if(k.target.closest(".action-btns"))return;const w=k.target.closest("tr");if(!w)return;const $=Number(w.dataset.id);if(!$)return;const _=C.find(A=>A.id===$);_&&Ta(_)})}async function Zt(){const e=document.getElementById("section-depoimentos");if(!e||e.dataset.loaded)return;e.dataset.loaded="1";const{data:t}=await b.from("site_content").select("value_pt").eq("key","testimonials").eq("tenant_id",z()).maybeSingle();let n=[];try{n=JSON.parse((t==null?void 0:t.value_pt)||"[]")}catch{n=[]}function a(s){const l=["#0d2144","#1a3a5c","#0a1628","#164a3c","#2d1b3d","#3d1a1a","#1a2f4a"];let d=0;for(const c of s||"?")d=d*31+c.charCodeAt(0)&4294967295;return l[Math.abs(d)%l.length]}function o(){e.querySelector("#dep-save-msg"),e.innerHTML=`
    ...
    `,e.dataset.loaded="1";let s=-1;function l(d=-1){s=d;const c=e.querySelector("#dep-form-wrap");c.style.display="",e.querySelector("#dep-form-title").textContent=d>=0?"Editar Depoimento":"Novo Depoimento";const r=d>=0?n[d]:{};e.querySelector("#dep-stars").value=String(r.stars||5),e.querySelector("#dep-text").value=r.text||"",e.querySelector("#dep-name").value=r.name||"",e.querySelector("#dep-role").value=r.role||"",e.querySelector("#dep-text").focus()}e.querySelector("#dep-add-btn").addEventListener("click",()=>l(-1)),e.querySelector("#dep-form-cancel").addEventListener("click",()=>{e.querySelector("#dep-form-wrap").style.display="none",s=-1}),e.addEventListener("click",d=>{const c=d.target.closest("[data-edit]"),r=d.target.closest("[data-del]");if(c&&l(parseInt(c.dataset.edit)),r){const m=parseInt(r.dataset.del);confirm("Remover este depoimento?")&&(n.splice(m,1),i().then(()=>o()))}}),e.querySelector("#dep-form-save").addEventListener("click",async()=>{const d=e.querySelector("#dep-form-save"),c=e.querySelector("#dep-save-msg"),r=e.querySelector("#dep-text").value.trim(),m=e.querySelector("#dep-name").value.trim(),u=e.querySelector("#dep-role").value.trim(),x=parseInt(e.querySelector("#dep-stars").value);if(!r||!m){alert("Preencha o depoimento e o nome.");return}d.disabled=!0,d.textContent="Salvando…";const E={stars:x,text:r,name:m,role:u};s>=0?n[s]=E:n.push(E);const p=await i();d.disabled=!1,d.textContent="Salvar",X(c,p),p&&(e.querySelector("#dep-form-wrap").style.display="none",s=-1,o())})}async function i(){const s=JSON.stringify(n);return await Oe("testimonials",{pt:s,en:s,es:s})}o()}document.addEventListener("DOMContentLoaded",async()=>{var l,d,c;const t=window.location.hostname.replace(/^www\./,"");if(t&&t!=="localhost"&&t!=="127.0.0.1"){const r=`imobi_tenant_${t}`,m=He(r);if(m)ye(m);else{let u=null;for(const x of[t,"www."+t]){const{data:E}=await b.from("tenants").select("id").eq("domain",x).maybeSingle();if(E!=null&&E.id){u=E;break}}u!=null&&u.id&&(ye(u.id),at(r,u.id,24*60*60*1e3))}}await Promise.all([ua(),Qt()]),Ce=W("company.whatsapp",Ce),pt(),La(),_a();const n=document.getElementById("adminCitySelect"),a=document.getElementById("adminNeighborhood");n&&a&&(yt(),n.addEventListener("change",()=>{const r=ge().find(u=>u.name===n.value),m=r?vt(r.id):[];a.innerHTML='<option value="">Selecione a cidade primeiro</option>'+m.map(u=>`<option value="${u.name}">${v(u.name)}</option>`).join("")}));const o=document.getElementById("admin-login"),i=document.getElementById("admin-root");if(o){const r=new URLSearchParams(window.location.hash.replace("#","")),m=new URLSearchParams(window.location.search),u=r.get("type")||m.get("type")||"",x=Ht||u==="recovery"||u==="invite"||window.location.hash.includes("access_token")||m.has("code"),E=document.getElementById("password-reset-overlay");if(x){o.style.display="none",i&&i.classList.add("hidden"),E&&(E.style.display="flex"),(l=document.getElementById("password-reset-form"))==null||l.addEventListener("submit",async f=>{var k,w;f.preventDefault();const g=((k=document.getElementById("new-password"))==null?void 0:k.value)||"",h=((w=document.getElementById("confirm-password"))==null?void 0:w.value)||"",I=document.getElementById("password-reset-msg"),S=f.target.querySelector('button[type="submit"]');if(I&&(I.style.display="none"),g!==h){I&&(I.textContent="As senhas não coincidem.",I.style.display="");return}S&&(S.disabled=!0,S.textContent="Salvando…");const{error:L}=await b.auth.updateUser({password:g});if(L){I&&(I.textContent="Erro: "+L.message,I.style.display=""),S&&(S.disabled=!1,S.textContent="Definir Senha");return}window.location.href=window.location.pathname}),m.has("code")&&await b.auth.exchangeCodeForSession(m.get("code")??"");return}const{data:{session:p}}=await b.auth.getSession();if(p){if(o.classList.add("hidden"),i&&i.classList.remove("hidden"),et(),Tt(),_t(),window.lucide&&lucide.createIcons(),y=await kt(p.user.id),!y){await b.auth.signOut(),o.classList.remove("hidden"),i&&i.classList.add("hidden");return}if(y.active===!1){await b.auth.signOut(),o.classList.remove("hidden"),i&&i.classList.add("hidden"),alert("Seu acesso está pausado. Entre em contato com o administrador.");return}if(y.needs_password_reset){o.style.display="none",i&&i.classList.add("hidden");const f=document.getElementById("password-reset-overlay");f&&(f.style.display="flex"),(d=document.getElementById("password-reset-form"))==null||d.addEventListener("submit",async g=>{var w,$;g.preventDefault();const h=((w=document.getElementById("new-password"))==null?void 0:w.value)||"",I=(($=document.getElementById("confirm-password"))==null?void 0:$.value)||"",S=document.getElementById("password-reset-msg"),L=g.target.querySelector('button[type="submit"]');if(S&&(S.style.display="none"),h!==I){S&&(S.textContent="As senhas não coincidem.",S.style.display="");return}if(h.length<6){S&&(S.textContent="Mínimo 6 caracteres.",S.style.display="");return}L&&(L.disabled=!0,L.textContent="Salvando…");const{error:k}=await b.auth.updateUser({password:h});if(k){S&&(S.textContent="Erro: "+k.message,S.style.display=""),L&&(L.disabled=!1,L.textContent="Definir Senha");return}await b.from("profiles").update({needs_password_reset:!1}).eq("id",y.id),window.location.href=window.location.pathname});return}ye((y==null?void 0:y.tenant_id)||null),Ge(y),St(y),Lt(y.role),await Xe(),await At(y),window.lucide&&lucide.createIcons(),Fa(),ae("dashboard")}else{i&&i.classList.add("hidden"),o.classList.remove("hidden");const f=document.getElementById("login-form");f&&((c=document.getElementById("forgot-password-btn"))==null||c.addEventListener("click",async()=>{var I,S;const g=(S=(I=f.querySelector('input[name="email"]'))==null?void 0:I.value)==null?void 0:S.trim();if(!g){alert("Digite seu e-mail no campo acima primeiro.");return}const{error:h}=await b.auth.resetPasswordForEmail(g,{redirectTo:"https://omarcorretor.com.br/ios.imobi.html"});alert(h?"Erro: "+h.message:"E-mail de redefinição enviado! Verifique sua caixa de entrada.")}),f.addEventListener("submit",async g=>{g.preventDefault();const h=f.querySelector('button[type="submit"]'),I=new FormData(f),S=I.get("email"),L=I.get("password");h&&(h.disabled=!0,h.textContent="Entrando…");try{if(await Ea(S,L)){o.classList.add("hidden"),i&&i.classList.remove("hidden"),et(),Tt(),window.lucide&&lucide.createIcons();const{data:{session:w}}=await b.auth.getSession();if(y=w?await kt(w.user.id):null,!y){await b.auth.signOut(),o.classList.remove("hidden"),i&&i.classList.add("hidden"),alert("Perfil não encontrado. Entre em contato com o administrador.");return}if(y.active===!1){await b.auth.signOut(),o.classList.remove("hidden"),i&&i.classList.add("hidden"),alert("Seu acesso está pausado. Entre em contato com o administrador.");return}_t(),ye((y==null?void 0:y.tenant_id)||null),Ge(y),St(y),Lt(y.role),await Xe(),await At(y),window.lucide&&lucide.createIcons(),ae("dashboard")}else alert("E-mail ou senha incorretos")}catch(k){alert("Erro ao fazer login: "+((k==null?void 0:k.message)||String(k)))}finally{h&&(h.disabled=!1,h.textContent="Entrar")}}))}}else et();await ce();const s=(()=>{try{return localStorage.getItem("lang")||"pt"}catch{return"pt"}})();Et(s),wt(Ce),window._applyDynamicContent=Et,window._applyWhatsAppLinks=wt,document.querySelectorAll(".nav-dropdown-btn").forEach(r=>{var u;const m=(u=r.closest(".nav-dropdown"))==null?void 0:u.querySelector(".nav-dropdown-menu");m&&r.addEventListener("click",x=>{x.stopPropagation(),m.classList.toggle("js-open"),document.querySelectorAll(".nav-dropdown-menu.js-open").forEach(E=>{E!==m&&E.classList.remove("js-open")})})}),document.addEventListener("click",()=>{document.querySelectorAll(".nav-dropdown-menu.js-open").forEach(r=>r.classList.remove("js-open"))})});async function Ya(){const e=C.filter(o=>!o.reference);if(!e.length)return;const t=C.map(o=>o.reference||"").filter(o=>/^IO-\d+$/.test(o)).map(o=>parseInt(o.replace("IO-",""),10));let n=t.length?Math.max(...t)+1:1;const a=[...e].sort((o,i)=>o.id-i.id);for(const o of a){const i="IO-"+String(n).padStart(4,"0"),{error:s}=await b.from("properties").update({reference:i}).eq("id",o.id);if(!s){const l=C.findIndex(d=>d.id===o.id);l>=0&&(C[l].reference=i),n++}}Me(gt(C))}async function Ka(){const e=C.filter(t=>{var n;return(n=t.images)==null?void 0:n.some(a=>!a.includes("/wm-"))});if(e.length){for(const t of e){if(!t.images.some(i=>!i.includes("/wm-")))continue;const a=[];let o=!1;for(const i of t.images)if(i.includes("/wm-"))a.push(i);else try{const s=await Qa(i);a.push(s),o=!0}catch{a.push(i)}if(o){await b.from("properties").update({images:a}).eq("id",t.id);const i=C.findIndex(s=>s.id===t.id);i>=0&&(C[i].images=a)}}Me(gt(C))}}async function Qa(e){try{const t=await fetch(e);if(!t.ok)return e;const n=await t.blob(),a=URL.createObjectURL(n),o=await fetch("/logo.png"),i=o.ok?await o.blob():null,s=i?URL.createObjectURL(i):null;return new Promise(l=>{const d=new Image;d.onload=()=>{URL.revokeObjectURL(a);const c=document.createElement("canvas"),r=1200;let m=d.width,u=d.height;m>r&&(u=Math.round(u*r/m),m=r),c.width=m,c.height=u;const x=c.getContext("2d");x.drawImage(d,0,0,m,u);const E=p=>{if(p){const f=Math.round(m*.18),g=Math.round(p.naturalHeight*f/p.naturalWidth),h=Math.round(m*.02);x.globalAlpha=.45,x.drawImage(p,m-f-h,u-g-h,f,g),x.globalAlpha=1}c.toBlob(async f=>{try{const g=`wm-${Date.now()}-${Math.random().toString(36).slice(2)}.jpg`,{error:h}=await b.storage.from("imoveis").upload(g,f,{contentType:"image/jpeg",cacheControl:"31536000",upsert:!1});if(h){console.error("Upload watermark error:",h),l(e);return}const{data:{publicUrl:I}}=b.storage.from("imoveis").getPublicUrl(g);l(I)}catch(g){console.error("Watermark upload exception:",g),l(e)}},"image/jpeg",.82)};if(s){const p=new Image;p.onload=()=>{URL.revokeObjectURL(s),E(p)},p.onerror=()=>{URL.revokeObjectURL(s),E(null)},p.src=s}else E(null)},d.onerror=()=>{URL.revokeObjectURL(a),l(e)},d.src=a})}catch(t){return console.error("applyWatermarkToUrl error:",t),e}}function X(e,t){e&&(e.textContent=t?"✓ Salvo com sucesso!":"✗ Erro ao salvar.",e.className="cfg-save-msg "+(t?"ok":"err"),e.style.display="",setTimeout(()=>{e.style.display="none"},3e3))}async function bt(e,t="assets"){const n=await Re(e,1200,.85),a=`${t}/${Date.now()}-${Math.random().toString(36).slice(2)}.jpg`,{error:o}=await b.storage.from("imoveis").upload(a,n,{contentType:"image/jpeg",cacheControl:"31536000",upsert:!1});if(o)throw o;const{data:{publicUrl:i}}=b.storage.from("imoveis").getPublicUrl(a);return i}async function ea(){const e=document.getElementById("section-empresa");if(!e||e.dataset.loaded)return;e.dataset.loaded="1";const{data:t}=await b.from("settings").select("key,value").eq("tenant_id",z()),n={};t==null||t.forEach(o=>{n[o.key]=o.value||""});const a=o=>v(String(n[o]||""));e.innerHTML=`
    <div class="section-topbar">
  ...
  `,await R())}async function R(){var x,E;const e=document.getElementById("crm-body");if(!e)return;const t=z(),[{data:n},{data:a},{data:o},{data:i}]=await Promise.all([b.from("crm_pipelines").select("*").eq("tenant_id",t).order("sort_order"),b.from("crm_stages").select("*").eq("tenant_id",t).order("sort_order"),b.from("crm_tags").select("*").eq("tenant_id",t).order("name"),b.from("crm_lead_statuses").select("*").eq("tenant_id",t).order("sort_order")]),s=n||[],l=s.find(p=>p.is_default)||s[0],d=s.map(p=>`<option value="${p.id}"${p.id===(l==null?void 0:l.id)?" selected":""}>${v(p.name)}</option>`).join(""),c=(a||[]).filter(p=>p.pipeline_id===(l==null?void 0:l.id)),r=c.map((p,f)=>`
    <div class="stage-item" data-id="${p.id}" data-idx="${f}">
      <div class="stage-color-dot" style="background:${p.color}"></div>
      <input type="text" class="stage-name-input form-control" value="${v(p.name)}" data-sid="${p.id}" data-orig="${v(p.name)}" style="flex:1;font-size:14px;padding:6px 10px;margin:0 4px;border:1px solid transparent;background:transparent" placeholder="Nome da etapa">
      <button class="icon-btn stage-up" data-id="${p.id}" data-idx="${f}" title="Mover para cima" ${f===0?'disabled style="opacity:0.3"':""}>▲</button>
      <button class="icon-btn stage-down" data-id="${p.id}" data-idx="${f}" title="Mover para baixo" ${f===c.length-1?'disabled style="opacity:0.3"':""}>▼</button>
  ...
  `,document.getElementById("crm-add-stage").addEventListener("click",async()=>{const p=document.getElementById("crm-new-stage").value.trim(),f=document.getElementById("crm-new-stage-color").value,g=parseInt(document.getElementById("crm-pipe-sel").value,10);p&&(await b.from("crm_stages").insert({pipeline_id:g,name:p,color:f,sort_order:99,tenant_id:z()}),document.getElementById("crm-new-stage").value="",await R())}),e.querySelectorAll(".stage-del").forEach(p=>{p.addEventListener("click",async()=>{confirm("Remover esta etapa?")&&(await b.from("crm_stages").delete().eq("id",p.dataset.id),await R())})}),e.querySelectorAll(".stage-name-input").forEach(p=>{p.addEventListener("focus",()=>{p.style.border="1px solid var(--gold,#C9A227)",p.style.background="#fff"}),p.addEventListener("blur",async()=>{p.style.border="1px solid transparent",p.style.background="transparent";const f=p.value.trim(),g=p.dataset.orig;if(!f||f===g){p.value=g;return}await b.from("crm_stages").update({name:f}).eq("id",p.dataset.sid),p.dataset.orig=f}),p.addEventListener("keydown",f=>{f.key==="Enter"&&(f.preventDefault(),p.blur()),f.key==="Escape"&&(p.value=p.dataset.orig,p.blur())})}),e.querySelectorAll(".stage-up").forEach(p=>{p.addEventListener("click",async()=>{if(p.disabled)return;const f=parseInt(p.dataset.idx,10);if(f<=0)return;const g=c[f],h=c[f-1];await Promise.all([b.from("crm_stages").update({sort_order:h.sort_order??f-1}).eq("id",g.id),b.from("crm_stages").update({sort_order:g.sort_order??f}).eq("id",h.id)]),await R()})}),e.querySelectorAll(".stage-down").forEach(p=>{p.addEventListener("click",async()=>{if(p.disabled)return;const f=parseInt(p.dataset.idx,10);if(f>=c.length-1)return;const g=c[f],h=c[f+1];await Promise.all([b.from("crm_stages").update({sort_order:h.sort_order??f+1}).eq("id",g.id),b.from("crm_stages").update({sort_order:g.sort_order??f}).eq("id",h.id)]),await R()})}),e.querySelectorAll(".stage-color-pick").forEach(p=>{p.addEventListener("change",async f=>{await b.from("crm_stages").update({color:f.target.value}).eq("id",p.dataset.sid);const g=p.closest(".stage-item").querySelector(".stage-color-dot");g&&(g.style.background=f.target.value)})});const u=async()=>{const p=document.getElementById("crm-new-tag"),f=document.getElementById("crm-new-tag-color"),g=p==null?void 0:p.value.trim(),h=(f==null?void 0:f.value)||"#6366F1";if(!g){p==null||p.focus();return}await b.from("crm_tags").insert({name:g,color:h,tenant_id:z()}),p&&(p.value=""),await R()};(x=document.getElementById("crm-add-tag"))==null||x.addEventListener("click",u),(E=document.getElementById("crm-new-tag"))==null||E.addEventListener("keydown",p=>{p.key==="Enter"&&u()}),e.querySelectorAll(".tm-del-btn").forEach(p=>{p.addEventListener("click",async()=>{confirm("Excluir esta tag? Os leads que a possuem não serão afetados.")&&(await b.from("crm_tags").delete().eq("id",p.dataset.id),await R())})}),e.querySelectorAll(".tm-name-input").forEach(p=>{const f=p.closest(".tm-row"),g=f==null?void 0:f.querySelector(".tm-save-btn");g&&(g.style.display="none"),p.addEventListener("input",()=>{const h=p.value.trim()!==p.dataset.orig;g&&(g.style.display=h?"":"none")})}),e.querySelectorAll(".tm-color-input").forEach(p=>{const f=p.closest(".tm-row"),g=f==null?void 0:f.querySelector(".tm-color-swatch"),h=f==null?void 0:f.querySelector(".tm-save-btn");p.addEventListener("input",I=>{g&&(g.style.background=I.target.value),h&&(h.style.display="")})}),e.querySelectorAll(".tm-save-btn").forEach(p=>{p.style.display="none",p.addEventListener("click",async()=>{var I,S;const f=p.closest(".tm-row"),g=(I=f.querySelector(".tm-name-input"))==null?void 0:I.value.trim(),h=(S=f.querySelector(".tm-color-input"))==null?void 0:S.value;g&&(p.disabled=!0,p.textContent="✓ Salvando…",await b.from("crm_tags").update({name:g,color:h}).eq("id",p.dataset.id),await R())})}),e.querySelectorAll(".tm-tpl-btn").forEach(p=>{p.addEventListener("click",async()=>{const f=p.dataset.name,g=p.dataset.color;p.disabled=!0,p.innerHTML="✓",await b.from("crm_tags").insert({name:f,color:g,tenant_id:z()}),await R()})}),e.querySelectorAll(".tag-chip-del").forEach(p=>{p.addEventListener("click",async()=>{await b.from("crm_tags").delete().eq("id",p.dataset.id),await R()})}),document.getElementById("crm-add-status").addEventListener("click",async()=>{const p=document.getElementById("crm-new-status").value.trim(),f=document.getElementById("crm-new-status-color").value,g=document.getElementById("crm-new-status-final").checked;p&&(await b.from("crm_lead_statuses").insert({name:p,color:f,is_final:g,sort_order:99,tenant_id:z()}),document.getElementById("crm-new-status").value="",await R())}),e.querySelectorAll(".status-del").forEach(p=>{p.addEventListener("click",async()=>{confirm("Remover este status?")&&(await b.from("crm_lead_statuses").delete().eq("id",p.dataset.id),await R())})}),document.getElementById("crm-add-pipeline").addEventListener("click",async()=>{var g;const p=(g=prompt("Nome do novo funil:"))==null?void 0:g.trim();if(!p)return;const{error:f}=await b.from("crm_pipelines").insert({name:p,sort_order:99,tenant_id:z()});if(f){alert("Erro ao criar funil: "+f.message);return}it=!1,await R()})}async function oa(){const e=document.getElementById("section-integracoes");if(!e||e.dataset.loaded)return;e.dataset.loaded="1";const{data:t}=await b.from("integrations").select("*"),n={};t==null||t.forEach(l=>{n[l.key]=l});const a=l=>{var d;return v(((d=n[l])==null?void 0:d.value)||"")},o=l=>{var d;return(d=n[l])!=null&&d.enabled?"checked":""},i=[{key:"meta_pixel_id",icon:"📘",label:"Meta Pixel",desc:"ID do Pixel do Facebook/Instagram para rastreamento de conversões",placeholder:"123456789012345"},{key:"ga_measurement_id",icon:"📊",label:"Google Analytics 4",desc:"Measurement ID do GA4 (ex: G-XXXXXXXXXX)",placeholder:"G-XXXXXXXXXX"},{key:"gtm_container_id",icon:"🏷️",label:"Google Tag Manager",desc:"ID do container do GTM (ex: GTM-XXXXXXX)",placeholder:"GTM-XXXXXXX"},{key:"webhook_new_lead",icon:"🔔",label:"Webhook — Novo Lead",desc:"URL chamada quando um novo lead chega (POST com JSON)",placeholder:"https://..."},{key:"webhook_new_property",icon:"🏠",label:"Webhook — Novo Imóvel",desc:"URL chamada quando um imóvel é publicado",placeholder:"https://..."}],s=[{key:"smtp_host",label:"Host SMTP",placeholder:"smtp.gmail.com"},{key:"smtp_port",label:"Porta",placeholder:"587"},{key:"smtp_user",label:"Usuário",placeholder:"email@dominio.com"},{key:"smtp_from_name",label:"Nome do remetente",placeholder:"Omar Corretor"}];e.innerHTML=`
    <div class="section-topbar">
  ...
  `,document.body.appendChild(t),b.from("plans").select("id, name").then(({data:c})=>{const r=document.getElementById("nt-plan");r&&c&&(r.innerHTML='<option value="">Sem plano</option>'+c.map(m=>`<option value="${m.id}">${v(m.name)}</option>`).join(""))}),(a=document.getElementById("nt-name"))==null||a.addEventListener("input",c=>{const r=document.getElementById("nt-slug");r&&!r.dataset.manual&&(r.value=an(c.target.value))}),(o=document.getElementById("nt-slug"))==null||o.addEventListener("input",c=>{c.target.dataset.manual="1"}),(i=document.getElementById("nt-pwd-toggle"))==null||i.addEventListener("click",()=>{const c=document.getElementById("nt-admin-password");c.type=c.type==="password"?"text":"password"});const n=()=>t.remove();(s=document.getElementById("sa-modal-close-btn"))==null||s.addEventListener("click",n),(l=document.getElementById("nt-cancel"))==null||l.addEventListener("click",n),t.addEventListener("click",c=>{c.target===t&&n()}),(d=document.getElementById("nt-save"))==null||d.addEventListener("click",async()=>{var L,k,w,$,_,A,B,q,T,V,O,H;const c=(k=(L=document.getElementById("nt-name"))==null?void 0:L.value)==null?void 0:k.trim(),r=($=(w=document.getElementById("nt-slug"))==null?void 0:w.value)==null?void 0:$.trim(),m=(A=(_=document.getElementById("nt-domain"))==null?void 0:_.value)==null?void 0:A.trim(),u=(B=document.getElementById("nt-plan"))==null?void 0:B.value,x=(T=(q=document.getElementById("nt-admin-email"))==null?void 0:q.value)==null?void 0:T.trim(),E=(O=(V=document.getElementById("nt-admin-password"))==null?void 0:V.value)==null?void 0:O.trim(),p=document.getElementById("nt-msg"),f=document.getElementById("nt-save");if(!c||!r){p.textContent="❌ Nome e slug são obrigatórios.",p.style.color="#ef4444";return}if(!x){p.textContent="❌ Informe o e-mail do admin.",p.style.color="#ef4444";return}if(!E||E.length<6){p.textContent="❌ A senha precisa ter mínimo 6 caracteres.",p.style.color="#ef4444";return}f.disabled=!0,f.textContent="Criando…",p.textContent="⏳ Criando imobiliária…",p.style.color="#64748b";const{data:g,error:h}=await b.from("tenants").insert({name:c,slug:r,domain:m||null,plan_id:u||null,active:!0}).select();if(h){f.disabled=!1,f.textContent="Criar Imobiliária",p.textContent="❌ "+h.message,p.style.color="#ef4444";return}const I=(H=g==null?void 0:g[0])==null?void 0:H.id;p.textContent="⏳ Criando usuário admin…";const S=await ue({email:x,password:E,role:"admin",tenant_id:I});if(!(S!=null&&S.success)){f.disabled=!1,f.textContent="Criar Imobiliária",p.innerHTML="⚠️ Imobiliária criada, mas erro ao criar usuário: "+v((S==null?void 0:S.error)||"Desconhecido"),p.style.color="#f59e0b",setTimeout(()=>{n(),de()},3e3);return}I&&(S!=null&&S.user_id)&&!(S!=null&&S.linked)&&await b.from("profiles").update({tenant_id:I}).eq("id",S.user_id),f.disabled=!1,f.textContent="Criar Imobiliária",S.email_sent===!1?(p.innerHTML=`
    ...
    <div style="background:#fff;border-radius:16px;width:100%;max-width:680px;max-height:90vh;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 24px 60px rgba(0,0,0,.25);">
      ...
      <div style="background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 1px 6px rgba(0,0,0,.07);">
        ...
        <table style="width:100%;border-collapse:collapse;min-width:560px;">
          ...
          <tbody>${r.map(u=>{const[x,E]=m(u.stage||u.status||"").split(","),p=(u.phone||"").replace(/\D/g,"");return`<tr data-lid="${u.id}" style="border-top:1px solid #f1f5f9;">
              ...
              <td style="padding:12px 16px;"><span style="background:${x};color:${E};font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px;">${v(u.stage||u.status||"Novo")}</span></td>
              <td style="padding:12px 16px;font-size:12px;color:#94a3b8;">${new Date(u.created_at).toLocaleDateString("pt-BR")}</td>
              <td style="padding:12px 16px;text-align:center;">
                <div style="display:flex;gap:6px;justify-content:center;align-items:center;">
                  ${p?`<a href="https://wa.me/${p}" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;justify-content:center;width:28px;height:28px;background:#25d366;border-radius:6px;color:#fff;text-decoration:none;flex-shrink:0;" title="WhatsApp" onclick="fbq('track', 'Contact')"><svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.123.555 4.116 1.527 5.845L.057 23.882l6.199-1.625A11.934 11.934 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.818a9.793 9.793 0 01-4.992-1.368l-.358-.213-3.685.967.983-3.596-.234-.369A9.79 9.79 0 012.182 12C2.182 6.57 6.57 2.182 12 2.182S21.818 6.57 21.818 12 17.43 21.818 12 21.818z"/></svg></a>`:""}
      ...
      <div style="display:grid;gap:20px;max-width:800px;">
        ...
        <div style="background:#fff;border-radius:12px;padding:24px;box-shadow:0 1px 6px rgba(0,0,0,.07);">
          ...
          <a href="${v(u)}" target="_blank" style="display:inline-block;background:#c9a84c;color:#fff;padding:10px 20px;border-radius:8px;text-decoration:none;font-size:13px;font-weight:700;">${p}</a>
      ...
      </div>`,(l=document.getElementById("tp-copy-key"))==null||l.addEventListener("click",()=>{var h;(h=navigator.clipboard)==null||h.writeText(e.id);const f=document.getElementById("tp-copy-key"),g=f.textContent;f.textContent="✅ Copiada!",setTimeout(()=>{f.textContent=g},2e3)})}if(t==="config"){const{data:r}=await b.from("settings").select("key,value").eq("tenant_id",e.id),m={};r==null||r.forEach(x=>{m[x.key]=x.value});const u=(x,E)=>`
      ...
      </div>`,(c=document.getElementById("tp-open-edit"))==null||c.addEventListener("click",()=>sa(e))}}function sa(e){var c,r,m,u,x,E,p,f;const t=document.getElementById("sa-edit-tenant-modal");t&&t.remove();const n=document.createElement("div");n.id="sa-edit-tenant-modal",n.className="sa-modal-backdrop";const a="https://onknpbzdcrhbfozzvxtz.supabase.co/functions/v1/public-api";n.innerHTML=`
    <div class="sa-modal" style="max-width:560px;">
      ...
      <div id="et-pane-api" class="sa-modal-body" style="display:none;">
        ...
        <div style="display:flex;flex-direction:column;gap:8px;">
          ${[["GET","properties","Lista imóveis publicados"],["GET","properties/ID","Detalhe de um imóvel"],["POST","leads","Registra lead / formulário de contato"],["GET","settings","Dados da empresa (nome, WhatsApp, logo…)"]].map(([g,h,I])=>`
...
await fetch(\`\${API}/leads?key=\${KEY}\`, {
  ...
  `,document.body.appendChild(n),b.from("plans").select("id, name").then(({data:g})=>{const h=document.getElementById("et-plan");h&&g&&(h.innerHTML='<option value="">Sem plano</option>'+g.map(I=>`<option value="${I.id}"${String(I.id)===String(e.plan_id)?" selected":""}>${v(I.name)}</option>`).join(""))}),(c=document.getElementById("et-logo-input"))==null||c.addEventListener("change",g=>{const h=g.target.files[0];if(!h)return;const I=URL.createObjectURL(h),S=document.getElementById("et-logo-preview");S&&(S.innerHTML=`<img src="${I}" style="width:100%;height:100%;object-fit:cover;">`)}),(r=document.getElementById("et-logo-preview"))==null||r.addEventListener("click",()=>{var g;(g=document.getElementById("et-logo-input"))==null||g.click()}),(m=document.getElementById("et-pwd-toggle"))==null||m.addEventListener("click",()=>{const g=document.getElementById("et-admin-password");g.type=g.type==="password"?"text":"password"}),(u=document.getElementById("et-copy-key"))==null||u.addEventListener("click",()=>{var S,L;const g=(S=document.getElementById("et-api-key"))==null?void 0:S.value;if(!g)return;(L=navigator.clipboard)==null||L.writeText(g);const h=document.getElementById("et-copy-key"),I=h.textContent;h.textContent="✅ Copiada!",setTimeout(()=>{h.textContent=I},2e3)});const o=["dados","config","api"];function i(g){o.forEach(h=>{document.getElementById(`et-pane-${h}`).style.display=h===g?"":"none";const I=document.getElementById(`et-tab-${h}`);I.style.borderBottomColor=h===g?"#2563eb":"transparent",I.style.color=h===g?"#2563eb":"#64748b",I.style.fontWeight=h===g?"600":"500"}),g==="config"&&l()}o.forEach(g=>{var h;return(h=document.getElementById(`et-tab-${g}`))==null?void 0:h.addEventListener("click",()=>i(g))});let s=!1;async function l(){var I;if(s)return;s=!0;const{data:g}=await b.from("settings").select("key,value").eq("tenant_id",e.id),h={};g==null||g.forEach(S=>{h[S.key]=S.value}),document.getElementById("et-pane-config").innerHTML=`
      <div class="form-group">
        <label>WhatsApp <span style="font-size:11px;color:#94a3b8;">(DDI+DDD+número, sem espaços ou símbolos)</span></label>
        <input id="et-cfg-wa"     class="form-input" type="text"  value="${v(h["company.whatsapp"]||"")}" placeholder="5547999701743">
      </div>
      <div class="form-group">
        <label>Telefone</label>
        <input id="et-cfg-phone"  class="form-input" type="text"  value="${v(h["company.phone"]||"")}"    placeholder="(47) 9 9970-1743">
      </div>
      <div class="form-group">
        <label>E-mail de contato</label>
        <input id="et-cfg-email"  class="form-input" type="email" value="${v(h["company.email"]||"")}"    placeholder="contato@nicimobiliaria.com.br">
      </div>
      <div class="form-group">
        <label>Cidade</label>
        <input id="et-cfg-city"   class="form-input" type="text"  value="${v(h["company.city"]||h["company.address"]||"")}" placeholder="Blumenau, SC">
      </div>
      <div class="form-group">
        <label>Slogan</label>
        <input id="et-cfg-slogan" class="form-input" type="text"  value="${v(h["company.slogan"]||"")}"   placeholder="Os melhores imóveis da região">
      </div>
    ...
    `,(I=document.getElementById("et-cfg-save"))==null||I.addEventListener("click",async()=>{const S=document.getElementById("et-cfg-save"),L=document.getElementById("et-cfg-msg");S.disabled=!0,S.textContent="Salvando…",L.textContent="",L.style.color="#64748b";const k=document.getElementById("et-cfg-wa").value.trim().replace(/\D/g,""),w=document.getElementById("et-cfg-phone").value.trim(),$=document.getElementById("et-cfg-email").value.trim(),_=document.getElementById("et-cfg-city").value.trim(),A=document.getElementById("et-cfg-slogan").value.trim(),{error:B}=await b.from("settings").upsert([{key:"company.whatsapp",value:k,tenant_id:e.id},{key:"company.phone",value:w,tenant_id:e.id},{key:"company.email",value:$,tenant_id:e.id},{key:"company.city",value:_,tenant_id:e.id},{key:"company.address",value:_,tenant_id:e.id},{key:"company.slogan",value:A,tenant_id:e.id}],{onConflict:"tenant_id,key"});S.disabled=!1,S.textContent="💾 Salvar configurações",B?(L.textContent="❌ "+B.message,L.style.color="#ef4444"):(L.textContent="✅ Configurações salvas!",L.style.color="#22c55e")})}const d=()=>n.remove();(x=document.getElementById("et-close"))==null||x.addEventListener("click",d),(E=document.getElementById("et-cancel"))==null||E.addEventListener("click",d),n.addEventListener("click",g=>{g.target===n&&d()}),(p=document.getElementById("et-delete"))==null||p.addEventListener("click",async()=>{if(!confirm(`⚠️ Tem certeza que deseja EXCLUIR a imobiliária "${e.name}"?

Essa ação é irreversível e removerá o registro da plataforma.`))return;const h=document.getElementById("et-delete");h.disabled=!0,h.textContent="Excluindo…";const{error:I}=await b.from("tenants").delete().eq("id",e.id);if(I){alert("Erro ao excluir: "+I.message),h.disabled=!1,h.textContent="🗑️ Excluir";return}d(),de()}),(f=document.getElementById("et-save"))==null||f.addEventListener("click",async()=>{var q,T,V,O,H,ne,re,ke,Se,P,ve,ht;const g=(T=(q=document.getElementById("et-name"))==null?void 0:q.value)==null?void 0:T.trim(),h=(O=(V=document.getElementById("et-slug"))==null?void 0:V.value)==null?void 0:O.trim(),I=(ne=(H=document.getElementById("et-domain"))==null?void 0:H.value)==null?void 0:ne.trim(),S=(re=document.getElementById("et-plan"))==null?void 0:re.value,L=(Se=(ke=document.getElementById("et-admin-email"))==null?void 0:ke.value)==null?void 0:Se.trim(),k=(ve=(P=document.getElementById("et-admin-password"))==null?void 0:P.value)==null?void 0:ve.trim(),w=(ht=document.getElementById("et-logo-input"))==null?void 0:ht.files[0],$=document.getElementById("et-msg"),_=document.getElementById("et-save");if(!g){$.textContent="❌ Nome é obrigatório.",$.style.color="#ef4444";return}_.disabled=!0,_.textContent="Salvando…",$.textContent="⏳ Salvando…",$.style.color="#64748b";let A=e.logo_url;if(w)try{const M=await Re(w,256,.85),xt=`tenant-logos/${e.id}-${Date.now()}.jpg`,{error:ma}=await b.storage.from("imoveis").upload(xt,M,{contentType:"image/jpeg",upsert:!0});if(!ma){const{data:{publicUrl:pa}}=b.storage.from("imoveis").getPublicUrl(xt);A=pa}}catch(M){console.error("Logo upload:",M)}const{error:B}=await b.from("tenants").update({name:g,slug:h||e.slug,domain:I||null,plan_id:S||null,logo_url:A}).eq("id",e.id);if(B){_.disabled=!1,_.textContent="Salvar",$.textContent="❌ "+B.message,$.style.color="#ef4444";return}if(L&&k&&k.length>=6){$.textContent="⏳ Criando usuário admin…";const M=await ue({email:L,password:k,role:"admin",tenant_id:e.id});M!=null&&M.success?(M!=null&&M.user_id&&!(M!=null&&M.linked)&&await b.from("profiles").update({tenant_id:e.id}).eq("id",M.user_id),$.textContent="✅ Salvo e admin criado!",$.style.color="#22c55e"):($.textContent="⚠️ Salvo, mas erro ao criar admin: "+((M==null?void 0:M.error)||"Tente novamente"),$.style.color="#f59e0b")}else $.textContent="✅ Imobiliária atualizada!",$.style.color="#22c55e";_.disabled=!1,_.textContent="Salvar",setTimeout(()=>{d(),de()},1200)})}const Rt=[{key:"name",label:"Nome",required:!0},{key:"phone",label:"Telefone",required:!1},{key:"email",label:"E-mail",required:!1},{key:"notes",label:"Notas",required:!1}];let Qe=[],fe=[],Ue={};function dn(){var e;(e=document.getElementById("btn-import-leads"))==null||e.addEventListener("click",rn)}function rn(){Qe=[],fe=[],Ue={};const e=document.createElement("div");e.id="import-leads-overlay",e.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;",e.innerHTML=`
...
`).map(l=>l.trimEnd()).filter(l=>l.length);if(a.length<2)return{error:"Arquivo vazio ou sem dados."};function o(l){const d=[];let c="",r=!1;for(let m=0;m<l.length;m++){const u=l[m];u==='"'?r&&l[m+1]==='"'?(c+='"',m++):r=!r:u===n&&!r?(d.push(c.trim()),c=""):c+=u}return d.push(c.trim()),d}const i=o(a[0]).map(l=>l.replace(/^["']+|["']+$/g,"")),s=a.slice(1).map(o);return{headers:i,rows:s}}async function cn(e){const t=document.getElementById("import-upload-error");try{window.XLSX||await new Promise((i,s)=>{const l=document.createElement("script");l.src="https://cdnjs.cloudflare.com/ajax/libs/xlsx/0.18.5/xlsx.full.min.js",l.onload=i,l.onerror=s,document.head.appendChild(l)});const n=window.XLSX.read(e,{type:"array"}),a=n.Sheets[n.SheetNames[0]],o=window.XLSX.utils.sheet_to_json(a,{header:1,defval:""});if(!o||o.length<2){t.textContent="Planilha vazia.",t.style.display="";return}fe=o[0].map(String),Qe=o.slice(1),ra()}catch(n){t.textContent="Erro ao ler o arquivo Excel: "+n.message,t.style.display=""}}function mn(e,t){for(let n=0;n<e.length;n++){const a=e[n].toLowerCase();if(t.some(o=>a.includes(o)))return n}return""}function ra(){document.getElementById("import-step-upload").style.display="none",document.getElementById("import-step-map").style.display="";const e=document.getElementById("import-field-rows"),t={name:["nome","name","contact","cliente","contato"],phone:["tel","fone","celular","whatsapp","phone","mobile"],email:["email","e-mail","mail"],notes:["obs","nota","note","comment","coment","descri"]},n='<option value="">— ignorar —</option>'+fe.map((a,o)=>`<option value="${o}">${v(a)}</option>`).join("");e.innerHTML=Rt.map(a=>{const o=mn(fe,t[a.key]||[]);return Ue[a.key]=o!==""?parseInt(o):"",`
      ...
      </div>`}).join("")}function f(){i.classList.remove("hidden"),p(""),s.value="",c.classList.add("hidden"),s.focus()}function g(){i.classList.add("hidden")}o.addEventListener("click",I=>{I.stopPropagation(),i.classList.contains("hidden")?f():g()}),document.addEventListener("mousedown",function I(S){e.contains(S.target)||(g(),document.removeEventListener("mousedown",I))}),i.addEventListener("mousedown",I=>I.stopPropagation()),s.addEventListener("input",()=>p(s.value)),l.addEventListener("click",I=>{const S=I.target.closest(".ldp-tag-opt");if(!S)return;const L=S.dataset.tag,k=x();k.includes(L)?E(k.filter(w=>w!==L)):E([...k,L]),p(s.value)}),a.addEventListener("click",I=>{const S=I.target.closest(".ldp-tag-rm");if(!S)return;const L=S.dataset.tag;E(x().filter(k=>k!==L))}),d.addEventListener("click",()=>{c.classList.toggle("hidden"),c.classList.contains("hidden")||r.focus()});async function h(){const I=r.value.trim();if(!I){r.focus();return}const S=m.value||"#6366F1";if(t.some(L=>L.name.toLowerCase()===I.toLowerCase())){r.style.borderColor="#ef4444",setTimeout(()=>{r.style.borderColor=""},1500);return}u.disabled=!0,u.textContent="Criando…";try{const{data:L,error:k}=await b.from("crm_tags").insert({name:I,color:S,tenant_id:y==null?void 0:y.tenant_id}).select().single();if(k)throw k;const w={id:L.id,name:L.name,color:L.color};t.push(w),n[w.name]=w,typeof me<"u"&&(me[w.name]=w),E([...x(),w.name]),p(s.value),r.value="",m.value="#6366F1",c.classList.add("hidden")}catch(L){console.error("Error creating tag:",L),alert("Erro ao criar tag: "+(L.message||L))}finally{u.disabled=!1,u.textContent="Criar e adicionar"}}u.addEventListener("click",h),r.addEventListener("keydown",I=>{I.key==="Enter"&&h()})}async function fn(){var f;const e=document.getElementById("section-dashboard");if(!e)return;e.dataset.dbInit==="1"&&(window._dbLeadsChartInstance&&(window.window._dbLeadsChartInstance.destroy(),window._dbLeadsChartInstance=null),window._dbOriginChartInstance&&(window.window._dbOriginChartInstance.destroy(),window._dbOriginChartInstance=null)),e.dataset.dbInit="1",e.innerHTML=`
<div class="db-wrap">
  ...
  <!-- KPI Cards -->
  <div class="db-kpis">
    ${["db-kpi-indigo","db-kpi-emerald","db-kpi-amber","db-kpi-sky"].map((g,h)=>`
    <div class="db-kpi ${g}" id="db-kpi-${h}">
  ...
  <div class="db-row-third">
...
</div>`;const n=new Date,a=n.getHours(),o=a<12?"Bom dia":a<18?"Boa tarde":"Boa noite",i=["Domingo","Segunda-feira","Terça-feira","Quarta-feira","Quinta-feira","Sexta-feira","Sábado"],s=["janeiro","fevereiro","março","abril","maio","junho","julho","agosto","setembro","outubro","novembro","dezembro"],l=`${i[n.getDay()]}, ${n.getDate()} de ${s[n.getMonth()]} de ${n.getFullYear()}`,d=((f=y==null?void 0:y.name)==null?void 0:f.split(" ")[0])||"Corretor",c=e.querySelector(".db-greeting");c&&(c.innerHTML=`${o}, <span class="db-greeting-name">${N(d)}</span> 👋`);const r=document.getElementById("db-subline");r&&(r.textContent=`Aqui está o resumo do seu negócio — ${l}`);let m=[],u=[];try{const[g,h]=await Promise.all([Ut(),vn()]);m=g||[],u=h||[]}catch(g){console.warn("[Dashboard] Erro ao carregar dados:",g)}yn(m,u,n);const x=m.filter(g=>g.published).length,E=u.filter(g=>mt(g.created_at)===mt(n.toISOString())).length,p=document.getElementById("db-header-chips");p&&(p.innerHTML=`
    ...
    <span class="db-chip db-chip-gold">
      ...
      ${l.split(",")[0]}
    </span>`),hn(u,m),xn(m),En(m,u,n),wn(m,u);try{await un(),jt(u,7),bn(u),document.querySelectorAll(".db-ptab").forEach(g=>{g.addEventListener("click",()=>{document.querySelectorAll(".db-ptab").forEach(I=>I.classList.remove("active")),g.classList.add("active");const h=parseInt(g.dataset.p);window._dbSelectedDays=h,jt(u,h)})})}catch(g){console.warn("[Dashboard] Chart.js não carregou:",g)}window.lucide&&lucide.createIcons(),window._dbRefreshTimer&&clearInterval(window._dbRefreshTimer),window._dbRefreshTimer=setInterval(()=>{const g=document.getElementById("dashboard-content")||document.querySelector(".db-dashboard");!g||g.offsetParent===null||typeof renderDashboard=="function"&&renderDashboard(!0)},3e4)}function N(e){return String(e||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function mt(e){return(e||"").slice(0,10)}function la(e,t){if(!e)return"—";const n=t-new Date(e),a=Math.floor(n/6e4);if(a<2)return"agora mesmo";if(a<60)return`há ${a}min`;const o=Math.floor(a/60);if(o<24)return`há ${o}h`;const i=Math.floor(o/24);return i===1?"ontem":i<7?`há ${i} dias`:new Date(e).toLocaleDateString("pt-BR",{day:"2-digit",month:"2-digit"})}function ca(e){return e>=1e6?(e/1e6).toFixed(1).replace(".",",")+"M":e>=1e3?(e/1e3).toFixed(0)+"k":String(e)}async function vn(){let e=b.from("leads").select("*").order("created_at",{ascending:!1}).limit(500);(y==null?void 0:y.role)==="corretor"?e=e.eq("assigned_to",y.id):y!=null&&y.tenant_id&&(e=e.eq("tenant_id",y.tenant_id));const{data:t,error:n}=await e;return n?(console.warn("[Dashboard] leads fetch error:",n.message),[]):t||[]}function yn(e,t,n){const a=e.length,o=e.filter(p=>p.published).length,i=t.length,s=t.filter(p=>p.stage&&p.stage!=="perdido"&&p.stage!=="fechado").length,l=new Date(n);l.setDate(l.getDate()-30);const d=new Date(n);d.setDate(d.getDate()-60);const c=e.filter(p=>p.created_at&&new Date(p.created_at)>=l).length,r=e.filter(p=>p.created_at&&new Date(p.created_at)>=d&&new Date(p.created_at)<l).length,m=t.filter(p=>p.created_at&&new Date(p.created_at)>=l).length,u=t.filter(p=>p.created_at&&new Date(p.created_at)>=d&&new Date(p.created_at)<l).length;function x(p,f,g){if(f===0&&p===0)return'<span class="db-kpi-trend db-trend-neu">Sem dados</span>';if(f===0)return'<span class="db-kpi-trend db-trend-up">▲ Novo</span>';const h=Math.round((p-f)/f*100);return h===0?'<span class="db-kpi-trend db-trend-neu">= Estável</span>':h>0?`<span class="db-kpi-trend db-trend-up">▲ +${h}% ${g}</span>`:`<span class="db-kpi-trend db-trend-down">▼ ${h}% ${g}</span>`}[{idx:0,val:a,label:"Total de Imóveis",trend:x(c,r,"este mês"),icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>'},{idx:1,val:o,label:"Publicados no Site",trend:o===0?'<span class="db-kpi-trend db-trend-neu">Nenhum publicado</span>':`<span class="db-kpi-trend db-trend-up">${Math.round(o/Math.max(a,1)*100)}% do total</span>`,icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 11-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>'},{idx:2,val:i,label:"Leads Recebidos",trend:x(m,u,"vs. mês ant."),icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/></svg>'},{idx:3,val:s,label:"Em Negociação",trend:s===0?'<span class="db-kpi-trend db-trend-neu">Nenhum ativo</span>':'<span class="db-kpi-trend db-trend-up">▲ Ativos</span>',icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6"/></svg>'}].forEach(({idx:p,val:f,label:g,trend:h,icon:I})=>{const S=document.getElementById(`db-kpi-${p}`);S&&(S.innerHTML=`
      <div class="db-kpi-icon">${I}</div>
      <div class="db-kpi-body">
        <div class="db-kpi-val">${ca(f)}</div>
